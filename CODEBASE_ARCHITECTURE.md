...
- Progress-table Select all launch orchestration is single-flight from exact-ID fetch through plan, candidate hydration, and flashcard commit. The click must synchronously open the flashcard popup and visible loader before scheduling any network work, and the same launch-UI owner must stay active through exact-ID resolution, planning, hydration, and widget commit; do not reopen the modal between stages. Disable conflicting launch, Select all, and clear controls while it owns the selection; abort or token-fence stale work after the filter/scope changes or an explicit popup dismissal; keep an inline live loading state until commit; and expose an inline Retry bound to the unchanged launch specification after a fetch, planning, or hydration failure. Only the explicit `selection_ids_only=1&include_words=0&include_word_ids=1` launch scope may use the reduced analytics contract: it validates submitted categories through the cached renderable membership map and returns immediately after projected progress filtering, while membership/progress query failures remain retryable errors. It must not rebuild full category cards/previews, media or gender aggregates, category metrics, or daily activity; the legacy `include_words=0&include_word_ids=1` contract remains unchanged. The exact-candidate launch planner reuses that membership map and hydrates only the effective presentation metadata Learning needs. The Progress render and normal analytics path also prime one user/wordset/epoch-keyed aggregate of those authoritative effective presentation fields; Learning planning must read and version-check that aggregate once, resolve only missing/stale categories, and never repeat count/aspect resolution across every category in a warm large-wordset launch. The authoritative processed-category cache key and the aggregate must share request-snapshotted category/aspect versions so concurrent invalidation cannot relabel an older row as current; optional aggregate writes fail open for the already-resolved launch, while source/version reads remain fail-closed.
