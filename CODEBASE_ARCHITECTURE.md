...
- Progress-table Select all launch orchestration is single-flight from exact-ID fetch through plan, candidate hydration, and flashcard commit. Disable conflicting launch, Select all, and clear controls while it owns the selection; abort or token-fence stale work after the filter/scope changes; keep an inline live loading state until commit; and expose an inline Retry bound to the unchanged launch specification after a fetch, planning, or hydration failure.
