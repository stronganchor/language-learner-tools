...
- `[wordset_buttons]` / `[ll_wordset_buttons]` (`includes/shortcodes/wordset-buttons-shortcode.php`).
  - Count generations are bounded and keyed by user identity plus current structural/content epochs because private-category grants can differ even for identical visible wordset IDs. Initial shortcode rendering never advances a cold count batch inline: it emits the current privacy-filtered wordset names and canonical page links without counts or media, then lets the bounded continuation progressively replace that navigation shell. The cold shell is alphabetical; the completed historical card order remains lesson-count descending. With `hide_empty="1"`, the shell follows WordPress taxonomy relationship emptiness and may briefly include a linked wordset that the completed quiz-eligible lesson count later removes. The links remain usable if count polling reaches Retry. An incomplete signed-in scope may render only complete anonymous public HTML from the structurally scoped LKG or exact anonymous cache; it must never publish partial counts or private markup to that fallback. Its authenticated, nonce-protected `ll_tools_wordset_buttons_refresh` loader advances one bounded batch at a time. A cold anonymous render is marked non-cacheable and emits a rate-limited `ll_tools_wordset_buttons_status` poll whose expiring HMAC scope token binds the normalized shortcode context; that endpoint only reads a complete current generation and schedules the deduplicated anonymous continuation, never acquires the builder lock or scans a pair. Complete anonymous HTML is published only through captured pre-render exact/LKG keys, and any mid-render epoch drift is refused. Both loaders replace their shell only after exact completion and stop at a reachable Retry state on failure. Continuation cron events carry the initiating user ID, restore that context only around the worker, and restore the prior user afterward. An authoritative complete-empty scope remains empty. Direct role-change invalidation is tracked separately in the maintenance backlog.
