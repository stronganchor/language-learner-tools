...
read_first:
  ...
  - includes/lib/mutation-job-state.php
...
- Word-editor category choices retain exact in-scope identities, including empty and image-only owned categories. Equal display labels show their unique slugs. Published-word badges use grouped SQL scoped to the current wordset, deduplicate ownerless legacy-source/copy membership, exclude foreign-owned aliases, and show unavailable after a failed read. Detached popup counts must not hydrate all words or media to calculate quiz readiness.
...
- Translated answer labels prefer the main word `translation` over a recording's full sentence translation. Keep `Util.getEffectiveOptionLabel()` and selection's compatibility fallback in agreement so correct answers, distractors and duplicate-label filtering use the same text. A missing main translation retains the existing recording/label/title fallbacks; title-only options and prompt text/audio keep their separate contracts. `tests/e2e/specs/practice-option-constraints.spec.js` guards the rendered number/place-name cases with the actual utility and card renderer.
...
- Durable wordset category search has a narrower materialization-source contract than quiz content: its generation advances only for published word title/status, current or legacy translation, and direct `wordset`/`word-category` membership changes. Store that generation in the dedicated non-autoloaded per-wordset `ll_tools_wcs_source_epoch_{id}` option; incomplete scope advances the global unknown-source option and a bounded scheduling sweep, while a failed unknown-source write advances the dedicated failsafe. Category labels/parents, audio/transcription/review edits, generic quiz-content epochs, and broad category/wordset cache epochs must not schedule a search materialization rebuild. The public AJAX result key also varies by plugin version, locale, structural category/wordset epochs, this narrow source generation, and the quiz-content epoch so hydrated image/media fields refresh without rebuilding search rows. Category-search payloads use their own structural access signature so an already-open search can query the current generation after a source edit; the shared lazy-card payload signature retains its quiz-content epoch and must still rotate for media/content freshness.
...
- Manual category ordering must preserve saved identities without reading every vocabulary lesson. Fetch first-lesson dates only for missing candidate categories through grouped SQL in chunks of 200 IDs; choose the earliest local post date with ID as the tie-breaker and retain its corresponding valid GMT timestamp. A complete saved order needs no lesson-date query. `tests/Integration/WordsetManualOrderingResourceTest.php` covers query bounds, metadata duplicates, date semantics, and failed reads.
- A cold flashcard category is a durable materialization, not an interactive full-category hydration. `includes/lib/flashcard-payload-materializer.php` advances bounded primary-word and prompt-card keyset batches into generation-scoped rows; readers expose only an unlocked, complete generation whose dependency signature still matches. That signature is owned by the table, payload, and builder schemas plus the exact scope/content epochs; an ordinary plugin release must not invalidate completed rows. Any code change that can alter materialized row shape, eligibility, media/presentation values, or stored option rules must explicitly bump `LL_TOOLS_FLASHCARD_PAYLOAD_BUILDER_SCHEMA`. Client-only label precedence using unchanged payload values does not require a rebuild; effective rendered labels and duplicate-label filtering are recalculated in the browser. The bounded recent-release compatibility history may adopt only a completed generation whose signature matches the exact former release-only formula; it must never bypass a schema, scope, visibility, locale, presentation, wordset, or content-epoch mismatch. The public/signed-in page endpoints must return retryable preparation state until a cold generation is complete, never partial quiz data disguised as a complete category. Browser warming retries must also have an overall elapsed deadline; exhaustion clears every loading surface, keeps the popup open, and exposes a translated manual Retry for the unchanged launch.
- Main wordset category rows must discard categories already proven ineligible before building content summaries, bulk-prime the exact candidate term, lesson, and wrong-answer-owner sets, and trust the exact membership established by flat `word-category` queries instead of re-querying each candidate word. Preserve the bounded initial-card/lazy-card split. If a concurrent structural generation change invalidates an authenticated lazy-card payload, rebuild its old lazy-shell identities from current authorized card data only for the same logged-in payload owner, preserving tombstone cursor positions and deferring newly added cards until reload; a current category/content source that is not provably complete returns retryable HTTP 503 without consuming the cursor, while guests, cross-user tokens, and revoked managers remain fail-closed.
...
- Logged-in broad or filtered multi-category wordset launches use `ll_user_study_selection_launch_plan`: resolve exact progress criteria against the cached ID-only category aggregate and, on success, preserve every matching word ID in balanced, category-aware transport chunks. Progress-table Select all launches with more than one transport chunk send their already-resolved canonical word IDs back as one scalar exact-candidate field (not one form variable per ID); the server intersects that scope with current renderable wordset/category membership without reapplying a progress filter. Each chunk remains capped at 30 word IDs and eight categories (15 and three preferred by default). A sparse match set that cannot be partitioned into five-word rounds even at the eight-category hard cap must return the typed fail-closed planning error instead of an incomplete plan. The browser hydrates media only for the current chunk, verifies that candidate hydration covers the planned IDs, and hands those verified rows directly to the flashcard runtime. Candidate reads must propagate the source-completeness flag: a database, visibility, configuration, or presentation read that is not provably complete returns typed HTTP 503 and is never cached or disguised as a successful empty/partial batch. Candidate option pools scan only a bounded raw window but apply the canonical quiz-eligibility projection before consuming the option limit; if that fast window still underfills, read at most 12 useful same-scope rows from a completed durable payload generation and return typed `option_pool_materializing` warming state until it is ready, without exposing or draining a cursor. A bounded handoff with fewer than two effective same-category option identities must fail atomically before quiz setup unless every target supplies a usable text wrong-answer fallback. Practice, Listen, and Self Check treat valid chunks as one logical session: they request and append the next chunk serially before results, preserve cumulative state plus full logical progress, and emit results plus mode completion only after the final chunk. Gender likewise owns one logical selection, but Level 1 pauses at results without completing the logical session while Levels 2/3 continue automatically; only the exhausted selection emits completion. Learning alone retains a results-stage Next action between deliberately separate pedagogical sets. The runtime must not issue a second AJAX fetch for the same chunk. Every selection/continuation candidate request has the configured bounded deadline; retryable `cache_warming` and `option_pool_materializing` responses may retain the existing loader across a bounded, `Retry-After`-aware loop, while timeouts, parser/network failures, and HTTP 408/425/500/502/503/504 receive at most two non-overlapping exponential-backoff retries. Both loops run only while that launch remains current. Closing, replacing, or switching mode must generation-invalidate queued and in-flight continuation work and stop retries immediately; exhausting the bound or receiving a deterministic failure must close the loader and expose Retry for the unchanged launch. Continuation errors use a plugin-owned neutral result state that hides in-round progress/mode controls and never claims that a transient failure means no content exists. Never hydrate every selected category at once or resume the former full-category payload drain.
- Progress-table Select all launch orchestration is single-flight from exact-ID acquisition through plan, candidate hydration, and flashcard commit. A complete filtered analytics page owns the exact filtered-ID snapshot when its returned ID count equals its authoritative total and its filter/visibility/category signature matches; reuse that snapshot instead of issuing a duplicate ID request, but invalidate it on every analytics refresh and progress/star mutation. The click must synchronously open the flashcard popup and visible loader before scheduling any network work, and the same launch-UI owner must stay active through exact-ID resolution, planning, hydration, and widget commit; do not reopen the modal between stages. Disable conflicting launch, Select all, and clear controls while it owns the selection; after a filter/scope change or explicit popup dismissal, actively abort whichever ID, plan, or hydration request is current and token-fence every callback and retry; keep an inline live loading state until commit; and expose an inline Retry bound to the unchanged launch specification after a fetch, planning, or hydration failure. Normal analytics refreshes are also single-flight: a newer request obligation token-invalidates the active response, merges loading/silence requirements into exactly one trailing refresh, and must settle every inherited loading indicator even when that trailing request fails. For an all-filtered snapshot of at least 1,000 words, exact-ID acquisition, planning, and initial current-chunk hydration each use three times the configured base request deadline, capped at 120 seconds; smaller selections and all later continuation/repeat hydration retain the base deadline, and every launch remains dismissible and stale-fenced. Only the explicit `selection_ids_only=1&include_words=0&include_word_ids=1` launch scope may use the reduced analytics contract: it validates submitted categories through the cached renderable membership map and returns immediately after projected progress filtering, while membership/progress query failures remain retryable errors. It must not rebuild full category cards/previews, media or gender aggregates, category metrics, or daily activity; the legacy `include_words=0&include_word_ids=1` contract remains unchanged. ID-only analytics and planner filters read starred IDs directly; they must not invoke saved-category repair merely to read that list. The exact-candidate launch planner reuses the membership map and hydrates only the effective presentation metadata Learning needs. The Progress render and normal analytics path also prime one schema/wordset/structural-content-epoch-keyed aggregate of those user-invariant effective presentation fields; routine releases and different users must reuse it. Learning planning reads and version-checks that aggregate once, resolves only missing/stale categories, and never repeats count/aspect resolution across every category in a warm large-wordset launch. The authoritative processed-category cache key and the aggregate must share request-snapshotted category/aspect versions so concurrent invalidation cannot relabel an older row as current; optional aggregate writes fail open for the already-resolved launch, while source/version reads remain fail-closed.
- Large exact Progress selections use that bounded planner for Learn, Practice, Listen, Gender, and Self Check alike. Hydrate only the first capped chunk at startup; Practice, Listen, and Self Check continue inside one pre-results logical session, Learn advances through results between bounded pedagogical sets, and Gender pauses only for Level 1 checkpoints while Levels 2/3 continue automatically.
- Every wordset-page flashcard entry point shares one launch owner and token. Starting a replacement or closing the popup aborts the tracked request, fences every retry, callback, UI mutation, and commit, settles Repeat/Continue and bounded-continuation guards, and allows only the newest launch to alter session state.
- The flashcard round header follows the concrete target category on every round, even when selection or bounded continuation has already advanced the stored category. Progress and starred criteria labels remain session/results metadata and may serve only as a fallback when no concrete round category is available; a bounded append must refresh the header as soon as the new chunk becomes current.
- Bounded Learning launches use presentation- and aspect-compatible chunks of 8-15 words across at most eight categories. Keep exact target IDs distinct from compatible filler IDs, allocate shared filler identities across all compatibility groups so an earlier flexible group cannot starve a constrained group, never expand a bounded Learning launch by hydrating complete categories or rebuilding against broader cached rows, and fail closed when a compatible minimum chunk cannot be formed. Later Learning chunks advance only through the results-screen Next action; they must not use Practice's continuous pre-results continuation.
...
- Recorder category overviews use the same bounded compact summary pipeline in both the manager and `[audio_recording_interface]`: hydrate authenticated batches through `ll_tools_recorder_queue_summaries` (six categories by default, twenty maximum), and remove categories whose completed summary has no queued work. The manager's normal visible overview always selects one assigned recorder and uses this stream; there is no separate synchronous or numbered category-overview path. Hidden overview retains recorder-level paging, and focused-category plus focused-hidden item views retain their bounded item paging. A summary retains only two preview identities, but its canonical word/image, legacy missing-audio, and prompt-card sources keep separate cumulative cursors and counts until every bounded scan genuinely exhausts; publish only the resulting exact total and never expose a lower-bound `3+` card. The manager may reuse complete cached summaries and a complete scope manifest during its PHP render, but it performs no cold summary refresh there. An unchanged manifest renders its known-positive cards immediately in canonical order and requires no summary AJAX; one stale category stays bounded to one requested batch while the remaining manifest entries remain reusable. Each AJAX batch computes one complete-catalog manifest scope/generation and reuses it for cache reads, merges, and the response. The optional aggregate stores only exact count and two-item preview card data; the richer per-category durable caches remain authoritative. In the manager stream, cold unknown identities render only a bounded set of neutral, identity-free shimmer shells, with source identities retained in hidden lightweight markers until exact summaries resolve them. Never expose a category name on an unknown manager shell that may disappear and allow a different identity to occupy its grid position. Unresolved work starts with one three-category AJAX request, then continuously advances one serial six-category batch while the dedicated end sentinel remains in or just below the viewport. There is never more than one request in flight, and moving away from the end pauses before the next batch. Normal operation has no visible loaded-count copy or Load more control. Select untouched categories before retrying pending scans, use a bounded request timeout and short retry delay, and clear `aria-busy` with an explicit error-only Retry control on timeout, incomplete catalogs, or unclassified responses. The recording shortcode renders all catalog categories as fixed-footprint native links: reuse exact-positive manifest entries during PHP render, omit exact-empty entries, and leave only stale or missing summaries as named links with a count and preview sheen. Assignment/config changes enqueue a bounded resumable full-cache warmup; a cache-schema upgrade schedules the same bounded work for existing recorder configurations in resumable user batches; a recording mutation queues a refresh only for its affected category. Keep cache warming outside the page render so the first HTML response never starts an unbounded wordset scan. The stream generation and scope manifest are structural: they key the ordered category identities plus recorder/wordset/filter scope, while each category summary and manifest entry has its own content signature. Ordinary word/audio/image/prompt or recorder-hidden changes invalidate only affected entries and must not restart or discard the rest of an otherwise valid bounded stream; true category identity/order/scope changes still reload it. The no-category shortcode route is an overview only: it must not hydrate or select a focused queue, render progress, or open New Word automatically. Its category cards navigate to dedicated `ll_record_category` pages, whose hidden active-category state supports bounded continuation and whose back link returns to the overview; do not restore the legacy visible category dropdown or in-place switching. An incomplete initial category catalog is not an authoritative empty queue: do not emit a generation or empty-state conclusion, render an explicit retry, and permit only bounded automatic retries before requiring user action. Focused-category and hidden queue item views stay paged; do not replace any of these surfaces with full-queue hydration.
...
- Recorder queue summary caches use each word-category's canonical cache version plus category-local recorder-hidden state, recording-type taxonomy changes, global desired-recording defaults, the recorder's effective `manage_options` private-category bypass, and the rarely changing structural category epoch as a wordless-image eligibility component. A hide or unhide mutation must invalidate only the summaries and scope-manifest entries whose classified category can contain that candidate; do not place one whole-recorder hidden-list hash in every categorized summary signature. User-scoped catalog, category-map, scope-manifest, and completed-summary identities must all separate the capability branch so an admin-to-recorder demotion cannot retain private category names, counts, or previews. Wordless images inherit the recording-type union of every user-visible, wordset-scoped sibling category, so sibling owner/source/privacy/access mutations and category deletion must invalidate a target summary even when the target category's own version is unchanged. Word, image, prompt-card, audio, hidden-candidate, and recording-type relationship mutations otherwise bump only their affected category versions or manifest entries; the virtual uncategorized summary conservatively retains the broader plugin epochs because it has no category identity. A matching completed summary and unchanged manifest entry stay reusable for their retention TTL (thirty days by default), without an age-only five-minute rebuild. The compact structural category catalog keeps its separate structure key.
- Expired database transients are reclaimed only by the hourly LL Tools maintenance job in `includes/lib/expired-transient-maintenance.php`: with an external object cache it skips general cache-transient rows but still scans the direct atomic guard rows that LL Tools writes to `wp_options`; otherwise it uses the full exact LL-owned cache/rate-limit prefix allowlist. Each pass selects at most 200 timeout rows after a five-minute grace period and conditionally deletes the exact timeout/value pair only while the selected timeout remains expired. Direct lease rows must also carry an embedded expiry older than the grace cutoff so cleanup cannot erase a lease between takeover and timeout refresh. A saturated/time-limited pass schedules one coalesced continuation. Do not broaden this into a global transient purge, persistent `ll_tools_*` options/jobs, request-path cleanup, or unbounded batches.
...
- `includes/bootstrap.php`
  ...
  - Recording metadata snapshots keep both the connection-decoded `meta_value` for WordPress cache/readback semantics and `HEX(meta_value)` for the exact stored bytes. Update/delete conditions must compare the captured row identity and physical bytes through `UNHEX()` while retaining the connection/lock fence; a connection charset can differ from the column charset, so encoding the decoded PHP value cannot recover the storage preimage. Outer admission, field reads, and refreshed snapshots must preserve the same shape; an absent byte snapshot must fail closed. `tests/Integration/RecordingMetadataWriteTest.php` covers this boundary.
- `includes/assets.php`
...
- `includes/lib/ai-crawler-support.php`
  ...
  - Cold anonymous export builds use atomic per-client admission and an exact-key lease. Source, generation, readiness, and lease failures return retryable non-cacheable errors; only complete bodies publish under the captured generation. Unicode browse candidates use exact generic/Turkic initial rows in the existing dictionary lookup table. A bounded background backfill must complete before those rows are authoritative; public requests never replace a missing projection with a title scan.
...
- includes/lib/wordset-category-search-incremental.php
- includes/lib/wordset-category-search-worker.php
...
includes/
  ...
  lib/
    ...
    wordset-category-search-incremental.php # Exact-word updates with source and lease fencing
    wordset-category-search-worker.php # Bounded CLI worker for managed background maintenance
...
- `ll_teacher_class` (hidden CPT)
  ...
  - Both the frontend and legacy wp-admin Classes surfaces page class/account results and hydrate at most one learner-progress page. Keep class/account queries at `page_size + 1`, use `ID ASC` after title/display-name ordering so offset pages are deterministic, preserve the selected teacher outside the current account page, globally order admin progress before applying its bounded `number`/`offset`, reset empty class pages, clamp stale final learner-page requests, and label paged progress metrics as page-scoped rather than rebuilding full-class aggregates during an interactive request. Frontend sortable columns must toggle in both directions even when their default is descending, preserve focus/`aria-sort`, and keep unavailable values at the stable tail in either direction.
...
- Semantic signature drift currently warms a replacement generation and rejects
  stale cursors. Release-only legacy signatures may be adopted in place only
  through the bounded exact-formula compatibility path described above. There
  is no general last-known-good serving path for flashcard payload rows.
...
- `/<wordset>` wordset hub pages (handled by `includes/pages/wordset-pages.php`).
  ...
  - The settings view can launch the Wordset Editor tool (`ll_wordset_tool=editor`, implemented in `includes/pages/wordset-editor.php`) for searchable word tables, media-status filters, bulk category/status/review actions, recording moves, saved views, and recent-action undo. Copy/Split Word is available both in table rows and in the shared word editor popup used by lessons (`includes/shortcodes/word-grid-shortcode.php`). It uses `includes/lib/word-copy.php` and `js/word-copy-dialog.js`; retaining the original recordings is the default, and selected moves are explicit. Popup triggers carry their wordset nonce, require saved edits, and invalidate detached markup after completion. See `docs/FRONTEND_RECORDING_TOOLS.md` and `word-copy-popup.spec.js`.
...
- Interactive category deletion from wordset and vocab-lesson pages always creates or continues the shared wordset-level durable delete job. The job persists before mutation, removes at most 25 linked lessons or words per request by default (hard cap 100), serializes the shared term-meta map with a token-fenced wordset lease plus a rollback-expiring bridge for the previous per-category lock namespace, and commits state in a transaction that locks and revalidates both lease rows. Continuations reconcile remaining rows after interrupted or failed writes, can finalize after the term is already gone, and keep Continue/Retry progress visible until completion. Category, owner, linked-content, and remaining-count reads fail closed; both the delete commit and missing-term recovery verify that the category taxonomy and term-meta rows are absent before recording completion. `ll_tools_wordset_page_run_category_delete_batch()` is the sole supported mutation helper; do not restore a synchronous compatibility wrapper.
- Global vocab-lesson settings and scheduled full syncs only queue the durable cleanup/sync reconciliation state; each cron continuation keyset-checks at most 10 existing pages or discovers at most 10 category candidates for one enabled wordset (hard cap 50), persists cursors/counts/failure state, and reschedules until completion. A full quiz/vocab request that arrives behind an active pass must persist its own CAS-token intent and distinct one-shot event, leave the active scope/cursor intact, then queue a fresh cursor-zero pass (including every enabled vocab wordset) only after the prior pass stops; worker-event transport calls and the daily safety net must not create recursive follow-up generations. Forced quiz orphan cleanup is monotonic while requests coalesce, and a request is consumed only after its exact fresh child state and normal worker event are both readable. Deactivation clears both owned transports but preserves child/request state; activation and early `admin_init` must re-arm pending requests and active child states. Reconciliation of an already-canonical lesson must not call `wp_update_post()`, rewrite identical identity meta, or rotate the wordset lesson-cache generation; title/content/status/slug/meta drift, creates, restores, and duplicate cleanup remain real mutations and must invalidate normally. Keep the synchronous `ll_tools_sync_vocab_lesson_pages()` helper out of web/admin/cron entry points.
- Enabled vocab-lesson wordsets are normalized and filtered to existing `wordset` terms with one set-based ID query before category maintenance. Preserve configured order, fail open to the normalized stored IDs on a taxonomy-query error, and do not write from this read path. Existing wordset create/delete mutations own physical pruning of stale option IDs under the auto-sync suppression guard; do not restore per-ID existence or deepest-count scans for deleted wordsets.
...
- `includes/lib/flashcard-payload-materializer.php` owns the durable full-category protocol:
  ...
  - The dependency signature deliberately excludes the plugin release version and binds the table/payload/builder schemas, canonical scope, structural/content/category epochs, the specific-wrong-answer ownership generation/integrity marker, image-similarity threshold, and masked-proxy mode. Stored signatures from the pre-stable version-bound formula may be adopted only through the bounded recognized-version compatibility path; a payload-contract change must bump the builder schema. A signature change creates a new fenced generation, while an ordinary plugin release can reuse a current generation; readers never mix rows or cursors across generations.
  - Quiz hydration fails closed while the durable specific-wrong-answer reverse-owner map is unverified. A missing map or an exact legacy bare empty array may be promoted on the public hot path only after the existing bounded double existence probe proves that no source owner rows exist and the writer/source-epoch fence publishes a complete empty generation. Non-empty, dirty, or otherwise unverified maps remain rebuild-only; never trust legacy entries or perform a full public-request scan.
...
- Export + Import tools: `includes/admin/export-import.php` (separate admin pages for bundle export and bundle import; zip of categories + word_images + attachments). A `bundle_type=wordset_template` import always creates a private staging wordset and ignores bundled visibility; publishing that staging object is a separate manager action. This does not change `ll_tools_create_wordset_from_template()`, which is the distinct end-user destination-wordset creation flow. Legacy serialized meta from a bundle, import comparison, or undo snapshot must pass through `ll_tools_import_decode_legacy_meta_value()`; it permits scalar/array compatibility only and rejects objects/resources before comparison or before existing meta is deleted.
...
- IPA Keyboard admin: `includes/admin/ipa-keyboard-admin.php`.
  - Search, paging, tab changes, and wordset changes must wait for text/IPA, review-state, and internal-note drafts to settle in their original scope. Retain only the latest requested destination; a failed save keeps the draft visible and cancels deferred navigation. Deferred blur handlers must not replay an already settled save. Search reads use bounded deadlines, explicit read retry, and request/view ownership so a late response cannot replace newer results or keyboard state. Reopening a clean loaded tab retains its existing DOM. Canonical browser coverage: `transcription-manager-review-filter-regression.spec.js`.
  - Transcription Manager search/save responses stay bounded to their requested recording window, but their keyboard configuration must merge that window with the compact wordset symbol inventory and current aggregate recording counts so ordinary keys do not disappear between result pages. A dirty search row still saves before the detached word editor fetch begins, while the modal loading shell opens synchronously in the initiating click. That prepared shell has a bounded timeout which cancels only the modal token and restores access to the manager; it must not abort, retry, or duplicate the ambiguous save mutation. Text/IPA autosaves and review-state writes for one recording must not replace a row concurrently: accept an autosave response as the new saved baseline, preserve newer field values across review-driven row replacement, and serialize follow-up work. Word-level internal review-note saves are latest-value-wins across duplicate rows; callbacks attached to a replaced row must never erase newer visible note text or report a stale value as saved.
  - `ll_tools_ipa_keyboard_update_recording_fields()` returns its underlying `WP_Error` for write failures. Result-consuming helpers and AJAX handlers preserve that error and its HTTP status (busy saves 429, unverifiable saves 503); only invalid recording/scope data receives the existing 400 validation response. Search autosave displays the server message and retains editable unsaved inputs without advancing the saved baseline on failure. `IpaKeyboardAdminAjaxTest.php` checks the actual search-save handler and error statuses; `transcription-manager-review-filter-regression.spec.js` checks visible error details, retained edits, and later retry.
...
- Broad or progress-filtered logged-in wordset selections, explicit signed-in category launches, and full-scope wordset fallbacks must launch through the same bounded server plan and candidate-specific hydration. Progress-table Select all passes the fetched canonical IDs as an exact, bounded candidate scope so the planner preserves that snapshot after intersecting it with current renderable membership instead of querying progress again. A successful plan preserves every matching ID across capped, category-aware transport chunks; Practice, Listen, and Self Check compose those chunks into one logical session, while an impossible sparse layout returns the typed fail-closed error instead of a partial plan. Direct words reserved only as specific wrong answers remain support rows for option payloads but must not enter plan target membership; a prompt card that canonically answers with the same word remains a valid generic target, but Gender requires direct published-target provenance because its runtime rejects prompt/support rows. Keep planning nonce- and wordset-access protected and exact for `new`/`studied`/`learned`/`starred`/`hard`; hydrate only the current chunk; require Practice/Listen/Learning option pools without applying that distractor invariant to Self Check or Gender; hand verified candidate rows directly to the runtime without refetching them; preserve cumulative score, progress, replay queues, and logical category scope across automatic continuations; emit completion once after the final chunk; and fail closed without leaving the popup loader busy when planning or hydration is rate-limited.
- Treat a Progress Select all launch as one single-flight operation through exact-ID acquisition, planning, bounded hydration, and commit. Reuse a complete signature-matched filtered-page ID snapshot, invalidate it on every analytics refresh and progress/star mutation, actively abort the current ID/plan/hydration request on dismissal or scope change, and token-fence all continuations. Duplicate mode clicks must not start parallel requests, and a failed unchanged operation must leave the selection intact behind one inline Retry state. For Learning, build only presentation- and aspect-compatible 8-15-word chunks with no more than eight categories, prefer a 12-word maximum for large scopes, use 13-15 only when a valid 8-word split is impossible, preserve separate exact-target and compatible-filler identities, allocate shared fillers across compatibility groups before committing chunk ownership, and advance later chunks explicitly from results rather than through automatic continuation.
...
- Wordset-page chunking must preserve full coverage of the filtered word pool and distribute words across server-planned transport chunks without duplicates or dropped leftovers (use balanced chunk sizes instead of creating tiny tail chunks that strand words). Keep each category-owned queue contiguous and order owned queues largest-first. After balancing, order runnable chunks by fewest represented categories, then fullest word count, then stable original order so the first card needs the fewest serial category requests without dropping or duplicating later work. Filtered bounded Practice, Listen, Self Check, and Gender sessions retain their filter label and full logical total across hydration boundaries. If full coverage cannot also satisfy the minimum round size and hard category cap, reject the plan explicitly. Practice, Listen, and Self Check request valid chunks serially just in time, append each verified candidate payload without reinitializing the active runtime, suppress intermediate results/actions/completion events, and retry a failed unchanged chunk without advancing. Closing the widget, replacing the launch, or switching mode must generation-invalidate both queued and in-flight continuation work before the replacement mode can render. Self Check must score a same-image word once even when its image group crosses a transport boundary. Gender planning owns every eligible selected ID exactly once, groups by effective starting level, round-robins populated Level 1/2/3 queues, caps Level 1 at 10 and Levels 2/3 at 15, checkpoints Level 1 at results without emitting logical-session completion, and automatically advances Level 2/3; the full bounded Gender selection completes exactly once. Newer browser Gender progress may re-bucket a stale server hint but must remain inside the immutable logical scope and cannot drop or duplicate the word. Learning retains explicit results-stage continuation and each deliberately separate pedagogical set completes independently. No mode may refetch a successful chunk or flatten category IDs into one all-category hydration request.
...
- Core full-locale readiness starts with the non-mutating `php scripts/check-i18n-source-pot.php` gate. A matching PO/POT count is not sufficient: the checked-in POT must match the canonical gettext keys extracted from current source, each configured core locale must cover that fresh POT, and its MO plus l10n PHP artifacts must match the PO before the catalog is called current.
...
- Public language-switcher choices are nonce-protected POST forms, never crawler-discoverable `ll_locale`/`ll_locale_nonce` links. Preserve signed GET handling only for inbound compatibility and clean redirects; do not reintroduce state-changing locale URLs into cacheable public HTML.
- Public static caches must exclude logged-in users, wp-admin, admin-ajax, REST/API, POST requests, preview/customizer requests, error/redirect responses, and any request carrying `ll_tools_auth` or `ll_tools_auth_feedback`. Front-end auth/feedback responses are private `no-store` pages and must bypass both public-cache reads and captures; the exact one-shot `ll_category_search_recovery=1` request has the same read/capture/downstream-cache bypass so it can mint a fresh search payload before the browser removes the marker. Anonymous baseline cache keys may still normalize unrelated noise such as `ll_locale_nonce` and `ll_wordset_back`.
...
- User-study progress-event, goals, recommendation, and analytics mutation collections must pass byte/count/shape admission before JSON decoding, ID mapping, or downstream queries. Oversized mutation input is rejected instead of silently truncated; stored-state sanitizers and the offline progress-snapshot read-scope cap remain separate defense-in-depth/compatibility boundaries.
...
- Expired-transient maintenance must remain database-only and cron-only: when an external object cache is active, skip general cache-transient rows but retain the audited direct-`wp_options` atomic guard namespaces; otherwise use the full audited LL-owned cache/rate-limit allowlist. Keep the five-minute grace and hard 200-row/two-second caps, conditionally recheck the exact timeout during pair deletion, require an expired embedded timestamp for direct lease rows, and schedule one coalesced continuation when bounded work remains. Emit aggregate counts/namespaces/bytes without transient keys or values. Timeout-only rows are eligible; active pairs, value-only rows, non-LL transients, and persistent options/jobs are not.
...
- Search maintenance has an opt-in managed-worker mode: public searches never advance a build or enqueue maintenance when `ll_tools_wordset_category_search_background_only` is enabled. Install the independent bounded `wp ll-tools wordset-search-worker` schedule before enabling it. Ordinary word changes invalidate result caches and refresh only their exact word rows under the current generation and owner lease; cold, contended, failed or incomplete scopes retain bounded rebuilds. See [WORDSET_SEARCH_MAINTENANCE.md](docs/WORDSET_SEARCH_MAINTENANCE.md) for fencing, deployment and recovery.
- Wordset main-page category search reads from the durable `wp_ll_wordset_category_search` materialization and stays scoped to allowed categories. Schema v3 keys rows by a durable build generation; expired-lease takeover atomically rotates that generation, readers expose only the completed published generation, and old generations are removed in bounded chunks, so a stalled stale write can never clobber visible replacement rows. Builders advance with an ID keyset in bounded batches, cap source text before PHP hydration, chunk relationship reads and byte/row-limited inserts, renew an exact-owner lease before writes, back off transient failures, stop deterministic failure loops, and publish completion only for the same dependency signature and generation. Dedicated source epochs proactively queue rebuilds for published word title/status, current/legacy translation, and direct wordset/category membership changes: complete scopes schedule only known wordsets, while incomplete scope advances the global unknown source generation and a generation-fenced keyset scheduling sweep that never loads every wordset ID in one request. Category deletion resolves its bounded former published-word set to affected wordsets; an unused/non-word category is a no-op, while oversized or failed scope resolution uses the unknown-source sweep. Plugin updates, category label/parent edits, audio/transcription/review edits, generic quiz epochs, and broad category/wordset cache epochs do not invalidate this materialization. Queries chunk category/image/detail hydration, preserve locale-independent diacritic matching and flat direct-category assignment, and return a retryable preparation state until the generation is complete; the frontend must retain provisional matches plus loading/error state and an explicit Retry control rather than treating preparation failure as no matches. Each request has a configured 10–15 second timeout inside the overall 120-second foreground preparation window, so a stalled transport enters the bounded retry/error path instead of shimmering forever. Automatic foreground durable-preparation retries stop when a result navigation or page hide makes them irrelevant, and pause while a flashcard launch owns the global loader; an unresolved query may resume after that popup closes. Manual recovery reloads the bare pathname with the one-shot exact `ll_category_search_recovery=1` marker; public static cache must bypass reads and captures, emit private/no-store headers, and the browser must strip only that marker with `history.replaceState` after the fresh page loads. Deleting a wordset immediately removes its state, lease, source epoch, and rebuild event, then removes durable rows in bounded continuation batches. Staff-only pending-transcription search remains a separate lookup requiring at least three characters: it first selects capped published word IDs in the wordset, then capped published `word_audio` IDs, and only then performs the unindexed text comparison. It may identify matching word/category cards but must not expose transcription text or enter anonymous shared caches.
  - Local/category-shell matches are provisional while that durable query prepares. Keep those cards visible and usable throughout index warming and retry state; only the brief synchronous filter transition may fade them. A slow or retryable durable lookup must never blank already-matched cards.
- Anonymous flashcard and dictionary cache misses use the atomic counters and expiring client leases in `includes/lib/public-ajax-resource-guards.php`; do not replace them with transient get/increment/set admission. Flashcards preserve the 1,000-ID session compatibility ceiling but bound raw candidate parsing, charge candidate scopes in 100-ID units, admit at most two uncached builders per client, and serialize public multi-category frontend hydration while retaining the popup loading state across bounded warming retries. Dictionary live search takes its same-query build lock before charging the client budget, briefly waits for the winning cache builder, admits at most two distinct cold builders per client, and returns a retryable warming payload while retaining the loading state. Before any public dictionary query argument is unslashed, normalized, cached, admitted, recursively traversed, or expanded into SQL, `q`, scope, page, canonical/legacy letter, POS, source, dialect, and canonical/legacy entry ID input must pass hard-clamped raw-byte, value-byte, cardinality, and shape limits. Only scope/POS/source accept bounded multi-values; scalar arrays are rejected. AJAX rejects invalid arguments and ordinary GET/static-cache normalization drops them to safe defaults. Cache hits bypass the cold-build guards.
- Frontend login, learner registration, username suggestion, offline login, offline sync, and offline logout share fixed-window atomic admissions; do not restore read/increment/write transients. The first weighted reservation uses an atomic `INSERT IGNORE` rather than `add_option()`, so a losing creator can only conditionally increment the winner and can never overwrite it. If zero-counter cleanup deletes the row between observation and increment, admission retries that missing-row creation once; every SQL error fails closed. Successful authentication or logout refunds only its own reserved unit, never the whole shared-IP bucket, while failed and malformed attempts retain their charge. Offline sync and logout reserve the stable IP scope first; only a well-formed bearer envelope may then reserve the stable parsed user/session scope, so changing only the secret cannot evade admission and malformed envelopes cannot create token rows. A later-scope rejection refunds the exact earlier reservation and removes its zero-valued counter/timeout pair without deleting a concurrent increment. Logout performs this admission before session authentication and returns only generic rejection details plus `Retry-After`. Authentication identifiers and passwords are byte-bounded before hashing; registration bounds the raw username to 60 bytes and raw email to 100 bytes before sanitization, validation, or database lookup; and username suggestions check one bounded candidate set instead of issuing thousands of collision queries. Redirect feedback is normalized and byte-bounded. Generic rejected-request feedback reuses one stable token per client/form/locale/window and remains readable by every outstanding redirect until expiry, while ordinary per-request feedback remains one-shot. Lazy wordset-card and public vocab-grid cold misses likewise reserve both scopes atomically, reject oversized raw ID lists before `wp_parse_id_list`, and release build locks only with the exact lease token that acquired them.
- After nonce and broad endpoint-capability admission, admin selected-row inspection endpoints must reject oversized or malformed raw ID collections before unslashing, integer normalization, per-post capability checks, post hydration, or taxonomy reads. The common-category and Word Audio bulk recording-type state endpoints accept only positive integers or canonical digit strings and use hard-clamped value-count, per-value byte, and total raw-byte ceilings; do not rely on the browser list table to bound them. A taxonomy read failure for any eligible selected row invalidates the whole state response rather than making a partial intersection or false uniform set authoritative. Their browser loaders coalesce duplicate opens, abort a superseded request when possible, require both the request generation and current selected-row signature before applying common-state or replacement controls, and leave category/replacement controls neutral and unavailable after a failed read.
...
- Learner progress ingestion treats each event ledger row and all of its derived word, prompt-card, and category progress mutations as one transaction. Invalidate the verified schema marker before repair, republish it only after full column/index readback succeeds, and require the current installed plus verified markers before batch, offline-event, or server-event source resolution and mutation; this runtime gate uses cacheable option reads rather than repeating full schema `SHOW` queries. Verify the plugin progress tables and the core `users`/`usermeta` tables are InnoDB before any event or offline state write, serialize mutations with the learner's `users` row, use a unique savepoint when the caller owns a transaction, and clear user-meta caches after the lock and after commit/rollback. Public AJAX admission must keep the complete event collection within its byte, depth, node, and item ceilings before individual events are processed. Offline sync rejects more than 200 submitted events before authentication, rate-limit counter writes, or mutation; it never slices an oversized journal into an apparently successful prefix. Before canonicalization, each raw event payload must also independently fit the hard-clamped exact encoded byte budget (64 KiB by default), depth six, and the node ceiling derived from the supported session-category limit (`max(512, category_limit + 128)`, 1,128 by default and 2,000 hard maximum). Rebuild each payload from the strict first-party schema for `word_exposure`, `word_outcome`, `category_study`, `mode_session_complete`, and server-only `stt_api_call`; bound and canonicalize every retained scalar, list, Practice result, and gender-state field, and silently discard unknown or cross-event keys so stale offline journals are acknowledged instead of retrying forever. Client identity may enter the payload only from the sanitized top-level fields when the explicit identity-storage policy is enabled; a nested `payload.device_id` or `payload.profile_id` must never bypass that policy. Repeat the same exact encoded budget check after normalization, category remapping, and optional identity enrichment; reject the canonical payload if it no longer fits. Use `scripts/audit-progress-event-payloads.php` through WP-CLI for a value-free, identifier-free live schema inventory before changing this contract; every source read in that audit must fail closed rather than converting a database error into a complete empty report. State-only offline sync remains available while event ingestion is paused. The browser must keep every failed event UUID in its scoped journal, honor typed progress-sync `Retry-After` JSON or headers as one bounded cooldown, persist that scoped deadline across same-tab or offline reloads, journal new activity without issuing repeat POSTs or postponing the server-owned deadline, and automatically retry the exact ordered batch when the cooldown expires. A partial success must preserve any newer-activity flush timer; a full successful sync or a real user/wordset context reset clears the cooldown. Never publish a ledger row without its derived state, silently drop a failed UUID, auto-convert WordPress core tables, or turn a retryable engine/schema failure into partial success.
...
- Deferred vocab-lesson taxonomy preparation advances only from complete parent/child term reads; a taxonomy error preserves the current materializer generation and cursor. A normal lesson's targeted category count must also be complete: failure returns a typed retryable HTTP 503 before the legacy full-category fallback can run. Sparse wordset-page fallbacks must not recover deferred progress counts by hydrating every prompt card, and catalog/game/media requests must retain their abort-generation fence plus a bounded request deadline so page navigation or a stalled response cannot publish stale state or leave a permanent loader.
- User-facing asynchronous mutations and bounded loaders must settle visibly on timeout: abort when the transport supports it, ignore late responses with the same generation/session token used for navigation, restore controls and `aria-busy`, and expose translated retry/error state. Every shared vocab-lesson category-settings writer must use the same named target-category lock, require transactional InnoDB term metadata, perform its verified settings writes plus one revision advance in the same transaction, and evict any term-meta snapshot that may predate the lock. This includes lesson autosave, taxonomy add/edit forms, direct Line-Up/default/audio/image writers, duplicate/copy paths, and import; category deletion takes the same lock before settings can disappear. Imported values must pass the canonical per-key shape and bounds, the private revision key is never imported, omitted Line-Up fields preserve local state, and a failed newly-created category rollback is successful only after the term and metadata are verified absent. Durable jobs validate the complete category-meta payload before starting category batches; each category-detail chunk and multi-category Line-Up remap also validates every planned item before its first write, and every positive Line-Up source word ID must have a positive imported-word mapping. Assign-existing imports resolve categories by the exact wordset-owned slug and add only newly inserted term IDs to undo, while duplicate `new` mode never reuses a same-name owned category. An older client revision is rejected after a newer target edit has committed, and the browser retains only the latest queued form state. That target revision does not represent a source category's later Line-Up order/direction change when an isolated target still uses source fallback; dependency revisions, materialization, or ending inheritance is a deliberate structural follow-up tracked in `MAINTENANCE_BACKLOG.md`. Category-title saves restore the editor for explicit retry after timeout. Timeouts must not cause a second ambiguous write, silently advance a queue, or replace a complete last-known-good payload with an empty result.
- Category creation for import, duplicate, and editor split flows must use `ll_tools_create_new_wordset_category()`. Its WordPress 6.1+ scoped creation token is carried through the earliest pre-cleanup duplicate-confidence filter and `create_term`; the exact term/taxonomy mapping is captured before ordinary filters run, both capture paths must agree when both run, and a final-priority protector suppresses core's unchecked cleanup after any capture conflict. Only that proven raw mapping may receive owner/settings writes or be rolled back, and a concurrent duplicate-confidence winner is never adopted, mutated, counted as created, or added to Undo. Template imports force isolated ownership even when the global isolation option is disabled. A created object whose cleanup cannot be verified remains explicitly tracked for Undo with a critical failure marker but does not increment successful-create statistics; rollback removes and proves relationships absent before deleting their discoverable taxonomy mapping, and Undo must repair a proven half-mapping or zero-taxonomy orphan and verify all exact rows absent before counting deletion. Regular imported term-meta replacements must prove deletion, check every add, compare ordered raw serialized readback, and propagate typed failures through both template and ordinary wordset imports.
- Multi-category import writes acquire every target-category lock in sorted ID order and hold one outer transaction across the complete validated chunk; an inner category writer may reuse only the same live database connection through its reference-counted lock depth and savepoint, and reconnect invalidates that ownership. Category-generated page work remains deferred until the outer commit. A thrown exception or returned `WP_Error` preserves the affected queue entry, clears its stage-specific synced marker, and schedules both bounded reconciliation passes; rollback restores the exact preexisting deferred queue rather than publishing maintenance for reverted settings.
- Import Undo treats post deletion as a verified database mutation: it snapshots and locks the exact post, metadata, revisions, relationships, comments, and comment metadata; repairs only proven residual rows; verifies taxonomy counts; and counts success only after commit. Attachment Undo requires a helper-owned top-level transaction. Every candidate file path is durably journaled and its one-shot worker is scheduled in the same transaction as attachment-row deletion, while physical unlinking happens only after commit; failure retains the journal for the bounded worker, and rollback evicts staged journal/cron option caches. Newly imported attachments enter Undo immediately after creation and before any thumbnail hook can run.
- Content-lesson completion autosave keeps one active request generation. A transport deadline aborts when possible, restores an explicit Retry state without changing the last confirmed completion value, and fences a late response from the timed-out request; do not automatically duplicate an ambiguous completion mutation.
- Text-document review-note autosave is a one-in-flight/latest-queued protocol. The browser submits its last saved note as the same-key compare-and-set base, fences timed-out and late callbacks, and surfaces a conflict instead of automatically overwriting it. The server updates the shared note map with bounded exact-previous-value retries so simultaneous different-key edits are preserved, verifies persistence by readback, and returns HTTP 409 for a stale same-key base; never restore an unchecked whole-array read/modify/write or report success after a failed meta write.
- Audio Processor processing and deletion share one UI mutex so destructive actions cannot overlap processing or another delete flow. Bulk deletion keeps bounded concurrency, per-request deadlines, `aria-busy`, and retry selection only for failed recordings; per-recording processing deadlines likewise release the UI without accepting a late result. The delete endpoint is idempotent through the four-phase receipt contract (`pending`, `post_deleted`, `cleanup_failed`, `deleted`) and an exact-owner per-recording lease, captures safe file paths before mutation, requires `wp_delete_post()` to succeed before unlinking files, and completes authoritative parent cleanup before terminal publication. A veto or database failure must leave the live post and files usable.
...
- Learner Line-Up availability and launch requests inspect only a capped saved-order prefix and bounded title-ordered fallback, clamp every filterable minimum/launch/scan value to the public hard ceilings, and hydrate at most 60 selected published word IDs. Database failures in category discovery or either bounded candidate query return typed retryable state instead of becoming an empty/not-configured category. Complete sequence expansion remains an explicit admin editing flow; do not reuse it in the learner catalog or launch path. The authoring order is still stored in `ll_category_lineup_word_order` termmeta, so WordPress may deserialize a complete legacy value when any key primes that category's metadata cache; this means iteration and candidate hydration are bounded, but physical legacy metadata hydration is not yet. A future storage migration must move the complete authoring order outside the shared termmeta object cache, retain a bounded runtime projection plus exact count/version, update every writer and export/import/isolation fallback, and remove legacy rows in bounded maintenance batches rather than pretending a local prefix parser removes the peak.
...
- The offline export admin page must lazy-load category options for the selected wordset through its guarded AJAX endpoint; do not rebuild an inline all-wordset category map during initial render. Once a durable export token exists, each continuation step has a bounded abortable deadline, ignores late generations, and pauses with an explicit Resume action that continues from the last completed server batch. Job creation has no automatic timeout/retry until it owns an idempotency key because a lost start response is ambiguous.
...
- A non-empty Practice session records the exact result shown to the learner in its existing idempotent `mode_session_complete` event under `payload.result = {schema:1, kind:"practice_first_try", score_given, score_maximum, score_basis:"first_try_distinct_words"}`. The server must reconstruct the canonical shape, require integer `0 <= score_given <= score_maximum` within the configured bound, discard result data for other modes, and compute percentages for reports instead of accepting a submitted percentage. Teacher-class reports may read only the already-paged current learners in the class's fixed wordset. Practice-result reads use a configured two-learner batch (five hard maximum) that automatically shrinks when the scan depth would exceed the approximately 24 MiB aggregate-query budget; the default fetch is 500 retained rows plus one truncation sentinel per learner, and SQL excludes payloads above 16 KiB before PHP hydration. A query failure must propagate as `query_failed` and render translated `Unavailable` cells with empty sort values rather than false zero/no-data results. Do not restore one ledger query per learner or increase the batch/scan budget without a peak-memory measurement. These are client-reported formative Practice results governed by detailed-event retention, not tamper-resistant grades or permanent class attribution. LMS passback must add server-verifiable assignments/attempts plus a durable after-commit delivery outbox; never perform outbound delivery inside the learner-progress transaction.
...
- Plugin activation must reschedule the earliest durable pending/retry delivery because deactivation clears only the worker event, not its rows. WordPress account deletion removes local delivery mappings before assignment grades/attempts and Google credentials without calling a provider. One atomic per-user/per-site option acts as the deletion tombstone and is installed at priority 1, before ordinary `delete_user`, `wpmu_delete_user`, or `remove_user_from_blog` cleanup. The bounded worker then drains the shared per-user advisory lock, transactionally erases and verifies local sessions/progress/study meta/class membership, and repeats that verification after core's post-delete boundary; a writer admitted before the tombstone is therefore removed even if it commits after the first cleanup attempt. Lock/schema/verification failure keeps the tombstone and schedules another pass, and post-delete cleanup may omit only the already-vanished user-row lock after the advisory barrier succeeds. Network deletion prepares every affected site before WordPress removes membership; ordinary multisite removal completes against durable absent-membership proof. Assignment, mapping, and credential writes recheck that tombstone under the user-row transaction fence, and a stale cron consumer must never recreate a completed tombstone. Recovery keyset-pages at most fifty tombstones per pass and a throttled init resumer covers failed cron writes. Manual privacy erasure holds an exact-owner per-call lease inside one stable bounded job fence, transfers ownership only by compare-and-swap between successful continuation pages, releases only its own lease on an explicit error, and lets an abandoned job expire. Privacy exporter/eraser storage failures return `WP_Error`; `done=false` is reserved for successful bounded continuation so WordPress does not silently omit data or immediately loop forever.
- Local study-data erasure must fail before mutation unless `users`, `usermeta`, `posts`, `postmeta`, offline-session, progress-word, and progress-event tables are all verified InnoDB. It establishes the per-user offline-session advisory lock before the user-row transaction, revokes current and legacy bearer sessions, removes both sides of teacher-class membership, deletes progress rows and erased user meta, verifies every surface, and rolls the complete local change back on any read, write, verification, or commit failure. Usermeta existence, deletion, and readback decisions must use error-detecting current reads; a failed read, delete, or absence verification rolls back erasure instead of treating uncertainty as absence. Serialized class-roster SQL is only a bounded candidate filter; exact deserialized values certify reverse membership so numeric array indexes cannot block erasure. Offline session creation/import and full sync, browser/direct progress batches, server-recorded STT events, ordinary class-membership mutations, browser study-state/goals/recommendation/queue writes, content completion, inactive-category hide, and progress reset use the same advisory lock and recheck the privacy fence after acquisition, so a writer either finishes before erasure and is removed or fails closed until the fence is released. Keep every read that feeds a grouped write inside that outer boundary. Automatic getter/migration repairs use exact non-inserting SQL compare-and-swap with cache eviction, and applied legacy completion replay takes a fresh per-user source snapshot under the lock; do not nest the non-reentrant advisory lock inside their low-level helpers.
...
- Client-originated progress events must supply their stable UUID; do not invent one during sanitization or retry-failure accounting because a retry would bypass ledger deduplication. Server-originated events may still generate a UUID when the unique server action is created.
- Audio Processor deletion receipts have `pending`, `post_deleted`, `cleanup_failed`, and `deleted` phases. They retain only uploads-root-validated cleanup paths, parent-cleanup state, and an exact receipt token. Prearm the cleanup retry before deleting the post, then authoritatively verify post absence and parent cleanup before terminal publication. Retry tokens and attempt generations are monotonic: a stale attempt cannot rearm or overwrite newer state, while lock contention reschedules without consuming an attempt. Bounded exponential single-event retries verify file and parent cleanup; exhausted automatic retries enter an explicit non-autoloaded retained-failure state so an authorized manual retry or maintenance review can finish cleanup without losing the orphan path. Only a stale pre-delete journal for a still-live post expires automatically.
- IPA Keyboard lesson links are a requested-category lookup, separate from the complete public lesson-category map. Pass only the current bounded category IDs and retain the 100-default/250-hard ceiling rather than scanning every published vocabulary lesson.
- Every teacher-class membership add/remove takes the user lock before the class-row lock and rechecks the exact-owner class-deletion lease after both. Deletion publishes that lease before its roster barrier, drains reverse membership only as the exact owner, and re-reads the roster under the class row lock before the transactional delete.
- Progress-report word aggregates use direct SQL in 100-learner chunks and may fall back to bounded raw-row evaluation only when the streak-relief filter changes the canonical calculation. The fallback is keyset-paged independently per learner (1,000 rows maximum per page), uses a 10,000-row default and 20,000-row hard scan cap, resolves current audio requirements, and returns no partial summary on a cap, query, or lookup failure. User, word, event, ID, and count query failures remain typed; teacher summaries exclude failed rows and display `Unavailable` rather than turning incomplete reads into zero.
- Direct grade-delivery identity and recipient constructors require the verified assignment-schema marker, open or join the assignment transaction, take the learner row lock, perform a current locking read of both privacy fences, and re-read authoritative mappings before writing. Nested failure rolls back only its savepoint.
- The external `simplefavorites` key remains owned by its source plugin. LL Tools erasure preserves it but uses error-detecting reads to write and verify an LL-owned legacy-completion-erased marker; any read/write/readback uncertainty fails and rolls back erasure. Later legacy completion snapshots must ignore Favorites-derived completion while retaining canonical LL Tools activity and separately owned sources.
