...
- Standard learner-facing vocab lessons above the generic paging threshold (48 visible words by default) use `ll_tools_get_vocab_lesson_grid` as a serial bounded loader. It keyset-scans at most 128 candidate IDs per preparation request, materializes the exact filtered/manual/grouped/title order behind an exact-owner lock, then renders at most 24 word cards per signed cursor page. The browser replaces the shimmer shell with the first real page, appends later pages one request at a time, retains already-rendered cards on failure, and enforces a request timeout with Retry. Keep this category-agnostic; prompt-card lessons and staff editing views deliberately retain their specialized/full-grid paths.
