...
- Frontend login, learner registration, username suggestion, offline login, and offline sync share those fixed-window atomic admissions; do not restore read/increment/write transients. Successful authentication refunds only its own reserved unit, never the whole shared-IP bucket, while failed and malformed attempts retain their charge. Authentication identifiers and passwords are byte-bounded before hashing; registration bounds the raw username to 60 bytes and raw email to 100 bytes before sanitization, validation, or database lookup; username suggestions check one bounded candidate set instead of issuing thousands of collision queries; and offline token admission is refunded if the paired IP admission loses its race. Redirect feedback is normalized and byte-bounded. Generic rejected-request feedback reuses one stable token per client/form/locale/window and remains readable by every outstanding redirect until expiry, while ordinary per-request feedback remains one-shot. Lazy wordset-card and public vocab-grid cold misses likewise reserve both scopes atomically, reject oversized raw ID lists before `wp_parse_id_list`, and release build locks only with the exact lease token that acquired them.
