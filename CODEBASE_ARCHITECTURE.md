...
- Logged-in broad or filtered multi-category wordset launches use `ll_user_study_selection_launch_plan`: resolve exact progress criteria against the cached ID-only category aggregate, return at most 30 word IDs from at most eight categories (15 and three preferred by default), and hydrate media only for that returned candidate pool. A 429 or other launch-plan/candidate failure must close the loader and expose a retryable error; never resume the former full-category payload drain or a long foreground warming loop.
...
- The durable server protocol is paged. Generic flashcard-widget launches and
  ...
  `ll_user_study_selection_launch_plan` request returns a capped ID-only session,
  then candidate-specific category requests hydrate only those words. Extend
  that contract deliberately if learning or another specialized flow moves to
  bounded startup; do not restore full selected-category drains there.
...
- Broad or progress-filtered logged-in wordset selections must launch through the bounded server plan and candidate-specific hydration. Keep plan results capped, nonce- and wordset-access protected, exact for `new`/`studied`/`learned`/`starred`/`hard`, and fail closed without leaving the popup loader busy when planning or hydration is rate-limited.
