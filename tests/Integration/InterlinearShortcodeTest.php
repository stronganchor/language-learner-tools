...
final class InterlinearShortcodeTest extends LL_Tools_TestCase
{
    public function test_shortcode_renders_simple_rows_with_shared_interlinear_markup(): void
    {
        ...
        $this->assertStringContainsString('<th scope="row">TEXT</th>', $output);
    ...
    public function test_shortcode_renders_sentence_and_free_translation_rows(): void
    {
        ...
        $this->assertStringContainsString('<th scope="row">IPA</th>', $output);
        $this->assertStringContainsString('ll-interlinear-shortcode__ipa ll-ipa', $output);
        $this->assertStringContainsString('lad̪͡ʒ qʰɨd̪͡ʒikʰ', $output);
        $this->assertStringContainsString('<th scope="row">MORPH</th>', $output);
        $this->assertStringContainsString('∅', $output);
        $this->assertStringContainsString('<th scope="row">FREE</th>', $output);
    ...
    {
        $output = do_shortcode(
            "[ll_interlinear show_text=\"0\" text=\"Hidden line\" ipa=\"hidden ipa\" free_translation=\"little boy\"]\n"
            . "MORPH | lac | ∅ | qicik\n"
            . "GLOSS | son | M.EZ | small\n"
            . '[/ll_interlinear]'
        );

        $this->assertStringNotContainsString('<th scope="row">TEXT</th>', $output);
        $this->assertStringNotContainsString('<th scope="row">IPA</th>', $output);
        $this->assertStringContainsString('<th scope="row">FREE</th>', $output);
    }
