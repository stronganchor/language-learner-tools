...
if [[ -n "${LL_TOOLS_LOCAL_SERVICES_ROOT:-}" ]]; then
    resolver_args+=("--service-root=$LL_TOOLS_LOCAL_SERVICES_ROOT")
fi
if [[ -n "${LL_TOOLS_LOCAL_TEMP_DIR:-}" ]]; then
    resolver_args+=("--temp-dir=$LL_TOOLS_LOCAL_TEMP_DIR")
fi
