...
- Its anonymous `/wp-admin/` warmup uses `bin/wordpress-readiness.sh`: at most three attempts, one-second retry delays, and one shared 180-second deadline (`LL_TOOLS_E2E_READINESS_TIMEOUT_SECONDS`, 1–600). Retry applies only to 502/503/504 and connection refused/reset during this idempotent GET, which can occur as Local recycles PHP-CGI; it never retries authentication/configuration failures, increases measured test budgets, or restarts Local. Mocked readiness contracts live in `maintenance-doc-contracts.spec.js` and need no Local site.
