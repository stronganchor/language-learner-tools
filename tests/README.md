...
- `tests/e2e/specs/wordset-pages-listening-launch.spec.js`
  - Verifies wordset page launch actions can open Listening mode with the
    ...
    kept contiguous for low request fan-out; category-diverse lighter batches
    precede isolated heavy batches and owned queues are ordered small-first so a
    large catch-all lesson cannot monopolize the opening chunk,
