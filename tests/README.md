...
- `tests/e2e/specs/wordset-page-category-search.spec.js`
  - Verifies main wordset category search uses the durable tokenized async word/translation lookup while keeping provisional local/category-shell matches visible and usable throughout bounded preparation retries, exposing an explicit error/Retry state instead of a false empty result, stopping irrelevant warming when a visible result navigates, pausing it while a result quiz owns the popup loader, and retaining hidden-selection cleanup, add-category hiding, clear-button behavior, and diacritic-insensitive matching. Staff pending-transcription visibility remains covered at the PHP privacy/query layer.
