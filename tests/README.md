...
- `UserStudyAnalyticsTest` verifies logged-in multi-category launch plans apply exact `new`/`studied`/`learned`/`starred`/`hard` progress criteria over the ID-only analytics scope, preserve every matching word exactly once across successful balanced category-aware chunks, widen sparse tails only to the hard category cap, return a typed fail-closed error when an even sparser layout cannot form valid rounds, enforce per-chunk word/category caps plus nonce and wordset access through the AJAX route, and leave media hydration to the current candidate-specific chunk launcher.
...
- `tests/e2e/specs/wordset-pages-listening-launch.spec.js`
  - Verifies wordset page launch actions can open Listening mode with the
    ...
    that either preserves every match across server-planned chunks or fails
