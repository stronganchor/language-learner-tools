...
function ll_tools_prompt_audio_import_max_rows(): int {
    return max(1, min(1000, (int) apply_filters('ll_tools_prompt_audio_import_max_rows', 200)));
}

function ll_tools_prompt_audio_import_max_bytes(): int {
    return max(1024, min(2 * 1024 * 1024, (int) apply_filters('ll_tools_prompt_audio_import_max_bytes', 256 * 1024)));
}
...
function ll_tools_render_prompt_audio_import_page(): void {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['ll_prompt_audio_import_nonce'])) {
        ...
        $raw_list = isset($_POST['ll_prompt_audio_list']) ? (string) wp_unslash($_POST['ll_prompt_audio_list']) : '';
        $parsed_rows = [];
        if (strlen($raw_list) > ll_tools_prompt_audio_import_max_bytes()) {
            $errors[] = sprintf(
                __('Prompt audio import is limited to %s of pasted text per request.', 'll-tools-text-domain'),
                size_format(ll_tools_prompt_audio_import_max_bytes())
            );
        } else {
            ...
            if (count($actionable_lines) > ll_tools_prompt_audio_import_max_rows()) {
                $max_rows = ll_tools_prompt_audio_import_max_rows();
                $errors[] = sprintf(
                    _n(
                        'Prompt audio import is limited to %d row per request. Split the list and submit the remaining rows separately.',
                        'Prompt audio import is limited to %d rows per request. Split the list and submit the remaining rows separately.',
                        $max_rows,
                ...
                $parsed_rows = array_map('ll_tools_prompt_audio_import_parse_line', $raw_lines);
            }
        }
