...
function ll_register_recording_types_admin_page() {
    add_submenu_page(
        'tools.php',
        __('Language Learner Tools - Recording Types', 'll-tools-text-domain'),
        __('LL Recording Types', 'll-tools-text-domain'),
        'manage_options',
        'll-recording-types',
...
function ll_render_recording_types_admin_page() {
    ...
    if (isset($_POST['delete_term']) && isset($_POST['term_id'])) {
        ...
        if (!is_wp_error($result)) {
            echo '<div class="notice notice-success"><p>' . esc_html__('Recording type deleted successfully.', 'll-tools-text-domain') . '</p></div>';
        } else {
            echo '<div class="notice notice-error"><p>'
                . sprintf(
                    /* translators: %s: error message */
                    esc_html__('Error deleting recording type: %s', 'll-tools-text-domain'),
    ...
    if (isset($_POST['add_term']) && !empty($_POST['term_name'])) {
        ...
        if (!is_wp_error($result)) {
            echo '<div class="notice notice-success"><p>' . esc_html__('Recording type added successfully.', 'll-tools-text-domain') . '</p></div>';
        } else {
            echo '<div class="notice notice-error"><p>'
                . sprintf(
                    /* translators: %s: error message */
                    esc_html__('Error adding recording type: %s', 'll-tools-text-domain'),
                    esc_html($result->get_error_message())
                )
                . '</p></div>';
        }
    ...
    ?>
    <div class="wrap">
        <h1><?php esc_html_e('Recording Types Management', 'll-tools-text-domain'); ?></h1>
        <p><?php esc_html_e('Manage the recording type taxonomy used for audio recordings. Each type represents a different recording style (e.g., isolation, sentence, question).', 'll-tools-text-domain'); ?></p>

        <div style="background: #fff; padding: 20px; margin: 20px 0; border: 1px solid #ccd0d4; border-radius: 4px;">
            <h2><?php esc_html_e('Add New Recording Type', 'll-tools-text-domain'); ?></h2>
            <form method="post" action="">
                <?php wp_nonce_field('ll_add_recording_type'); ?>
                <table class="form-table">
                    <tr>
                        <th scope="row"><label for="term_name"><?php esc_html_e('Name', 'll-tools-text-domain'); ?></label></th>
                        <td>
                            <input type="text" name="term_name" id="term_name" class="regular-text" required>
                            <p class="description"><?php esc_html_e('The display name for this recording type.', 'll-tools-text-domain'); ?></p>
                        </td>
                    </tr>
                    <tr>
                        <th scope="row"><label for="term_slug"><?php esc_html_e('Slug', 'll-tools-text-domain'); ?></label></th>
                        <td>
                            <input type="text" name="term_slug" id="term_slug" class="regular-text">
                            <p class="description"><?php esc_html_e('Optional. Will be auto-generated from name if left blank.', 'll-tools-text-domain'); ?></p>
                        </td>
                    </tr>
                </table>
                <p class="submit">
                    <input type="submit" name="add_term" class="button button-primary" value="<?php echo esc_attr__('Add Recording Type', 'll-tools-text-domain'); ?>">
                </p>
            </form>
        </div>

        <h2><?php esc_html_e('Existing Recording Types', 'll-tools-text-domain'); ?></h2>
        <?php if (!empty($terms) && !is_wp_error($terms)): ?>
            <table class="wp-list-table widefat fixed striped">
                <thead>
                    <tr>
                        <th style="width: 30%;"><?php esc_html_e('Name', 'll-tools-text-domain'); ?></th>
                        <th style="width: 30%;"><?php esc_html_e('Slug', 'll-tools-text-domain'); ?></th>
                        <th style="width: 15%;"><?php esc_html_e('Count', 'll-tools-text-domain'); ?></th>
                        <th style="width: 25%;"><?php esc_html_e('Actions', 'll-tools-text-domain'); ?></th>
                    </tr>
                </thead>
                <tbody>
                        ...
                        <tr>
                            ...
                            <td>
                                ...
                                printf(
                                    ...
                                    esc_html(_n('%d recording', '%d recordings', (int) $term->count, 'll-tools-text-domain')),
                                    (int) $term->count
                                );
                                ?>
                            </td>
                            <td>
                                <a href="<?php echo esc_url(admin_url('edit-tags.php?action=edit&taxonomy=recording_type&tag_ID=' . $term->term_id . '&post_type=word_audio')); ?>" class="button button-small"><?php esc_html_e('Edit', 'll-tools-text-domain'); ?></a>

                                <form method="post" style="display: inline-block;" onsubmit="return confirm('<?php echo esc_js(__('Are you sure you want to delete this recording type? This cannot be undone.', 'll-tools-text-domain')); ?>');">
                                    <?php wp_nonce_field('ll_delete_recording_type'); ?>
                                    <input type="hidden" name="term_id" value="<?php echo esc_attr($term->term_id); ?>">
                                    <input type="submit" name="delete_term" class="button button-small" value="<?php echo esc_attr__('Delete', 'll-tools-text-domain'); ?>">
                                </form>
                            </td>
                        </tr>
                    <?php endforeach; ?>
        ...
        <?php else: ?>
            <p><?php esc_html_e('No recording types found.', 'll-tools-text-domain'); ?></p>
        <?php endif; ?>
        ...
        <div style="margin-top: 20px; padding: 15px; background: #f0f0f1; border-left: 4px solid #72aee6;">
            <h3><?php esc_html_e('Tips', 'll-tools-text-domain'); ?></h3>
            <ul>
                <li><?php esc_html_e('Common recording types: Isolation (word alone), Question (asking about the word), Introduction (using in context), Sentence (word in a sentence)', 'll-tools-text-domain'); ?></li>
                <li><?php esc_html_e('Slugs should be lowercase and use hyphens instead of spaces', 'll-tools-text-domain'); ?></li>
                <li><?php esc_html_e('Deleting a recording type will not delete associated recordings, but they will lose their type classification', 'll-tools-text-domain'); ?></li>
            </ul>
        </div>
    </div>
    <?php
