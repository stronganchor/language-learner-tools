...
if (!defined('LL_TOOLS_VOCAB_LESSON_PREVIEW_ONLY_META')) {
    define('LL_TOOLS_VOCAB_LESSON_PREVIEW_ONLY_META', '_ll_tools_vocab_preview_only');
}
if (!defined('LL_TOOLS_VOCAB_LESSON_SEPARATOR')) {
    define('LL_TOOLS_VOCAB_LESSON_SEPARATOR', '-');
}
if (!defined('LL_TOOLS_VOCAB_LESSON_RECENT_UPDATE_TRANSIENT')) {
    define('LL_TOOLS_VOCAB_LESSON_RECENT_UPDATE_TRANSIENT', 'll_tools_vocab_lesson_recent_update');
}
...
if (!defined('LL_TOOLS_VOCAB_LESSON_FULL_SYNC_EVENT')) {
    define('LL_TOOLS_VOCAB_LESSON_FULL_SYNC_EVENT', 'll_tools_vocab_lesson_full_sync_event');
}
if (!defined('LL_TOOLS_VOCAB_LESSON_FULL_SYNC_REQUEST_OPTION')) {
    define('LL_TOOLS_VOCAB_LESSON_FULL_SYNC_REQUEST_OPTION', 'll_tools_vocab_lesson_full_sync_request');
}

function ll_tools_finalize_wordset_page_lesson_cache_invalidation(): void {
    $state = isset($GLOBALS['ll_tools_wordset_page_lesson_cache_invalidation_state'])
        && is_array($GLOBALS['ll_tools_wordset_page_lesson_cache_invalidation_state'])
        ? $GLOBALS['ll_tools_wordset_page_lesson_cache_invalidation_state']
        : [];
    ...
    // An early mutation bump can purge while a bounded lesson sync is still
    // running. Permit the final generation to purge both local static files
    // and the edge again after the last write closes that race window.
...
function ll_tools_invalidate_wordset_page_lesson_cache(): void {
    ...
    $state['dirty'] = true;
    if (empty($state['epoch_bumped']) && function_exists('ll_tools_bump_wordset_cache_epoch')) {
        ll_tools_bump_wordset_cache_epoch();
        $state['epoch_bumped'] = true;
    }
    if (empty($state['finalizer_registered'])) {
        add_action('shutdown', 'll_tools_finalize_wordset_page_lesson_cache_invalidation', PHP_INT_MAX, 0);
        $state['finalizer_registered'] = true;
    }

    $GLOBALS['ll_tools_wordset_page_lesson_cache_invalidation_state'] = $state;
}

function ll_tools_maybe_invalidate_wordset_page_lesson_cache_for_post(int $post_id, $post = null): void {
    $post = $post instanceof WP_Post ? $post : get_post($post_id);
    if ($post instanceof WP_Post && $post->post_type === 'll_vocab_lesson') {
        ll_tools_invalidate_wordset_page_lesson_cache();
    }
}
add_action('save_post_ll_vocab_lesson', 'll_tools_maybe_invalidate_wordset_page_lesson_cache_for_post', 30, 3);
add_action('untrashed_post', 'll_tools_maybe_invalidate_wordset_page_lesson_cache_for_post', 30, 1);
add_action('before_delete_post', 'll_tools_maybe_invalidate_wordset_page_lesson_cache_for_post', 30, 2);

function ll_tools_maybe_invalidate_wordset_page_lesson_cache_for_meta(
    $meta_id,
    int $post_id,
    string $meta_key
): void {
    ...
    if (!in_array($meta_key, [
        ...
        LL_TOOLS_VOCAB_LESSON_PREVIEW_ONLY_META,
    ], true)) {
        return;
    }
    if ((string) get_post_type($post_id) === 'll_vocab_lesson') {
        ll_tools_invalidate_wordset_page_lesson_cache();
    }
}
add_action('added_post_meta', 'll_tools_maybe_invalidate_wordset_page_lesson_cache_for_meta', 30, 3);
add_action('updated_post_meta', 'll_tools_maybe_invalidate_wordset_page_lesson_cache_for_meta', 30, 3);
add_action('deleted_post_meta', 'll_tools_maybe_invalidate_wordset_page_lesson_cache_for_meta', 30, 3);
...
function ll_tools_mark_vocab_lesson_recent_update(): void {
    ...
    set_transient(LL_TOOLS_VOCAB_LESSON_RECENT_UPDATE_TRANSIENT, time(), $ttl);
}
...
function ll_tools_vocab_lesson_cleanup_invalid_pages_allowed(array $args): bool {
    if (!array_key_exists('cleanup_invalid', $args)) {
        return false;
    }

    return (bool) $args['cleanup_invalid'];
}

function ll_tools_vocab_lesson_cleanup_unavailable_categories_allowed(array $args): bool {
    if (!array_key_exists('cleanup_unavailable_categories', $args)) {
        return false;
    }

    return (bool) $args['cleanup_unavailable_categories'];
}
...
function ll_tools_vocab_lesson_category_supports_gender_mode(int $wordset_id, $category): bool {
    ...
    $category_lookup_complete = true;
    if (!($category instanceof WP_Term)) {
        $wpdb->last_error = '';
        $category = get_term($category, 'word-category');
        $category_lookup_complete = $wpdb->last_error === '' && !is_wp_error($category);
    }
    if (!$category_lookup_complete || !($category instanceof WP_Term) || is_wp_error($category)) {
        return false;
    }
    ...
    $base_config_complete = true;
    if (function_exists('ll_tools_get_category_quiz_config')) {
        ll_tools_get_category_quiz_config($category, $base_config_complete);
    } else {
        $base_config_complete = false;
    }
    $effective_config_complete = true;
    $quiz_config = function_exists('ll_tools_resolve_effective_category_quiz_config')
        ? ll_tools_resolve_effective_category_quiz_config(
            $category,
            $min_word_count,
            [$wordset_id],
        ...
        : (
            function_exists('ll_tools_get_category_quiz_config')
                ? ll_tools_get_category_quiz_config($category, $effective_config_complete)
                : ['prompt_type' => 'audio', 'option_type' => 'image']
        );
    if (!$base_config_complete || !$effective_config_complete) {
        return false;
    }
...
function ll_tools_get_vocab_lesson_trash_notice_state(): array {
    ...
    $post_ids = isset($raw['post_ids']) && is_array($raw['post_ids']) ? $raw['post_ids'] : [];
...
function ll_tools_save_vocab_lesson_trash_notice_state(array $state): void {
    $post_ids = isset($state['post_ids']) && is_array($state['post_ids']) ? $state['post_ids'] : [];
    $post_ids = array_values(array_unique(array_filter(array_map('intval', $post_ids), static function (int $post_id): bool {
        return $post_id > 0;
    })));
    ...
    update_option(LL_TOOLS_VOCAB_LESSON_TRASH_NOTICE_OPTION, [
        'post_ids' => $post_ids,
        'updated_at' => isset($state['updated_at']) ? (int) $state['updated_at'] : time(),
    ], false);
}

function ll_tools_record_vocab_lesson_trash_notice(array $post_ids): void {
    $post_ids = array_values(array_unique(array_filter(array_map('intval', $post_ids), static function (int $post_id): bool {
        return $post_id > 0;
    })));
    if (empty($post_ids)) {
    ...
    $state = ll_tools_get_vocab_lesson_trash_notice_state();
    $state['post_ids'] = array_values(array_unique(array_merge($state['post_ids'], $post_ids)));
    $state['updated_at'] = time();

    ll_tools_save_vocab_lesson_trash_notice_state($state);
}

function ll_tools_get_trashed_vocab_lesson_notice_post_ids(): array {
    $state = ll_tools_get_vocab_lesson_trash_notice_state();
    ...
    $post_ids = get_posts([
        ...
        'post_status' => 'trash',
        'posts_per_page' => -1,
        'post__in' => $state['post_ids'],
        'orderby' => 'post__in',
        'fields' => 'ids',
        'no_found_rows' => true,
    ]);

    $post_ids = array_values(array_unique(array_filter(array_map('intval', (array) $post_ids), static function (int $post_id): bool {
        return $post_id > 0;
    })));

    if ($post_ids !== $state['post_ids']) {
...
function ll_tools_trash_vocab_lesson_post(int $post_id, array $context = []): bool {
    ...
    if (!isset($GLOBALS['ll_tools_vocab_lesson_trash_context']) || !is_array($GLOBALS['ll_tools_vocab_lesson_trash_context'])) {
        $GLOBALS['ll_tools_vocab_lesson_trash_context'] = [];
    }

    $GLOBALS['ll_tools_vocab_lesson_trash_context'][$post_id] = [
        'notify' => array_key_exists('notify', $context) ? (bool) $context['notify'] : true,
    ];

    $trashed = wp_trash_post($post_id);
    if (!($trashed instanceof WP_Post)) {
        unset($GLOBALS['ll_tools_vocab_lesson_trash_context'][$post_id]);
        return false;
    }
...
function ll_tools_handle_trashed_vocab_lesson_post($post_id): void {
    $post_id = (int) $post_id;
    if ($post_id <= 0) {
        return;
    }
    ...
    if (array_key_exists('notify', $context) && !$context['notify']) {
        return;
    }

    ll_tools_record_vocab_lesson_trash_notice([$post_id]);
}
add_action('trashed_post', 'll_tools_handle_trashed_vocab_lesson_post');

function ll_tools_get_active_vocab_lesson_page_count(array $wordset_ids = []): int {
    $query_args = [
        'post_type' => 'll_vocab_lesson',
    ...
    if (!empty($wordset_ids)) {
        $query_args['meta_query'] = [
            [
                ...
                'value' => array_map('strval', $wordset_ids),
                'compare' => 'IN',
            ],
        ];
    }
    ...
    return count((array) $post_ids);
}

function ll_tools_get_active_vocab_lesson_counts_by_wordset(array $wordset_ids): array {
    $wordset_ids = array_values(array_unique(array_filter(array_map('intval', $wordset_ids), static function (int $wordset_id): bool {
        return $wordset_id > 0;
    })));
    if (empty($wordset_ids)) {
...
function ll_tools_normalize_vocab_lesson_wordset_ids($raw): array {
    if (is_string($raw)) {
...
function ll_tools_get_vocab_lesson_wordset_ids(): array {
    ...
    $ids = ll_tools_normalize_vocab_lesson_wordset_ids(get_option('ll_vocab_lesson_wordsets', []));
    if ($ids === []) {
        return [];
    }
    ...
    if (is_wp_error($existing_ids) || !is_array($existing_ids) || $wpdb->last_error !== '') {
        return $ids;
    }

    $existing_lookup = array_fill_keys(array_map('intval', $existing_ids), true);
    return array_values(array_filter($ids, static function (int $wordset_id) use ($existing_lookup): bool {
        return isset($existing_lookup[$wordset_id]);
    }));
}
...
function ll_tools_resolve_vocab_lesson_category_for_wordset(
    $category,
    int $wordset_id,
    ...
    if (!($category instanceof WP_Term)) {
        ...
        $wpdb->last_error = '';
        $category = get_term($category, 'word-category');
        $complete = $wpdb->last_error === '' && !is_wp_error($category);
    }
    ...
    if ($wordset_id > 0 && function_exists('ll_tools_get_effective_category_id_for_wordset')) {
        ...
        if ($effective_category_id > 0 && $effective_category_id !== (int) $category->term_id) {
            ...
            $complete = $complete && $wpdb->last_error === '' && !is_wp_error($effective_category);
...
function ll_tools_get_vocab_lesson_category_meta_candidates(
    int $category_id,
    int $wordset_id,
    ?bool &$complete = null
): array {
    $complete = true;
    $require_complete = func_num_args() >= 3;
    ...
    if ($require_complete && function_exists('ll_tools_wordset_isolation_remap_category_id_list_for_wordset_complete')) {
        $effective_category_ids = ll_tools_wordset_isolation_remap_category_id_list_for_wordset_complete(
            [$category_id],
            $wordset_id,
            false
        );
        if ($effective_category_ids === null) {
            $complete = false;
        }
        $effective_category_id = (int) (($effective_category_ids ?? [])[0] ?? 0);
    ...
    if (function_exists('ll_tools_get_category_isolation_source_id')) {
        if ($require_complete) {
            ...
            $source_category_id = (int) ll_tools_get_category_isolation_source_id($category_id, $source_complete);
        ...
        if ($source_category_id > 0) {
            $category_ids[] = $source_category_id;
        }
    }

    return array_values(array_unique(array_filter(array_map('intval', $category_ids), static function (int $id): bool {
        return $id > 0;
    })));
}

function ll_tools_vocab_lesson_category_requires_images(
    $category,
    int $wordset_id = 0,
    ?bool &$complete = null
): bool {
    ...
    $config = ll_tools_get_category_quiz_config($category, $config_complete);
    ...
    if ($wordset_id > 0 && function_exists('ll_tools_apply_wordset_quiz_presentation_overrides')) {
        ...
        $complete = $complete && $presentation_complete;
    }
 ...
 * @return int[]
 */
function ll_tools_get_vocab_lesson_prompt_card_preview_ids(int $wordset_id, $category, int $limit = 2, int $offset = 0, ?bool &$complete = null): array {
    $complete = true;
    $wordset_id = (int) $wordset_id;
    ...
    $resolved_category = ll_tools_resolve_vocab_lesson_category_for_wordset(
        $category,
        $wordset_id,
        false,
        $category_resolution_complete
    );
    $complete = $category_resolution_complete;
    if (!$category_resolution_complete || !($resolved_category instanceof WP_Term) || is_wp_error($resolved_category)) {
        return [];
    }
    ...
    $wordset_tt_complete = true;
    $wordset_tt_id = function_exists('ll_tools_vocab_lesson_get_wordset_term_taxonomy_id')
        ? ll_tools_vocab_lesson_get_wordset_term_taxonomy_id($wordset_id, $wordset_tt_complete)
        : 0;
    if (!function_exists('ll_tools_vocab_lesson_get_wordset_term_taxonomy_id')) {
        $wordset_tt_complete = false;
    }
    $depth_rows_complete = true;
    $depth_rows = function_exists('ll_tools_vocab_lesson_get_word_category_depth_rows')
        ? ll_tools_vocab_lesson_get_word_category_depth_rows($depth_rows_complete)
        : [];
    if (!function_exists('ll_tools_vocab_lesson_get_word_category_depth_rows')) {
        $depth_rows_complete = false;
    }
    $depth_table_sql = function_exists('ll_tools_vocab_lesson_build_depth_table_sql')
        ? ll_tools_vocab_lesson_build_depth_table_sql($depth_rows)
        : '';
    $complete = $complete && $wordset_tt_complete && $depth_rows_complete;
    ...
    $complete = $complete && $wpdb->last_error === '';
    return array_values(array_filter(array_map('intval', (array) $rows), static function (int $post_id): bool {
        return $post_id > 0;
    }));
}
 ...
 */
function ll_tools_get_vocab_lesson_prompt_card_preview_data(
    int $wordset_id,
    $category,
    int $limit = 2,
    int $offset = 0,
    ?bool &$complete = null,
    ?int &$fetched_source_count = null
): array {
    $complete = true;
    $fetched_source_count = 0;
    $query_limit = min(100, max(1, (int) $limit));
    $ids_complete = true;
    $prompt_card_ids = ll_tools_get_vocab_lesson_prompt_card_preview_ids($wordset_id, $category, $query_limit, $offset, $ids_complete);
    $fetched_source_count = count($prompt_card_ids);
    $complete = $ids_complete;
    if (empty($prompt_card_ids) || !function_exists('ll_tools_get_prompt_card_reference_data_for_ids')) {
        if (!function_exists('ll_tools_get_prompt_card_reference_data_for_ids')) {
            $complete = false;
        }
        return [];
    }
    ...
    $data_complete = true;
    $data = ll_tools_get_prompt_card_reference_data_for_ids($prompt_card_ids, false, $data_complete);
    $complete = $complete && $data_complete;
    return $data;
}

function ll_tools_get_vocab_lesson_prompt_card_preview_image_word_ids(int $wordset_id, $category, int $limit = 2, ?bool &$complete = null): array {
    $complete = true;
    $limit = max(1, (int) $limit);
    $word_ids = [];
    $seen = [];
    $query_limit = min(100, max($limit * 5, $limit, 10));
    $scan_limit = max($query_limit, min(1000, (int) apply_filters(
        'll_tools_vocab_lesson_prompt_preview_scan_limit',
        300,
        $wordset_id,
        $category,
        $limit
    ...
    do {
        ...
        $page_limit = min($query_limit, $scan_limit - $scanned);
        $page_complete = true;
        $fetched_source_count = 0;
        $cards = ll_tools_get_vocab_lesson_prompt_card_preview_data(
            $wordset_id,
            $category,
            $page_limit,
            $offset,
        ...
        $scanned += $fetched_source_count;
        $offset += $fetched_source_count;
    } while ($fetched_source_count >= $page_limit);

    return $word_ids;
}

function ll_tools_get_vocab_lesson_prompt_card_preview_pairs(int $wordset_id, $category, int $limit = 2, ?bool &$complete = null): array {
    $complete = true;
    $limit = max(1, (int) $limit);
    $pairs = [];
    $seen = [];
...
function ll_tools_vocab_lesson_get_effective_quiz_config(
    int $wordset_id,
    $category,
    ?bool &$complete = null
): array {
    ...
    if (function_exists('ll_tools_get_category_quiz_config')) {
        $config_complete = true;
        $quiz_config = ll_tools_get_category_quiz_config($category, $config_complete);
        $complete = $config_complete;
    } else {
        $complete = false;
        $quiz_config = [
    ...
    if ($wordset_id > 0 && function_exists('ll_tools_apply_wordset_quiz_presentation_overrides')) {
        $presentation_complete = true;
        $quiz_config = ll_tools_apply_wordset_quiz_presentation_overrides(
            (array) $quiz_config,
            [$wordset_id],
            $presentation_complete
        );
    ...
    return is_array($quiz_config) ? $quiz_config : [];
}

function ll_tools_vocab_lesson_prompt_cards_use_image_choice_grid(int $wordset_id, $category): bool {
...
function ll_tools_render_vocab_lesson_prompt_cards_grid(
    int $wordset_id,
    $category,
    int $lesson_id = 0,
    ?bool &$complete = null
    ...
    $complete = true;
    $config_complete = true;
    $effective_quiz_config = ll_tools_vocab_lesson_get_effective_quiz_config(
        $wordset_id,
        $category,
        $config_complete
    );
    ...
    $audio_by_word = (!$use_image_choice_grid && function_exists('ll_tools_word_grid_collect_audio_files'))
    ...
    <div <?php echo $render_attrs($grid_attrs); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>>
        <?php foreach ($cards as $card) : ?>
            ...
            $prompt_has_body_content = $prompt_audio_url !== ''
                || trim((string) ($prompt_parts['text'] ?? '')) !== ''
                || trim((string) ($prompt_parts['translation'] ?? '')) !== ''
                || trim((string) ($prompt_parts['transcription'] ?? '')) !== '';
            ...
            <article class="ll-vocab-prompt-card" data-prompt-card-id="<?php echo esc_attr((string) $prompt_card_id); ?>">
                ...
                <div class="ll-vocab-prompt-card__body">
                    <?php if ($prompt_has_body_content) : ?>
...
/**
 * @return array<int,array{term_taxonomy_id:int,term_id:int,depth:int}>
 */
function ll_tools_vocab_lesson_get_word_category_depth_rows(?bool &$complete = null): array {
    global $wpdb;
...
function ll_tools_vocab_lesson_build_depth_table_sql(array $depth_rows): string {
    if (empty($depth_rows)) {
        return '';
    }
    ...
    foreach ($depth_rows as $row) {
        ...
        $term_taxonomy_id = isset($row['term_taxonomy_id']) ? (int) $row['term_taxonomy_id'] : 0;
        $term_id = isset($row['term_id']) ? (int) $row['term_id'] : 0;
        $depth = isset($row['depth']) ? max(0, (int) $row['depth']) : 0;
 ...
 * @return int[]
 */
function ll_tools_vocab_lesson_get_wrong_answer_candidate_word_ids(int $wordset_id, array $category_ids = [], ?bool &$complete = null): array {
    $complete = true;
    ...
    $wrong_answer_candidate_ids = function_exists('ll_tools_get_specific_wrong_answer_owner_candidate_word_ids')
        ...
        : [];
    if (function_exists('ll_tools_get_specific_wrong_answer_owner_candidate_word_ids')) {
    ...
    $term_taxonomy_complete = true;
    $wordset_tt_id = ll_tools_vocab_lesson_get_wordset_term_taxonomy_id($wordset_id, $term_taxonomy_complete);
    $complete = $complete && $term_taxonomy_complete;
    if ($wordset_tt_id <= 0) {
        return [];
    }
 ...
 * @return int[]
 */
function ll_tools_vocab_lesson_get_specific_wrong_answer_only_word_ids(int $wordset_id, array $category_ids = [], ?bool &$complete = null): array {
    $complete = true;
    ...
    $candidate_word_ids = ll_tools_vocab_lesson_get_wrong_answer_candidate_word_ids($wordset_id, $category_ids, $candidate_complete);
    $complete = $complete && $candidate_complete;
    if ($candidate_word_ids === []) {
        return [];
    }

    $lookup_complete = true;
    $lookup = ll_tools_get_specific_wrong_answer_only_word_lookup($candidate_word_ids, $lookup_complete);
    $complete = $complete && $lookup_complete;
    if (empty($lookup) || !is_array($lookup)) {
        return [];
    }

    $word_ids = array_values(array_filter(array_map('intval', array_keys($lookup)), static function (int $word_id): bool {
        return $word_id > 0;
    }));
 ...
 * @return array<int,int>
 */
function ll_tools_vocab_lesson_normalize_count_rows($rows): array {
    $counts = [];
    foreach ((array) $rows as $row) {
        if (!is_array($row)) {
            continue;
        ...
        $counts[$category_id] = ($counts[$category_id] ?? 0) + $total;
    }
 ...
 * @return array<int,int>
 */
function ll_tools_vocab_lesson_merge_count_maps(array ...$maps): array {
    $merged = [];
    foreach ($maps as $map) {
        foreach ($map as $category_id_raw => $total_raw) {
            $category_id = (int) $category_id_raw;
            $total = (int) $total_raw;
            if ($category_id <= 0 || $total <= 0) {
                continue;
            }
            $merged[$category_id] = ($merged[$category_id] ?? 0) + $total;
        }
    }
...
function ll_tools_vocab_lesson_count_word_deepest_categories(
    ...
    $complete = $wpdb->last_error === '';
    return ll_tools_vocab_lesson_normalize_count_rows($rows);
}
...
function ll_tools_vocab_lesson_count_prompt_card_deepest_categories(
    ...
    if ($only_with_images) {
        $prompt_image_meta_key = defined('LL_TOOLS_PROMPT_CARD_PROMPT_IMAGE_WORD_ID_META_KEY')
            ...
            : '_ll_prompt_card_prompt_image_word_id';
        $correct_answer_meta_key = defined('LL_TOOLS_PROMPT_CARD_CORRECT_ANSWER_WORD_ID_META_KEY')
            ? LL_TOOLS_PROMPT_CARD_CORRECT_ANSWER_WORD_ID_META_KEY
            : '_ll_prompt_card_correct_answer_word_id';

        $image_joins = "
            LEFT JOIN {$wpdb->postmeta} AS correct_answer_meta
                ON correct_answer_meta.post_id = posts.ID
                AND correct_answer_meta.meta_key = %s
                AND correct_answer_meta.meta_value <> ''
                AND CAST(correct_answer_meta.meta_value AS UNSIGNED) > 0
            LEFT JOIN {$wpdb->posts} AS correct_answer_words
                ON correct_answer_words.ID = CAST(correct_answer_meta.meta_value AS UNSIGNED)
                AND correct_answer_words.post_type = %s
                AND correct_answer_words.post_status = %s
            LEFT JOIN {$wpdb->postmeta} AS prompt_image_meta
                ON prompt_image_meta.post_id = posts.ID
                AND prompt_image_meta.meta_key = %s
                AND prompt_image_meta.meta_value <> ''
                AND CAST(prompt_image_meta.meta_value AS UNSIGNED) > 0
            LEFT JOIN {$wpdb->posts} AS prompt_image_words
        ...
        $image_presence_sql = ll_tools_effective_word_image_presence_sql('COALESCE(prompt_image_words.ID, correct_answer_words.ID)', $wordset_id, $image_where_params);
...
function ll_tools_get_vocab_lesson_deepest_counts_for_wordset(int $wordset_id, bool $force_refresh = false): array {
    ...
    $wordset_id = (int) $wordset_id;
    if ($wordset_id <= 0) {
        return ['all' => [], 'with_images' => [], 'complete' => true];
    }
    ...
    if ($force_refresh) {
        ...
        if (is_array($cached) && !empty($cached['complete'])) {
    ...
    $complete = $complete && $term_taxonomy_complete;
    $depth_rows_complete = true;
    $depth_rows = ll_tools_vocab_lesson_get_word_category_depth_rows($depth_rows_complete);
    $depth_by_term_taxonomy_id = ll_tools_vocab_lesson_depth_by_term_taxonomy_id($depth_rows);
    $complete = $complete && $depth_rows_complete;
    if ($wordset_tt_id <= 0 || empty($depth_by_term_taxonomy_id)) {
        $result = ['all' => [], 'with_images' => [], 'complete' => $complete];
        if ($complete) {
    ...
    $prompt_card_post_type = defined('LL_TOOLS_PROMPT_CARD_POST_TYPE') ? LL_TOOLS_PROMPT_CARD_POST_TYPE : 'll_prompt_card';
    $wrong_answer_complete = true;
    $excluded_word_ids = ll_tools_vocab_lesson_get_specific_wrong_answer_only_word_ids($wordset_id, [], $wrong_answer_complete);
    $complete = $complete && $wrong_answer_complete;

    $word_all_complete = true;
    $word_all_counts = ll_tools_vocab_lesson_count_word_deepest_categories($wordset_id, $wordset_tt_id, $depth_by_term_taxonomy_id, false, $excluded_word_ids, [], $word_all_complete);
    $complete = $complete && $word_all_complete;
    $word_image_complete = true;
    $word_image_counts = ll_tools_vocab_lesson_count_word_deepest_categories($wordset_id, $wordset_tt_id, $depth_by_term_taxonomy_id, true, $excluded_word_ids, [], $word_image_complete);
    $complete = $complete && $word_image_complete;
    $prompt_all_complete = true;
    $prompt_card_all_counts = ll_tools_vocab_lesson_count_prompt_card_deepest_categories($wordset_id, $wordset_tt_id, $depth_by_term_taxonomy_id, $prompt_card_post_type, false, [], $prompt_all_complete);
    $complete = $complete && $prompt_all_complete;
    $prompt_image_complete = true;
    $prompt_card_image_counts = ll_tools_vocab_lesson_count_prompt_card_deepest_categories($wordset_id, $wordset_tt_id, $depth_by_term_taxonomy_id, $prompt_card_post_type, true, [], $prompt_image_complete);
    $complete = $complete && $prompt_image_complete;

    $result = [
        'all' => ll_tools_vocab_lesson_merge_count_maps($word_all_counts, $prompt_card_all_counts),
        'with_images' => ll_tools_vocab_lesson_merge_count_maps($word_image_counts, $prompt_card_image_counts),
        'complete' => $complete,
    ];
    if ($complete) {
        $request_cache[$cache_key] = $result;
...
function ll_tools_get_vocab_lesson_category_word_count($category, int $wordset_id, ?array $counts = null, ?bool &$complete = null): int {
    $complete = true;
    $wordset_id = (int) $wordset_id;
    if ($wordset_id <= 0) {
        return 0;
    ...
    $category_resolution_complete = true;
    $category = ll_tools_resolve_vocab_lesson_category_for_wordset(
        $category,
        $wordset_id,
        false,
        $category_resolution_complete
    ...
    $complete = $category_resolution_complete;
    if (!$category_resolution_complete || !($category instanceof WP_Term)) {
        return 0;
    }
    ...
    if (array_key_exists('complete', $counts)) {
        $complete = $complete && !empty($counts['complete']);
    }

    $cat_id = (int) $category->term_id;
    $config_complete = true;
    $requires_images = ll_tools_vocab_lesson_category_requires_images($category, $wordset_id, $config_complete);
    $complete = $complete && $config_complete;
    if ($requires_images) {
...
function ll_tools_get_vocab_lesson_category_word_count_targeted_for_requirement(
    $category,
    int $wordset_id,
    bool $requires_images,
    ?bool &$complete = null
): int {
    static $request_cache = [];
    ...
    $category_id = (int) $category->term_id;
    $category_epoch = function_exists('ll_tools_get_category_cache_version')
    ...
    $cache_key = 'll_vocab_lesson_category_count_' . md5((string) wp_json_encode([
        'schema' => 4,
        'wordset_id' => $wordset_id,
        'category_id' => $category_id,
        'category_epoch' => $category_epoch,
    ...
    if ($cached !== false && is_numeric($cached)) {
        $request_cache[$cache_key] = max(0, (int) $cached);
        return $request_cache[$cache_key];
    }

    $term_taxonomy_complete = true;
    $wordset_tt_id = ll_tools_vocab_lesson_get_wordset_term_taxonomy_id($wordset_id, $term_taxonomy_complete);
    $complete = $term_taxonomy_complete && $wordset_tt_id > 0;
    $wrong_answer_complete = true;
    $excluded_word_ids = ll_tools_vocab_lesson_get_specific_wrong_answer_only_word_ids($wordset_id, [$category_id], $wrong_answer_complete);
    $complete = $complete && $wrong_answer_complete;
    $count_complete = true;
    $counts = ll_tools_vocab_lesson_count_word_deepest_categories(
        $wordset_id,
        $wordset_tt_id,
        [],
        $requires_images,
    ...
    $complete = $complete && $count_complete;
    $count = max(0, (int) ($counts[$category_id] ?? 0));
    $cache_ttl = max(MINUTE_IN_SECONDS, (int) apply_filters(
        'll_tools_vocab_lesson_category_count_cache_ttl',
        HOUR_IN_SECONDS,
        $wordset_id,
        $category_id
    ));
    if ($complete) {
        $request_cache[$cache_key] = $count;
        wp_cache_set($cache_key, $count, 'll_tools', $cache_ttl);
        set_transient($cache_key, $count, $cache_ttl);
    }
    return $count;
}

function ll_tools_get_vocab_lesson_category_word_count_targeted($category, int $wordset_id, ?bool &$complete = null): int {
    $complete = true;
    $wordset_id = (int) $wordset_id;
    if ($wordset_id <= 0) {
        return 0;
    ...
    $count_complete = true;
    $count = ll_tools_get_vocab_lesson_category_word_count_targeted_for_requirement(
        $category,
        $wordset_id,
        $requires_images,
        $count_complete
    );
    $complete = $config_complete && $count_complete;

    return $count;
}

function ll_tools_can_generate_vocab_lesson($category, int $wordset_id, ?array $counts = null, ?bool &$complete = null): bool {
    $complete = true;
    $wordset_id = (int) $wordset_id;
    if ($wordset_id <= 0) {
        return false;
    ...
    $category_resolution_complete = true;
    $category = ll_tools_resolve_vocab_lesson_category_for_wordset($category, $wordset_id, false, $category_resolution_complete);
    $complete = $category_resolution_complete;
    if (!$category_resolution_complete || !($category instanceof WP_Term)) {
        return false;
    }
    ...
    $word_count_complete = true;
    $word_count = ll_tools_get_vocab_lesson_category_word_count($category, $wordset_id, $counts, $word_count_complete);
    if (!$word_count_complete) {
        $complete = false;
        return false;
    }
    ...
    if (function_exists('ll_can_category_generate_quiz')) {
        ...
        $quiz_complete = true;
        $can_generate_quiz = ll_can_category_generate_quiz($category, $quiz_min_words, [$wordset_id], $quiz_complete);
        $complete = $complete && $quiz_complete;
        if (!$quiz_complete || !$can_generate_quiz) {
            return false;
        }
    }
...
function ll_tools_get_vocab_lesson_category_ids_for_wordset(int $wordset_id, bool $force_refresh = false, ?bool &$complete = null): array {
    $complete = true;
    $wordset_id = (int) $wordset_id;
    if ($wordset_id <= 0) {
        return [];
    ...
    $quiz_content_epoch = function_exists('ll_tools_get_quiz_content_cache_epoch')
        ? ll_tools_get_quiz_content_cache_epoch([$wordset_id])
        : (string) $category_epoch;
    $access_signature = function_exists('ll_tools_wordset_page_payload_access_signature')
        ? (string) ll_tools_wordset_page_payload_access_signature($wordset_id, (int) get_current_user_id())
        : '';
    $cache_key = 'll_vocab_lesson_cats_' . md5((string) wp_json_encode([
        'schema' => 5,
        'wordset_id' => $wordset_id,
    ...
    $ids = [];
    $eligibility_complete = !empty($counts['complete']);
    foreach ($category_ids as $category_id) {
        $category_complete = true;
        if (ll_tools_can_generate_vocab_lesson($category_id, $wordset_id, $counts, $category_complete)) {
            $ids[] = (int) $category_id;
        }
        $eligibility_complete = $eligibility_complete && $category_complete;
    }
...
function ll_tools_get_or_create_vocab_lesson_page(int $category_id, int $wordset_id) {
    ...
    $category_meta_candidates = array_map('strval', ll_tools_get_vocab_lesson_category_meta_candidates((int) $category->term_id, (int) $wordset->term_id));

    $meta_query = [
        'relation' => 'AND',
        [
            'key'   => LL_TOOLS_VOCAB_LESSON_CATEGORY_META,
            'value' => $category_meta_candidates,
            'compare' => 'IN',
        ],
        [
            'key'   => LL_TOOLS_VOCAB_LESSON_WORDSET_META,
    ...
    if (!empty($active_posts)) {
        ...
        if ($existing) {
            $update = ['ID' => $post_id];
            if ((string) $existing->post_title !== $title) {
                $update['post_title'] = $title;
            }
            if ((string) $existing->post_content !== $content) {
                $update['post_content'] = $content;
            }
            if ((string) $existing->post_status !== 'publish') {
                $update['post_status'] = 'publish';
            }
            if ($existing->post_name !== $slug) {
                $canonical_slug = wp_unique_post_slug(
                    $slug,
                    $post_id,
                ...
                if ((string) $existing->post_name !== $canonical_slug) {
                    $update['post_name'] = $canonical_slug;
                }
            }
            if (count($update) > 1) {
        ...
        foreach (array_slice($active_posts, 1) as $dup_id) {
            if (ll_tools_trash_vocab_lesson_post((int) $dup_id, ['notify' => false])) {
                $lesson_mutated = true;
            }
        }
        foreach ($trashed_posts as $trash_id) {
            if (wp_delete_post((int) $trash_id, true)) {
                $lesson_mutated = true;
            }
        }
        $status = 'updated';
    ...
    if ($post_id && !is_wp_error($post_id)) {
        $canonical_category_id = (string) $category->term_id;
        $canonical_wordset_id = (string) $wordset->term_id;
        $identity_meta_is_canonical = static function (array $stored_values, string $canonical_id): bool {
            if ($stored_values === []) {
                return false;
            }
            foreach ($stored_values as $stored_value) {
                if (!is_scalar($stored_value) || (string) $stored_value !== $canonical_id) {
                    return false;
                }
            }
            return true;
        };
        $stored_category_ids = (array) get_post_meta(
            $post_id,
            LL_TOOLS_VOCAB_LESSON_CATEGORY_META,
            false
        );
        if (!$identity_meta_is_canonical($stored_category_ids, $canonical_category_id)) {
            update_post_meta($post_id, LL_TOOLS_VOCAB_LESSON_CATEGORY_META, $canonical_category_id);
            $lesson_mutated = true;
        }
        $stored_wordset_ids = (array) get_post_meta(
            $post_id,
            LL_TOOLS_VOCAB_LESSON_WORDSET_META,
            false
        );
        if (!$identity_meta_is_canonical($stored_wordset_ids, $canonical_wordset_id)) {
            update_post_meta($post_id, LL_TOOLS_VOCAB_LESSON_WORDSET_META, $canonical_wordset_id);
            $lesson_mutated = true;
        }
        if (metadata_exists('post', $post_id, LL_TOOLS_VOCAB_LESSON_PREVIEW_ONLY_META)) {
            if (delete_post_meta($post_id, LL_TOOLS_VOCAB_LESSON_PREVIEW_ONLY_META)) {
                $lesson_mutated = true;
            }
        }
        if ($lesson_mutated) {
...
function ll_tools_vocab_lesson_is_preview_only(int $lesson_id): bool {
    $lesson_id = (int) $lesson_id;
    if ($lesson_id <= 0) {
        return false;
    }

    return get_post_meta($lesson_id, LL_TOOLS_VOCAB_LESSON_PREVIEW_ONLY_META, true) === '1';
}

function ll_tools_current_user_can_view_vocab_lesson_preview(int $lesson_id): bool {
    $lesson_id = (int) $lesson_id;
    ...
    return function_exists('ll_tools_current_user_can_manage_wordset_content')
        && ll_tools_current_user_can_manage_wordset_content($wordset_id);
}

function ll_tools_get_or_create_vocab_lesson_preview_page(int $category_id, int $wordset_id) {
    $category = function_exists('ll_tools_resolve_vocab_lesson_category_for_wordset')
        ? ll_tools_resolve_vocab_lesson_category_for_wordset($category_id, $wordset_id, true)
        : get_term($category_id, 'word-category');
    $wordset = get_term($wordset_id, 'wordset');
    ...
    $post_id = 0;
    $status = 'created';
    foreach ((array) $active_posts as $active_post_id) {
        ...
        $status = ll_tools_vocab_lesson_is_preview_only($active_post_id) ? 'updated' : 'existing';
        break;
    }

    $is_existing_public_lesson = false;
    if ($post_id > 0) {
        $existing = get_post($post_id);
        $is_existing_public_lesson = $existing instanceof WP_Post
            && $existing->post_status === 'publish'
            && !ll_tools_vocab_lesson_is_preview_only($post_id);
        $update = [
            'ID' => $post_id,
            'post_title' => $title,
            'post_content' => $content,
        ...
        if ($existing instanceof WP_Post && $existing->post_name !== $slug) {
        ...
        $post_id = wp_insert_post([
            'post_title' => $title,
            'post_name' => wp_unique_post_slug($slug, 0, 'publish', 'll_vocab_lesson', 0),
            'post_content' => $content,
            'post_status' => 'publish',
            'post_type' => 'll_vocab_lesson',
            'post_parent' => 0,
    ...
    if (!$is_existing_public_lesson) {
        update_post_meta($post_id, LL_TOOLS_VOCAB_LESSON_PREVIEW_ONLY_META, '1');
    }
    ll_tools_invalidate_wordset_page_lesson_cache();
...
function ll_tools_trash_vocab_lesson_page(
    int $category_id,
    int $wordset_id,
    ?bool &$complete = null
): int {
    ...
    $complete = true;
    $category_meta_candidates = array_map('strval', ll_tools_get_vocab_lesson_category_meta_candidates($category_id, $wordset_id));
    $meta_query = [
        'relation' => 'AND',
        [
            'key'   => LL_TOOLS_VOCAB_LESSON_CATEGORY_META,
    ...
    foreach ($posts as $pid) {
        if (ll_tools_trash_vocab_lesson_post((int) $pid)) {
            $removed++;
        } else {
            $complete = false;
            break;
...
function ll_tools_trash_vocab_lesson_pages_for_wordset(int $wordset_id): int {
    $wordset_id = (int) $wordset_id;
    if ($wordset_id <= 0) {
        return 0;
    }
...
function ll_tools_cleanup_invalid_vocab_lesson_pages(
    array $enabled_wordset_ids,
    array $args = [],
    ?bool &$complete = null
): int {
    global $wpdb;
    ...
    $cleanup_unavailable_categories = ll_tools_vocab_lesson_cleanup_unavailable_categories_allowed($args);
    $skip_wordset_ids = isset($args['skip_wordset_ids']) && is_array($args['skip_wordset_ids'])
        ? array_values(array_unique(array_filter(array_map('intval', $args['skip_wordset_ids']), static function (int $wordset_id): bool {
            return $wordset_id > 0;
        })))
        : [];
    ...
    foreach ($posts as $post) {
        ...
        $wordset_meta_complete = $wpdb->last_error === '';
        $wpdb->last_error = '';
        $category_id = (int) get_post_meta($post->ID, LL_TOOLS_VOCAB_LESSON_CATEGORY_META, true);
        $category_meta_complete = $wpdb->last_error === '';
        if (!$wordset_meta_complete || !$category_meta_complete) {
            $complete = false;
            break;
        }
        ...
        if (in_array($wordset_id, $skip_wordset_ids, true)) {
            continue;
        }

        if (!in_array($wordset_id, $enabled_wordset_ids, true)) {
            if (ll_tools_trash_vocab_lesson_post((int) $post->ID)) {
                $removed++;
            } else {
                $complete = false;
                break;
        ...
        $wpdb->last_error = '';
        $wordset = get_term($wordset_id, 'wordset');
        $wordset_lookup_complete = $wpdb->last_error === '' && !is_wp_error($wordset);
        $wpdb->last_error = '';
        $category = get_term($category_id, 'word-category');
        $category_lookup_complete = $wpdb->last_error === '' && !is_wp_error($category);
        if (!$wordset_lookup_complete || !$category_lookup_complete) {
            $complete = false;
            break;
        }
        if (!$wordset || !$category) {
        ...
        $can_generate = ll_tools_can_generate_vocab_lesson($category, $wordset_id, null, $eligibility_complete);
        if (!$eligibility_complete) {
            $complete = false;
            break;
        }
        if (!$can_generate) {
            if ($cleanup_unavailable_categories && ll_tools_trash_vocab_lesson_post((int) $post->ID)) {
                $removed++;
            } elseif ($cleanup_unavailable_categories) {
                $complete = false;
                break;
            }
        }
...
function ll_tools_sync_vocab_lesson_pages($wordset_ids = null, array $args = []): array {
    ...
    $cleanup_invalid_pages = ll_tools_vocab_lesson_cleanup_invalid_pages_allowed($args);
    $guard_recent_update = $cleanup_invalid_pages && ll_tools_vocab_lesson_recent_update_guard_active() && empty($args['manual']);

    if (empty($wordset_ids)) {
        if ($cleanup_invalid_pages && (!$guard_recent_update || ll_tools_get_active_vocab_lesson_page_count() <= 0)) {
            $cleanup_complete = true;
            $result['removed'] = ll_tools_cleanup_invalid_vocab_lesson_pages($wordset_ids, $args, $cleanup_complete);
            if (!$cleanup_complete) {
                ll_tools_queue_vocab_lesson_reconciliation_retry([], $args);
                return $result;
            }
        }
    ...
    foreach ($wordset_ids as $wordset_id) {
        $wordset_plan_complete = true;
        $sync_plan[$wordset_id] = ll_tools_get_vocab_lesson_category_ids_for_wordset(
            $wordset_id,
            true,
            $wordset_plan_complete
        );
        $sync_plan_complete[$wordset_id] = $wordset_plan_complete;
    }
    $incomplete_wordset_ids = array_values(array_map('intval', array_keys(array_filter(
        $sync_plan_complete,
        static function (bool $wordset_complete): bool {
            return !$wordset_complete;
        }
    ))));
    $result['incomplete_wordsets'] = count($incomplete_wordset_ids);

    if ($cleanup_invalid_pages) {
        $skip_cleanup_wordset_ids = $incomplete_wordset_ids;
        if ($guard_recent_update) {
            $active_counts = ll_tools_get_active_vocab_lesson_counts_by_wordset($wordset_ids);
            foreach ($wordset_ids as $wordset_id) {
                if (($active_counts[$wordset_id] ?? 0) > 0 && empty($sync_plan[$wordset_id])) {
                    $skip_cleanup_wordset_ids[] = (int) $wordset_id;
                }
            }
            $result['guarded_wordsets'] = count($skip_cleanup_wordset_ids);
        }

        $cleanup_complete = true;
        $result['removed'] = ll_tools_cleanup_invalid_vocab_lesson_pages($wordset_ids, array_merge($args, [
            'skip_wordset_ids' => $skip_cleanup_wordset_ids,
        ]), $cleanup_complete);
        if (!$cleanup_complete) {
            $incomplete_wordset_ids = $wordset_ids;
            $result['incomplete_wordsets'] = count($incomplete_wordset_ids);
        }
    }

    foreach ($wordset_ids as $wordset_id) {
        if (empty($sync_plan_complete[$wordset_id])) {
            continue;
        }
        $category_ids = isset($sync_plan[$wordset_id]) && is_array($sync_plan[$wordset_id])
            ? $sync_plan[$wordset_id]
            : [];
        foreach ($category_ids as $category_id) {
            $created = ll_tools_get_or_create_vocab_lesson_page((int) $category_id, (int) $wordset_id);
            if (is_wp_error($created)) {
                $incomplete_wordset_ids[] = (int) $wordset_id;
                break;
            }
    ...
    $incomplete_wordset_ids = array_values(array_unique(array_filter(array_map('intval', $incomplete_wordset_ids))));
    $result['incomplete_wordsets'] = count($incomplete_wordset_ids);
    if ($incomplete_wordset_ids !== []) {
        ll_tools_queue_vocab_lesson_reconciliation_retry($incomplete_wordset_ids, $args);
        return $result;
    }
...
function ll_tools_queue_vocab_lesson_reconciliation_retry($wordset_ids = null, array $args = []): array {
    $state = ll_tools_queue_vocab_lesson_reconciliation($wordset_ids, $args, false);
...
function ll_tools_process_vocab_lesson_reconciliation_cleanup_batch(array $state, int $batch_size): array {
    ...
    if ($wpdb->last_error !== '') {
        $state['status'] = 'queued';
        $state['message'] = __('Vocab lesson cleanup will retry after a temporary database error.', 'll-tools-text-domain');
        return $state;
    }
    ...
    foreach ($batch_ids as $page_id) {
        ...
        if (!$remove) {
            ...
            $remove = !($wordset instanceof WP_Term) || !($category instanceof WP_Term);
            if (!$remove && !empty($state['cleanup_unavailable_categories'])) {
                $eligibility_complete = true;
                $remove = !ll_tools_can_generate_vocab_lesson($category, $wordset_id, null, $eligibility_complete);
                if (!$eligibility_complete) {
                    $batch_incomplete = true;
                    break;
                }
        ...
        if ($remove) {
            if (!ll_tools_trash_vocab_lesson_post($page_id)) {
                $batch_incomplete = true;
                break;
            }
    ...
    $state['cleanup_processed'] = (int) ($state['cleanup_processed'] ?? 0) + $processed_in_batch;
    ...
    $state['status'] = 'queued';
    if ($batch_incomplete) {
        $state['message'] = __('Vocab lesson cleanup paused because category eligibility or source data was incomplete.', 'll-tools-text-domain');
        return $state;
    }
    if (!$has_more) {
        $state['phase'] = 'sync';
...
function ll_tools_get_vocab_lesson_reconciliation_candidate_ids(
    ...
    $complete = true;
    $wordset_tt_complete = true;
    $wordset_tt_id = ll_tools_vocab_lesson_get_wordset_term_taxonomy_id($wordset_id, $wordset_tt_complete);
    $complete = $wordset_tt_complete;
    if ($wordset_tt_id <= 0) {
        return [];
    }
    $prompt_card_post_type = defined('LL_TOOLS_PROMPT_CARD_POST_TYPE') ? LL_TOOLS_PROMPT_CARD_POST_TYPE : 'll_prompt_card';
...
/** @return array{all:array<int,int>,with_images:array<int,int>,complete:bool} */
function ll_tools_get_vocab_lesson_reconciliation_counts(
    int $wordset_id,
    array $category_ids,
    ?bool &$complete = null
): array {
    ...
    if ($wordset_tt_id <= 0 || $category_ids === []) {
        return ['all' => [], 'with_images' => [], 'complete' => $complete];
    }
...
/**
 * @return array{ids:int[],has_more:bool,counts:array<string,mixed>,complete:bool}
 */
function ll_tools_get_vocab_lesson_reconciliation_category_batch(
    ...
    $counts_complete = $candidate_complete;
    $counts = $candidate_complete
        ? ll_tools_get_vocab_lesson_reconciliation_counts($wordset_id, $batch_ids, $counts_complete)
        : ['all' => [], 'with_images' => [], 'complete' => false];

    return [
        'ids' => $batch_ids,
        'has_more' => count($candidate_ids) > $batch_size,
        'counts' => $counts,
        'complete' => $candidate_complete && $counts_complete && !empty($counts['complete']),
    ];
}
...
function ll_tools_process_vocab_lesson_reconciliation_sync_batch(array $state, int $batch_size): array {
    ...
    if ($wpdb->last_error !== '' || is_wp_error($wordset)) {
        $state['status'] = 'queued';
        $state['message'] = __('Vocab lesson synchronization will retry after a temporary wordset lookup error.', 'll-tools-text-domain');
        return $state;
    }
    ...
    if (empty($category_batch['complete'])) {
        $state['status'] = 'queued';
        $state['message'] = __('Vocab lesson synchronization paused because category candidates or counts were incomplete.', 'll-tools-text-domain');
        return $state;
    }
    $category_ids = (array) ($category_batch['ids'] ?? []);
    $counts = is_array($category_batch['counts'] ?? null)
        ? $category_batch['counts']
        : ['all' => [], 'with_images' => [], 'complete' => false];
    ...
    foreach ($category_ids as $category_id) {
        ...
        $eligibility_complete = true;
        $can_generate = ll_tools_can_generate_vocab_lesson(
            $category_id,
            $wordset_id,
            $counts,
        ...
        $state['category_cursor'] = $category_id;
        $processed_in_batch++;
    }

    $state['categories_processed'] = (int) ($state['categories_processed'] ?? 0) + $processed_in_batch;
    if ($batch_incomplete) {
        $state['status'] = 'queued';
        $state['message'] = __('Vocab lesson synchronization paused because category eligibility or a page write was incomplete.', 'll-tools-text-domain');
        return $state;
    }
...
function ll_tools_sync_vocab_lessons_for_category_immediate(int $category_id, bool $force = false, bool $allow_cleanup = false) {
    $category_id = (int) $category_id;
    if ($category_id <= 0) {
        return;
    }
    ...
    foreach ($wordset_ids as $wordset_id) {
        ...
        $eligibility_complete = !empty($counts['complete']);
        $can_generate = $eligibility_complete
            ? ll_tools_can_generate_vocab_lesson($category_id, $wordset_id, $counts, $eligibility_complete)
            : false;
        if (!$eligibility_complete) {
            $retry_wordset_ids[] = $wordset_id;
            continue;
        }
        if ($can_generate) {
            $created = ll_tools_get_or_create_vocab_lesson_page($category_id, $wordset_id);
        ...
        } elseif ($allow_cleanup) {
            $cleanup_complete = true;
            ll_tools_trash_vocab_lesson_page($category_id, $wordset_id, $cleanup_complete);
            if (!$cleanup_complete) {
                $retry_wordset_ids[] = $wordset_id;
            }
        }
    ...
    if ($retry_wordset_ids !== []) {
        if (isset($state) && is_array($state)) {
            unset($state['synced_vocab_category_ids'][$category_id]);
        }
        ll_tools_queue_vocab_lesson_reconciliation_retry($retry_wordset_ids);
    }
}
...
add_action('created_word-category', 'll_tools_handle_vocab_lesson_category_sync', 15, 1);
add_action('edited_word-category', 'll_tools_handle_vocab_lesson_category_sync', 15, 1);

function ll_tools_handle_vocab_lesson_category_delete($term_id) {
    $scoped_wordset_id = function_exists('ll_tools_wordset_page_get_category_delete_wordset_context')
        ? ll_tools_wordset_page_get_category_delete_wordset_context()
        : 0;
    if ($scoped_wordset_id > 0) {
        ll_tools_trash_vocab_lesson_page((int) $term_id, $scoped_wordset_id);
        return;
    }
...
function ll_tools_handle_vocab_lesson_wordset_delete($term_id) {
    $term_id = (int) $term_id;
    if ($term_id > 0) {
        $stored_wordset_ids = ll_tools_normalize_vocab_lesson_wordset_ids(
            get_option('ll_vocab_lesson_wordsets', [])
        );
        $remaining_wordset_ids = array_values(array_diff(
            ll_tools_get_vocab_lesson_wordset_ids(),
            [$term_id]
        ));

        if ($remaining_wordset_ids !== $stored_wordset_ids) {
            $previous_skip_auto_sync = !empty($GLOBALS['ll_tools_vocab_lesson_skip_auto_sync']);
            $GLOBALS['ll_tools_vocab_lesson_skip_auto_sync'] = true;
            try {
                update_option('ll_vocab_lesson_wordsets', $remaining_wordset_ids, false);
            } finally {
                if ($previous_skip_auto_sync) {
                    $GLOBALS['ll_tools_vocab_lesson_skip_auto_sync'] = true;
                } else {
    ...
    ll_tools_trash_vocab_lesson_pages_for_wordset($term_id);
}
add_action('delete_wordset', 'll_tools_handle_vocab_lesson_wordset_delete', 10, 1);

function ll_tools_vocab_lesson_full_sync_update_request_exact(array $before, array $after): bool {
    global $wpdb;
    ...
    $updated = $wpdb->query($wpdb->prepare(
        ...
        LL_TOOLS_VOCAB_LESSON_FULL_SYNC_REQUEST_OPTION,
        maybe_serialize($before)
    ));
    wp_cache_delete(LL_TOOLS_VOCAB_LESSON_FULL_SYNC_REQUEST_OPTION, 'options');
    return $updated === 1 && get_option(LL_TOOLS_VOCAB_LESSON_FULL_SYNC_REQUEST_OPTION, false) === $after;
}

function ll_tools_vocab_lesson_full_sync_delete_request_exact($before): bool {
    global $wpdb;
    ...
    wp_cache_delete(LL_TOOLS_VOCAB_LESSON_FULL_SYNC_REQUEST_OPTION, 'options');
    return $deleted === 1 || get_option(LL_TOOLS_VOCAB_LESSON_FULL_SYNC_REQUEST_OPTION, false) === false;
}

function ll_tools_request_vocab_lesson_full_sync(): array {
    $request = [
        'schema' => 1,
    ...
    for ($attempt = 0; $attempt < 3; $attempt++) {
        $existing = get_option(LL_TOOLS_VOCAB_LESSON_FULL_SYNC_REQUEST_OPTION, false);
        if ($existing === false) {
            $saved = add_option(LL_TOOLS_VOCAB_LESSON_FULL_SYNC_REQUEST_OPTION, $request, '', false);
        } elseif (is_array($existing)) {
            $saved = ll_tools_vocab_lesson_full_sync_update_request_exact($existing, $request);
        } else {
            $saved = ll_tools_vocab_lesson_full_sync_delete_request_exact($existing)
                && add_option(LL_TOOLS_VOCAB_LESSON_FULL_SYNC_REQUEST_OPTION, $request, '', false);
        }
        wp_cache_delete(LL_TOOLS_VOCAB_LESSON_FULL_SYNC_REQUEST_OPTION, 'options');
        if (!$saved) {
            continue;
        ...
        $stored = get_option(LL_TOOLS_VOCAB_LESSON_FULL_SYNC_REQUEST_OPTION, false);
        if (is_array($stored) && hash_equals($request['token'], (string) ($stored['token'] ?? ''))) {
            return $stored;
        }
    }
...
function ll_tools_schedule_vocab_lesson_sync_event(int $delay = 1): bool {
    if (!wp_next_scheduled(LL_TOOLS_VOCAB_LESSON_SYNC_EVENT)) {
        wp_schedule_single_event(time() + max(0, $delay), LL_TOOLS_VOCAB_LESSON_SYNC_EVENT);
    }
    return wp_next_scheduled(LL_TOOLS_VOCAB_LESSON_SYNC_EVENT) !== false;
}

function ll_tools_schedule_vocab_lesson_full_sync_follow_up(int $delay = MINUTE_IN_SECONDS): bool {
    if (!wp_next_scheduled(LL_TOOLS_VOCAB_LESSON_FULL_SYNC_EVENT)) {
        wp_schedule_single_event(time() + max(1, $delay), LL_TOOLS_VOCAB_LESSON_FULL_SYNC_EVENT);
    }
    return wp_next_scheduled(LL_TOOLS_VOCAB_LESSON_FULL_SYNC_EVENT) !== false;
}

function ll_tools_vocab_lesson_state_matches_full_sync_request(array $state, array $request): bool {
    $actual_wordset_ids = ll_tools_normalize_vocab_lesson_wordset_ids($state['wordset_ids'] ?? []);
    $expected_wordset_ids = ll_tools_normalize_vocab_lesson_wordset_ids(ll_tools_get_vocab_lesson_wordset_ids());
    ...
    return (string) ($state['status'] ?? '') === 'queued'
        ...
        && $actual_wordset_ids === $expected_wordset_ids;
}

function ll_tools_run_vocab_lesson_full_sync_follow_up(): void {
    $request = get_option(LL_TOOLS_VOCAB_LESSON_FULL_SYNC_REQUEST_OPTION, false);
    if (!is_array($request) || (string) ($request['token'] ?? '') === '') {
        if ($request !== false) {
            ll_tools_vocab_lesson_full_sync_delete_request_exact($request);
        }
        return;
    }
    ...
    ll_tools_schedule_vocab_lesson_full_sync_follow_up(MINUTE_IN_SECONDS);
    ...
    $state = ll_tools_get_vocab_lesson_reconciliation_state();
    $worker_scheduled = ll_tools_vocab_lesson_reconciliation_is_active($state)
        && ll_tools_schedule_vocab_lesson_sync_event(1);
    if ($worker_scheduled && ll_tools_vocab_lesson_state_matches_full_sync_request($state, $request)) {
        ll_tools_vocab_lesson_full_sync_delete_request_exact($request);
    }
}
add_action(LL_TOOLS_VOCAB_LESSON_FULL_SYNC_EVENT, 'll_tools_run_vocab_lesson_full_sync_follow_up');

function ll_tools_maybe_schedule_vocab_lesson_full_sync_follow_up(): void {
    $request = get_option(LL_TOOLS_VOCAB_LESSON_FULL_SYNC_REQUEST_OPTION, false);
    if (is_array($request) && (string) ($request['token'] ?? '') !== '') {
        ll_tools_schedule_vocab_lesson_full_sync_follow_up(1);
    }
    $state = ll_tools_get_vocab_lesson_reconciliation_state();
    if (ll_tools_vocab_lesson_reconciliation_is_active($state)) {
        ll_tools_schedule_vocab_lesson_sync_event(1);
...
add_action('admin_init', 'll_tools_maybe_schedule_vocab_lesson_full_sync_follow_up', 6);

function ll_tools_schedule_vocab_lesson_full_sync(int $delay = 30): void {
    $delay = max(0, $delay);
    $request = ll_tools_request_vocab_lesson_full_sync();
    $state = ll_tools_get_vocab_lesson_reconciliation_state();

    if (ll_tools_vocab_lesson_reconciliation_is_active($state)) {
        // Preserve the current scope and cursor. The distinct request event
        // will launch a fresh all-enabled-wordset pass after it finishes.
        ll_tools_schedule_vocab_lesson_sync_event($delay);
        if (!empty($request)) {
            ll_tools_schedule_vocab_lesson_full_sync_follow_up(max(1, $delay));
        }
        return;
    }
    ...
    $consumed = $worker_scheduled
        && !empty($request)
        && ll_tools_vocab_lesson_state_matches_full_sync_request($state, $request)
        && ll_tools_vocab_lesson_full_sync_delete_request_exact($request);
...
function ll_tools_vocab_lesson_state_covers_daily_full_sync(array $state): bool {
    if (!ll_tools_vocab_lesson_reconciliation_is_active($state)) {
        return false;
    }
    ...
    sort($actual_wordset_ids, SORT_NUMERIC);
    sort($expected_wordset_ids, SORT_NUMERIC);
    return $actual_wordset_ids === $expected_wordset_ids;
}

/** Ensure the daily safety net without chaining another pass behind this one. */
function ll_tools_ensure_daily_vocab_lesson_full_sync(int $delay = 30): void {
    $request = get_option(LL_TOOLS_VOCAB_LESSON_FULL_SYNC_REQUEST_OPTION, false);
    if (is_array($request) && (string) ($request['token'] ?? '') !== '') {
        ll_tools_schedule_vocab_lesson_full_sync_follow_up(1);
        return;
    ...
    $state = ll_tools_get_vocab_lesson_reconciliation_state();
    if (ll_tools_vocab_lesson_state_covers_daily_full_sync($state)) {
        ll_tools_schedule_vocab_lesson_sync_event($delay);
        return;
    }

    ll_tools_schedule_vocab_lesson_full_sync($delay);
}
...
add_action('admin_init', function () {
    ...
    if ($last < (time() - DAY_IN_SECONDS)) {
        ll_tools_ensure_daily_vocab_lesson_full_sync();
    }
});
...
function ll_tools_vocab_lesson_enforce_frontend_access(): void {
    ...
    if (ll_tools_vocab_lesson_is_preview_only($lesson_id) && !ll_tools_current_user_can_view_vocab_lesson_preview($lesson_id)) {
        global $wp_query;
...
function ll_tools_enqueue_vocab_lesson_word_options_modal_assets(): void {
    ...
    wp_localize_script('ll-tools-vocab-lesson-word-options-modal', 'llToolsVocabLessonWordOptions', [
        ...
        'i18n' => [
            ...
            'loadError' => __('Word options could not be opened.', 'll-tools-text-domain'),
            'loadTimeout' => __('Word options are taking too long to open.', 'll-tools-text-domain'),
            'retryLabel' => __('Retry', 'll-tools-text-domain'),
            'directOpenLabel' => __('Open in a new tab', 'll-tools-text-domain'),
        ],
    ]);
}
...
function ll_tools_get_vocab_lesson_category_title_edit_target($category, int $wordset_id = 0): array {
    if (!($category instanceof WP_Term)) {
        $category = get_term($category, 'word-category');
    }
    ...
    $translation_enabled = function_exists('ll_tools_is_category_translation_enabled')
        ? ll_tools_is_category_translation_enabled($wordset_ids)
...
function ll_tools_user_can_manage_vocab_lesson_category_settings(int $category_id = 0, int $wordset_id = 0): bool {
    if (!is_user_logged_in()) {
        return false;
    }

    if (($category_id <= 0 || $wordset_id <= 0) && is_singular('ll_vocab_lesson')) {
        $lesson_id = (int) get_queried_object_id();
        if ($lesson_id > 0) {
            if ($category_id <= 0) {
                $category_id = (int) get_post_meta($lesson_id, LL_TOOLS_VOCAB_LESSON_CATEGORY_META, true);
    ...
    if (current_user_can('manage_options') || current_user_can('edit_term', $category_id) || current_user_can('manage_categories')) {
        return true;
    }
...
function ll_tools_get_vocab_lesson_category_settings_panel_data($category, int $wordset_id = 0): array {
    ...
    if (!($category instanceof WP_Term)) {
        $wpdb->last_error = '';
        $category = get_term($category, 'word-category');
        if (is_wp_error($category) || $wpdb->last_error !== '') {
            $complete = false;
        }
    }
    ...
    $quiz_config = function_exists('ll_tools_get_category_quiz_config')
        ? ll_tools_get_category_quiz_config($category, $quiz_config_complete)
        : [
            'prompt_type' => 'audio',
            'option_type' => 'image',
        ];
    $complete = $complete && $quiz_config_complete;
    $visibility_complete = true;
    $lesson_grid_text_visibility = function_exists('ll_tools_get_category_lesson_grid_text_visibility_override')
    ...
    $enabled_games = function_exists('ll_tools_get_category_enabled_games')
        ...
        : [];
    $complete = $complete && $enabled_games_complete;
    $game_definitions = function_exists('ll_tools_get_category_game_definitions')
        ? ll_tools_get_category_game_definitions()
        : [];
    $recording_disabled_complete = true;
    $recording_disabled = function_exists('ll_tools_is_category_recording_disabled')
        ? ll_tools_is_category_recording_disabled((int) $category->term_id, $recording_disabled_complete)
        : false;
    $complete = $complete && $recording_disabled_complete;
    $recording_types_complete = true;
    $recording_types = function_exists('ll_tools_get_desired_recording_types_for_category')
        ? ll_tools_get_desired_recording_types_for_category((int) $category->term_id, $recording_types_complete)
        : [];
    ...
    $lineup = function_exists('ll_tools_get_category_lineup_stored_config')
        ? ll_tools_get_category_lineup_stored_config($category, $lineup_complete)
        : [
            'direction' => 'auto',
            'word_ids' => [],
        ];
...
function ll_tools_get_vocab_lesson_category_settings_notice(): ?array {
    $rawStatus = $_GET['ll_vocab_lesson_category_settings'] ?? '';
    if (!is_string($rawStatus) || strlen($rawStatus) > 32) {
        return null;
    }
    $status = sanitize_key(wp_unslash($rawStatus));
    if ($status === '') {
        return null;
    }

    $rawMessage = $_GET['ll_vocab_lesson_category_settings_message'] ?? '';
    $message = is_string($rawMessage) && strlen($rawMessage) <= 500
        ? sanitize_text_field(wp_unslash($rawMessage))
        : '';
    if ($status === 'ok') {
        return [
            'type' => 'success',
            'message' => $message !== '' ? $message : __('Category settings saved.', 'll-tools-text-domain'),
        ];
    }
    ...
    $rawError = $_GET['ll_vocab_lesson_category_settings_error'] ?? '';
    $error = is_string($rawError) && strlen($rawError) <= 64
        ? sanitize_key(wp_unslash($rawError))
        : '';

    switch ($error) {
        case 'permission':
            $message = __('You do not have permission to update this category.', 'll-tools-text-domain');
            break;
        case 'nonce':
            $message = __('Your session expired. Please try again.', 'll-tools-text-domain');
            break;
        case 'wordset':
            $message = __('This lesson no longer matches that word set category.', 'll-tools-text-domain');
            break;
        case 'category':
        ...
        case 'delete':
            ...
            break;
        case 'category_delete_state':
            $message = __('Category deletion progress could not be saved. Please try again.', 'll-tools-text-domain');
            break;
        case 'category_delete_busy':
            $message = __('Another category deletion batch is already running for this word set. Please try again shortly.', 'll-tools-text-domain');
            break;
        default:
...
function ll_tools_get_vocab_lesson_category_settings_success_redirect_url(int $wordset_id, int $category_id, string $fallback_url = ''): string
{
    $wordset_id = (int) $wordset_id;
    $category_id = (int) $category_id;
    $fallback_url = is_string($fallback_url) ? $fallback_url : '';
    $fallback_url = remove_query_arg([
        'll_vocab_lesson_category_settings',
        'll_vocab_lesson_category_settings_error',
        'll_vocab_lesson_category_settings_message',
    ], $fallback_url);

    $category_meta_candidates = function_exists('ll_tools_get_vocab_lesson_category_meta_candidates')
        ? array_map('strval', ll_tools_get_vocab_lesson_category_meta_candidates($category_id, $wordset_id))
        : [(string) $category_id];
    if (empty($category_meta_candidates)) {
        $category_meta_candidates = [(string) $category_id];
    }

    $active_lesson_ids = get_posts([
        'post_type' => 'll_vocab_lesson',
    ...
    if (!empty($active_lesson_ids)) {
        $active_lesson_permalink = get_permalink((int) $active_lesson_ids[0]);
        if (is_string($active_lesson_permalink) && $active_lesson_permalink !== '') {
            return add_query_arg([
                'll_vocab_lesson_category_settings' => 'ok',
            ], remove_query_arg([
                'll_vocab_lesson_category_settings',
                'll_vocab_lesson_category_settings_error',
                'll_vocab_lesson_category_settings_message',
            ], $active_lesson_permalink));
        }
    }

    $wordset_term = get_term($wordset_id, 'wordset');
    $wordset_redirect = ($fallback_url !== '') ? $fallback_url : home_url('/');
    if ($wordset_term instanceof WP_Term && !is_wp_error($wordset_term)) {
        if (function_exists('ll_tools_get_wordset_settings_tool_url')) {
            $wordset_redirect = ll_tools_get_wordset_settings_tool_url($wordset_term, 'categories');
        } else {
            $wordset_redirect = add_query_arg([
                'll_wordset_page' => (string) $wordset_term->slug,
...
function ll_tools_get_vocab_lesson_category_delete_redirect_url(int $wordset_id, string $fallback_url = '', string $result = 'deleted'): string {
    $wordset_id = (int) $wordset_id;
    $result = sanitize_key($result);
    if (!in_array($result, ['deleted', 'deleting'], true)) {
        $result = 'deleted';
    }
    ...
    return add_query_arg([
        ...
        'll_wordset_inactive_category_result' => $result,
    ], $redirect_url);
}

function ll_tools_get_vocab_lesson_category_delete_error_redirect_url(int $wordset_id, string $fallback_url, string $error, string $message): string {
    $redirect_url = ll_tools_get_vocab_lesson_category_delete_redirect_url($wordset_id, $fallback_url, 'deleting');
    ...
    return add_query_arg([
        'll_wordset_inactive_category' => 'error',
        'll_wordset_inactive_category_error' => sanitize_key($error),
        'll_wordset_inactive_category_message' => sanitize_text_field($message),
    ], $redirect_url);
}
...
function ll_tools_update_vocab_lesson_category_title_handler() {
    ...
    $wordset_id = (int) get_post_meta($lesson_id, LL_TOOLS_VOCAB_LESSON_WORDSET_META, true);
    $edit_target = ll_tools_get_vocab_lesson_category_title_edit_target($category, $wordset_id);
    ...
    $updated_target = ll_tools_get_vocab_lesson_category_title_edit_target($updated_category, $wordset_id);
    $display_name = function_exists('ll_tools_get_category_display_name')
...
function ll_tools_vocab_lesson_category_settings_error(string $error_code, string $message, int $status = 400, array $details = []): WP_Error {
    return new WP_Error(
...
function ll_tools_vocab_lesson_category_settings_revision_meta_key(): string {
    return '_ll_vocab_lesson_category_settings_revision';
}

function ll_tools_normalize_vocab_lesson_category_settings_client_id($value): string {
    $client_id = strtolower(trim((string) $value));
    return preg_match('/^[a-f0-9]{8}-(?:[a-f0-9]{4}-){3}[a-f0-9]{12}$/D', $client_id)
        ? $client_id
        : '';
}

function ll_tools_normalize_vocab_lesson_category_settings_revision_state($raw_state): array {
    $raw_state = maybe_unserialize($raw_state);
    if (!is_array($raw_state)) {
        return [
            'revision' => max(0, (int) $raw_state),
            'client_id' => '',
            'sequence' => 0,
        ];
    }
    ...
    return [
        'revision' => max(0, (int) ($raw_state['revision'] ?? 0)),
        'client_id' => ll_tools_normalize_vocab_lesson_category_settings_client_id($raw_state['client_id'] ?? ''),
        'sequence' => max(0, (int) ($raw_state['sequence'] ?? 0)),
    ];
}

function ll_tools_get_vocab_lesson_category_settings_revision_state(int $category_id): array {
    if ($category_id <= 0) {
        return ll_tools_normalize_vocab_lesson_category_settings_revision_state(0);
    }

    return ll_tools_normalize_vocab_lesson_category_settings_revision_state(get_term_meta(
        $category_id,
...
function ll_tools_get_vocab_lesson_category_settings_revision(int $category_id): int {
    $state = ll_tools_get_vocab_lesson_category_settings_revision_state($category_id);
    return (int) $state['revision'];
}

/**
 * Read the revision directly while the caller owns the category settings lock.
 *
 * Bypassing the metadata cache here ensures a request that waited for an older
 * writer observes the revision that writer committed before releasing the lock.
 *
 * @return array<string,int|string>|WP_Error
 */
function ll_tools_read_vocab_lesson_category_settings_revision_state(int $category_id) {
    global $wpdb;
    ...
    $wpdb->last_error = '';
    $raw_revision = $wpdb->get_var($wpdb->prepare(
        "SELECT meta_value
         FROM {$wpdb->termmeta}
    ...
    return ll_tools_normalize_vocab_lesson_category_settings_revision_state($raw_revision);
}

/**
 * @return int|WP_Error
 */
function ll_tools_read_vocab_lesson_category_settings_revision(int $category_id) {
    $state = ll_tools_read_vocab_lesson_category_settings_revision_state($category_id);
    return is_wp_error($state) ? $state : (int) $state['revision'];
}

function ll_tools_write_vocab_lesson_category_settings_revision_state(
    int $category_id,
    int $revision,
    string $client_id,
    int $sequence
): bool {
    if ($category_id <= 0 || $revision <= 0) {
        return false;
    }

    $state = [
        'revision' => $revision,
        'client_id' => ll_tools_normalize_vocab_lesson_category_settings_client_id($client_id),
    ...
    $confirmed = ll_tools_read_vocab_lesson_category_settings_revision_state($category_id);
    return !is_wp_error($confirmed) && $confirmed === $state;
}

/**
 * Persist one category-settings meta value and confirm the database row(s)
 * reflect it. A false update result can mean either failure or no change, so
 * the direct readback is authoritative.
 *
 * @param mixed $value
 */
function ll_tools_write_verified_vocab_lesson_category_setting_meta(
    int $category_id,
    string $meta_key,
    $value,
    bool $delete = false
    ...
    $write_result = $delete
        ...
        : update_term_meta($category_id, $meta_key, $value);
    if (is_wp_error($write_result) || $wpdb->last_error !== '') {
        return false;
    }
    ...
    foreach ($raw_values as $raw_value) {
        if (maybe_unserialize($raw_value) !== $value) {
            return false;
        }
    }
    return true;
...
function ll_tools_vocab_lesson_category_settings_termmeta_is_transactional(): bool {
    global $wpdb;

    static $detected_by_table = [];
    $table_name = (string) $wpdb->termmeta;
    if (!array_key_exists($table_name, $detected_by_table)) {
        try {
            $wpdb->last_error = '';
            ...
            $detected_by_table[$table_name] = $wpdb->last_error === ''
                && is_array($table_status)
                && strtolower((string) ($table_status['Engine'] ?? '')) === 'innodb';
        } catch (Throwable $throwable) {
            $detected_by_table[$table_name] = false;
        }
    }
...
function ll_tools_vocab_lesson_category_settings_lock_name(int $category_id): string {
    global $wpdb;

    $database_name = defined('DB_NAME') ? (string) DB_NAME : '';
    return 'll_vocab_cat_settings_' . substr(hash(
        'sha256',
        $database_name
...
function ll_tools_vocab_lesson_category_settings_connection_id(): string {
    global $wpdb;

    try {
        if ($wpdb->dbh instanceof mysqli) {
            return 'mysqli:' . spl_object_id($wpdb->dbh) . ':' . mysqli_thread_id($wpdb->dbh);
        }
        if (is_object($wpdb->dbh)) {
            return get_class($wpdb->dbh) . ':' . spl_object_id($wpdb->dbh);
        }
    } catch (Throwable $throwable) {
        // A closed handle cannot still own a named lock.
    }
    return '';
}

/** @return string|WP_Error */
function ll_tools_acquire_vocab_lesson_category_settings_lock(int $category_id) {
    global $wpdb;
    ...
    $lock_name = ll_tools_vocab_lesson_category_settings_lock_name($category_id);
    $connection_id = ll_tools_vocab_lesson_category_settings_connection_id();
    $held_locks = isset($GLOBALS['ll_tools_vocab_lesson_category_settings_held_locks'])
        && is_array($GLOBALS['ll_tools_vocab_lesson_category_settings_held_locks'])
        ? $GLOBALS['ll_tools_vocab_lesson_category_settings_held_locks']
        : [];
    if (
        isset($held_locks[$lock_name])
        && is_array($held_locks[$lock_name])
        && $connection_id !== ''
        && hash_equals((string) ($held_locks[$lock_name]['connection_id'] ?? ''), $connection_id)
    ) {
        $held_locks[$lock_name]['depth'] = max(1, (int) ($held_locks[$lock_name]['depth'] ?? 1)) + 1;
        $GLOBALS['ll_tools_vocab_lesson_category_settings_held_locks'] = $held_locks;
        return $lock_name;
    }
    if (isset($held_locks[$lock_name])) {
        unset($held_locks[$lock_name]);
        $GLOBALS['ll_tools_vocab_lesson_category_settings_held_locks'] = $held_locks;
    }

    try {
        $wait_seconds = max(0, min(10, (int) apply_filters(
            'll_tools_vocab_lesson_category_settings_lock_wait_seconds',
            3,
            $category_id
        )));
    } catch (Throwable $throwable) {
    ...
    if ($wpdb->last_error !== '' || $acquired === null || !in_array((string) $acquired, ['0', '1'], true)) {
        return ll_tools_vocab_lesson_category_settings_error(
            'lock_storage',
            __('Category settings cannot be saved because database locking is unavailable.', 'll-tools-text-domain'),
            503,
...
function ll_tools_release_vocab_lesson_category_settings_lock(string $lock_name): void {
    global $wpdb;
    ...
    $held_locks = isset($GLOBALS['ll_tools_vocab_lesson_category_settings_held_locks'])
        ...
        ? $GLOBALS['ll_tools_vocab_lesson_category_settings_held_locks']
        : [];
    if (isset($held_locks[$lock_name]) && is_array($held_locks[$lock_name])) {
        $current_connection_id = ll_tools_vocab_lesson_category_settings_connection_id();
        $held_connection_id = (string) ($held_locks[$lock_name]['connection_id'] ?? '');
        if ($current_connection_id === '' || !hash_equals($held_connection_id, $current_connection_id)) {
            unset($held_locks[$lock_name]);
            $GLOBALS['ll_tools_vocab_lesson_category_settings_held_locks'] = $held_locks;
            return;
        }
        $depth = max(1, (int) ($held_locks[$lock_name]['depth'] ?? 1));
        if ($depth > 1) {
            $held_locks[$lock_name]['depth'] = $depth - 1;
            $GLOBALS['ll_tools_vocab_lesson_category_settings_held_locks'] = $held_locks;
            return;
        }
        unset($held_locks[$lock_name]);
    ...
    try {
        ...
        // Named locks are connection-scoped and are released at request end.
    }
}

/** @return true|WP_Error */
function ll_tools_require_vocab_lesson_category_exists_after_lock(int $category_id) {
    global $wpdb;
...
/**
 * Serialize an external category-settings mutation with lesson autosaves and
 * commit one conservative revision bump in the same transaction.
 *
 * @param callable():mixed $mutation
 * @return mixed|WP_Error
 */
function ll_tools_run_vocab_lesson_category_settings_external_mutation(
    int $category_id,
    callable $mutation,
    ...
    $lock_name = ll_tools_acquire_vocab_lesson_category_settings_lock($category_id);
    ...
    $settings_transaction = null;
    try {
        // A caller may have primed term meta before waiting for this lock. Drop
        // that snapshot so every read made while holding the lock observes the
        // writer that committed immediately before us.
        try {
            wp_cache_delete($category_id, 'term_meta');
        } catch (Throwable $throwable) {
            // Direct database reads below remain authoritative.
        }

        $category_exists = ll_tools_require_vocab_lesson_category_exists_after_lock($category_id);
        if (is_wp_error($category_exists)) {
            return $category_exists;
        }

        $revision_state = ll_tools_read_vocab_lesson_category_settings_revision_state($category_id);
        if (is_wp_error($revision_state)) {
            return $revision_state;
        }
        $current_revision = (int) $revision_state['revision'];
        if ($expected_revision !== null && $expected_revision !== $current_revision) {
            return ll_tools_vocab_lesson_category_settings_error(
                'revision_conflict',
                __('Category settings changed elsewhere. Reload the page and try again.', 'll-tools-text-domain'),
                409,
                [
                    'current_revision' => $current_revision,
                    'same_client_predecessor' => 0,
                ]
            );
        }
        if ($current_revision >= 2147483646) {
            return ll_tools_vocab_lesson_category_settings_error(
                'revision_write',
                __('Unable to save category settings right now.', 'll-tools-text-domain'),
                503,
        ...
        if (!ll_tools_vocab_lesson_category_settings_termmeta_is_transactional()) {
            return ll_tools_vocab_lesson_category_settings_error(
                'transaction_storage',
                __('Category settings cannot be saved until the term metadata table supports transactions.', 'll-tools-text-domain'),
                503,
                ['retryable' => false]
            );
        }

        $settings_transaction = function_exists('ll_tools_strict_term_insert_begin_transaction')
            ? ll_tools_strict_term_insert_begin_transaction()
            : null;
        if (!is_array($settings_transaction)) {
            return ll_tools_vocab_lesson_category_settings_error(
                'transaction_start',
                __('Unable to save category settings right now.', 'll-tools-text-domain'),
                503,
        ...
        if (!ll_tools_write_vocab_lesson_category_settings_revision_state(
            $category_id,
            $next_revision,
            '',
            0
        ...
        if (!ll_tools_strict_term_insert_commit_transaction($settings_transaction)) {
            return ll_tools_vocab_lesson_category_settings_error(
                'transaction_commit',
                __('Unable to save category settings right now.', 'll-tools-text-domain'),
                503,
        ...
        if ($transaction_active) {
            try {
                if (is_array($settings_transaction) && function_exists('ll_tools_strict_term_insert_rollback_transaction')) {
                    ll_tools_strict_term_insert_rollback_transaction($settings_transaction);
                } else {
                    $wpdb->query('ROLLBACK');
                }
            } catch (Throwable $throwable) {
                // The named lock remains held until the connection closes.
            }
            try {
                wp_cache_delete($category_id, 'term_meta');
            } catch (Throwable $throwable) {
                // The SQL rollback result is independent of cache eviction.
            }
        }
        ll_tools_release_vocab_lesson_category_settings_lock($lock_name);
    }
}
 ...
 */
function ll_tools_vocab_lesson_category_settings_bounded_scalar($value, int $max_bytes, bool &$valid): string {
    ...
    if (strlen($value) > max(1, $max_bytes)) {
        $valid = false;
        return '';
    }
    return $value;
...
function ll_tools_vocab_lesson_category_settings_positive_id($value, bool &$valid): int {
 ...
 * @return string[]
 */
function ll_tools_vocab_lesson_category_settings_bounded_string_list(
    $value,
    int $max_values,
    int $max_value_bytes,
    int $max_total_bytes,
    ...
    $valid = is_array($value) && count($value) <= max(1, $max_values);
    if (!$valid) {
        return [];
    }
    ...
    foreach ($value as $key => $item) {
        ...
        $total_bytes += strlen((string) $key) + strlen($item);
        if (strlen($item) > max(1, $max_value_bytes) || $total_bytes > max(1, $max_total_bytes)) {
            $valid = false;
            return [];
        }
        $values[] = $item;
...
function ll_tools_validate_vocab_lesson_category_settings_request_shape(array $request) {
    ...
    $prompt_type = $prompt_present
        ? ll_tools_vocab_lesson_category_settings_bounded_scalar(
            $request['ll_vocab_lesson_quiz_prompt_type'],
            64,
            $valid
        )
        : '';
    ...
    $option_present = array_key_exists('ll_vocab_lesson_quiz_option_type', $request);
    $option_type = $option_present
        ? ll_tools_vocab_lesson_category_settings_bounded_scalar(
            $request['ll_vocab_lesson_quiz_option_type'],
            64,
            $valid
        )
        : '';
    ...
    $grid_visibility = array_key_exists('ll_vocab_lesson_grid_text_visibility', $request)
        ? sanitize_key(ll_tools_vocab_lesson_category_settings_bounded_scalar(
            $request['ll_vocab_lesson_grid_text_visibility'],
            16,
            $valid
        ))
        : 'inherit';
    if (!$valid || !in_array($grid_visibility, ['inherit', 'show', 'hide'], true)) {
        return ll_tools_vocab_lesson_category_settings_error(
            'settings_request',
            __('Invalid request.', 'll-tools-text-domain'),
            400
    ...
    $enabled_games = ll_tools_vocab_lesson_category_settings_bounded_string_list(
        $request['ll_vocab_lesson_category_enabled_games'] ?? [],
        32,
        64,
        2048,
        $valid
    ...
    $recording_types = ll_tools_vocab_lesson_category_settings_bounded_string_list(
        $request['ll_vocab_lesson_desired_recording_types'] ?? [],
        100,
        200,
        20000,
        $valid
    ...
    $lineup_submitted = array_key_exists('ll_vocab_lesson_category_lineup_submitted', $request);
    $lineup_direction = 'auto';
    $lineup_replace = false;
    $lineup_word_ids = '';
    if ($lineup_submitted) {
        $submitted_marker = ll_tools_vocab_lesson_category_settings_bounded_scalar(
            $request['ll_vocab_lesson_category_lineup_submitted'],
            1,
            $valid
        );
        $lineup_direction = sanitize_key(ll_tools_vocab_lesson_category_settings_bounded_scalar(
            $request['ll_vocab_lesson_category_lineup_direction'] ?? 'auto',
            8,
            $valid
        ));
        $replace_marker = ll_tools_vocab_lesson_category_settings_bounded_scalar(
            $request['ll_vocab_lesson_category_lineup_replace'] ?? '0',
            1,
            $valid
        );
        if (
            !$valid
            || $submitted_marker !== '1'
            || !in_array($lineup_direction, ['auto', 'ltr', 'rtl'], true)
            || !in_array($replace_marker, ['0', '1'], true)
        ) {
            return ll_tools_vocab_lesson_category_settings_error(
                'settings_request',
                __('Invalid request.', 'll-tools-text-domain'),
        ...
        $lineup_replace = $replace_marker === '1';
        if ($lineup_replace) {
            $lineup_word_ids = ll_tools_vocab_lesson_category_settings_bounded_scalar(
                $request['ll_vocab_lesson_category_lineup_word_ids'] ?? '',
                10000,
                $valid
            );
            if (!$valid) {
    ...
    return [
        ...
        'lineup_replace' => $lineup_replace,
        'lineup_word_ids' => $lineup_word_ids,
    ];
}
...
function ll_tools_save_vocab_lesson_category_settings_from_request(array $request) {
    ...
    $action_valid = true;
    $action = sanitize_key(ll_tools_vocab_lesson_category_settings_bounded_scalar(
        $request['ll_vocab_lesson_category_settings_action'] ?? '',
        16,
        $action_valid
    ));
    if (!$action_valid || !in_array($action, ['save', 'delete'], true)) {
        return ll_tools_vocab_lesson_category_settings_error(
            'action',
            __('Invalid request.', 'll-tools-text-domain'),
            400
    ...
    $revision_required = !empty($request['_ll_vocab_lesson_category_settings_require_revision']);
    $revision_present = array_key_exists('ll_vocab_lesson_category_settings_revision', $request);
    $client_id_present = array_key_exists('ll_vocab_lesson_category_settings_client_id', $request);
    $sequence_present = array_key_exists('ll_vocab_lesson_category_settings_sequence', $request);
    $submitted_revision = 0;
    $submitted_client_id = '';
    $submitted_sequence = 0;
    if ($revision_present) {
        $revision_value = $request['ll_vocab_lesson_category_settings_revision'];
        if (is_array($revision_value) || is_object($revision_value)) {
            return ll_tools_vocab_lesson_category_settings_error(
                'revision_request',
                __('Unable to save category settings right now.', 'll-tools-text-domain'),
                409
        ...
        $raw_revision = trim((string) $revision_value);
        if ($raw_revision === '' || !preg_match('/^\d+$/D', $raw_revision) || strlen($raw_revision) > 10) {
            return ll_tools_vocab_lesson_category_settings_error(
                'revision_request',
                __('Unable to save category settings right now.', 'll-tools-text-domain'),
                409
        ...
        $submitted_revision = (int) $raw_revision;
        if ($submitted_revision > 2147483646) {
            return ll_tools_vocab_lesson_category_settings_error(
                'revision_request',
                __('Unable to save category settings right now.', 'll-tools-text-domain'),
                409
    ...
    if ($client_id_present) {
        $client_id_value = $request['ll_vocab_lesson_category_settings_client_id'];
        if (is_array($client_id_value) || is_object($client_id_value) || strlen((string) $client_id_value) > 36) {
            return ll_tools_vocab_lesson_category_settings_error(
                'revision_request',
                __('Unable to save category settings right now.', 'll-tools-text-domain'),
                409
        ...
        $submitted_client_id = ll_tools_normalize_vocab_lesson_category_settings_client_id(
            wp_unslash((string) $client_id_value)
        );
    ...
    if ($sequence_present) {
        $sequence_value = $request['ll_vocab_lesson_category_settings_sequence'];
        if (is_array($sequence_value) || is_object($sequence_value)) {
            return ll_tools_vocab_lesson_category_settings_error(
                'revision_request',
                __('Unable to save category settings right now.', 'll-tools-text-domain'),
                409
        ...
        $raw_sequence = trim((string) $sequence_value);
        if ($raw_sequence === '' || !preg_match('/^\d+$/D', $raw_sequence) || strlen($raw_sequence) > 10) {
            return ll_tools_vocab_lesson_category_settings_error(
                'revision_request',
                __('Unable to save category settings right now.', 'll-tools-text-domain'),
                409
        ...
        $submitted_sequence = (int) $raw_sequence;
        if ($submitted_sequence <= 0 || $submitted_sequence > 2147483646) {
            return ll_tools_vocab_lesson_category_settings_error(
                'revision_request',
                __('Unable to save category settings right now.', 'll-tools-text-domain'),
                409
    ...
    $fence_present = $revision_present || $client_id_present || $sequence_present;
    if (
        $action === 'save'
        && (($revision_required || $fence_present) && (!$revision_present || !$client_id_present || !$sequence_present))
    ) {
        return ll_tools_vocab_lesson_category_settings_error(
            'revision_request',
            __('Unable to save category settings right now.', 'll-tools-text-domain'),
    ...
    $lesson_id = ll_tools_vocab_lesson_category_settings_positive_id(
        $request['ll_vocab_lesson_category_settings_lesson_id'] ?? '',
        $lesson_id_valid
    );
    $wordset_id_valid = true;
    $submitted_wordset_id = ll_tools_vocab_lesson_category_settings_positive_id(
        $request['ll_vocab_lesson_category_settings_wordset_id'] ?? '',
        $wordset_id_valid
    );
    $category_id_valid = true;
    $submitted_category_id = ll_tools_vocab_lesson_category_settings_positive_id(
        $request['ll_vocab_lesson_category_settings_category_id'] ?? '',
        $category_id_valid
    );
    if (!$lesson_id_valid || !$wordset_id_valid || !$category_id_valid) {
        return ll_tools_vocab_lesson_category_settings_error(
            'request',
            __('Invalid request.', 'll-tools-text-domain'),
            400
    ...
    $nonce_valid = true;
    $nonce = ll_tools_vocab_lesson_category_settings_bounded_scalar(
        $request['ll_vocab_lesson_category_settings_nonce'] ?? '',
        128,
        $nonce_valid
    );
    if (!$nonce_valid) {
    ...
    $owner_wordset_id = function_exists('ll_tools_get_category_wordset_owner_id')
        ? (int) ll_tools_get_category_wordset_owner_id($category, $owner_source_complete)
        : 0;
    if (!$owner_source_complete) {
        return ll_tools_vocab_lesson_category_settings_error(
            'owner_source',
            __('Unable to save category settings right now.', 'll-tools-text-domain'),
            503,
    ...
    if ($action === 'delete') {
        if (!function_exists('ll_tools_wordset_page_run_category_delete_batch')) {
            return ll_tools_vocab_lesson_category_settings_error(
                'delete',
        ...
        $delete_result = ll_tools_wordset_page_run_category_delete_batch($category_id, $wordset_id, 'delete_blocked');
        if (is_wp_error($delete_result)) {
            if (
                $delete_result->get_error_code() === 'category_delete_busy'
                && function_exists('ll_tools_wordset_page_build_category_delete_action_result')
            ) {
                $delete_job = ll_tools_wordset_page_get_category_delete_job($category_id, $wordset_id);
                if (empty($delete_job)) {
                    return ll_tools_vocab_lesson_category_settings_error(
                        'category_delete_busy',
                        $delete_result->get_error_message(),
                        409
                    );
                }
                $action_result = ll_tools_wordset_page_build_category_delete_action_result(
                    $category_id,
                    $wordset_id,
                    $delete_job,
                    $delete_result->get_error_message()
                );
                $action_result['lesson_id'] = $lesson_id;
                return $action_result;
            }
            return ll_tools_vocab_lesson_category_settings_error(
                ll_tools_get_vocab_lesson_category_settings_error_code($delete_result),
                $delete_result->get_error_message(),
                in_array($delete_result->get_error_code(), ['delete_blocked', 'category_delete_busy'], true) ? 409 : 400
            );
        }

        $action_result = ll_tools_wordset_page_build_category_delete_action_result($category_id, $wordset_id, $delete_result);
        $action_result['lesson_id'] = $lesson_id;
        return $action_result;
    }

    $validated_settings_request = ll_tools_validate_vocab_lesson_category_settings_request_shape($request);
    ...
    if (!empty($validated_settings_request['lineup_submitted'])) {
        $lineup_update = [
            'direction' => (string) $validated_settings_request['lineup_direction'],
            'replace' => !empty($validated_settings_request['lineup_replace']),
            'word_ids' => null,
        ];

        if ($lineup_update['replace']) {
            $raw_lineup_sequence = (string) $validated_settings_request['lineup_word_ids'];
    ...
    $settings_lock = ll_tools_acquire_vocab_lesson_category_settings_lock($category_id);
    if (is_wp_error($settings_lock)) {
    ...
    $settings_transaction_active = false;
    $settings_transaction = null;
    try {
        wp_cache_delete($category_id, 'term_meta');
        ...
        $wpdb->last_error = '';
        $locked_lesson_wordset_id = (int) get_post_meta(
            $lesson_id,
            LL_TOOLS_VOCAB_LESSON_WORDSET_META,
            true
        );
        $locked_lesson_wordset_complete = $wpdb->last_error === '';
        $wpdb->last_error = '';
        $locked_lesson_category_id = (int) get_post_meta(
            $lesson_id,
            LL_TOOLS_VOCAB_LESSON_CATEGORY_META,
            true
        );
        $locked_lesson_category_complete = $wpdb->last_error === '';
        if (!$locked_lesson_wordset_complete || !$locked_lesson_category_complete) {
            return ll_tools_vocab_lesson_category_settings_error(
                'lesson_source',
                __('Unable to save category settings right now.', 'll-tools-text-domain'),
                503,
        ...
        if (
            $locked_lesson_wordset_id !== $wordset_id
            || $locked_lesson_category_id !== $category_id
        ) {
            return ll_tools_vocab_lesson_category_settings_error(
                'wordset',
                __('This lesson no longer matches that word set category.', 'll-tools-text-domain'),
        ...
        $locked_owner_source_complete = true;
        $locked_owner_wordset_id = function_exists('ll_tools_get_category_wordset_owner_id')
            ? (int) ll_tools_get_category_wordset_owner_id($category_id, $locked_owner_source_complete)
            : 0;
        if (!$locked_owner_source_complete) {
            return ll_tools_vocab_lesson_category_settings_error(
                'owner_source',
                __('Unable to save category settings right now.', 'll-tools-text-domain'),
                503,
        ...
        if ($locked_owner_wordset_id > 0 && $locked_owner_wordset_id !== $wordset_id) {
            return ll_tools_vocab_lesson_category_settings_error(
                'wordset',
                __('This lesson no longer matches that word set category.', 'll-tools-text-domain'),
                409
        ...
        if (is_wp_error($current_revision_state)) {
            return $current_revision_state;
        }
        $current_revision = (int) $current_revision_state['revision'];
        $same_client = $submitted_client_id !== ''
            && hash_equals((string) $current_revision_state['client_id'], $submitted_client_id);
        if ($revision_present && $submitted_revision !== $current_revision) {
            $same_client_predecessor = $same_client
                && (int) $current_revision_state['sequence'] > 0
                && (int) $current_revision_state['sequence'] < $submitted_sequence;
            return ll_tools_vocab_lesson_category_settings_error(
                'revision_conflict',
                __('Unable to save category settings right now.', 'll-tools-text-domain'),
                409,
                [
                    'current_revision' => $current_revision,
                    'same_client_predecessor' => $same_client_predecessor ? 1 : 0,
                ]
            );
        }
        if (
            ...
            && $same_client
            && (int) $current_revision_state['sequence'] > 0
            && $submitted_sequence <= (int) $current_revision_state['sequence']
        ) {
            return ll_tools_vocab_lesson_category_settings_error(
                'revision_conflict',
                __('Unable to save category settings right now.', 'll-tools-text-domain'),
        ...
        $wpdb->last_error = '';
        $available_recording_type_terms = get_terms([
            'taxonomy' => 'recording_type',
            'hide_empty' => false,
        ]);
        if (is_wp_error($available_recording_type_terms) || $wpdb->last_error !== '') {
            return ll_tools_vocab_lesson_category_settings_error(
        ...
        foreach ($available_recording_type_terms as $recording_type_term) {
            if (!($recording_type_term instanceof WP_Term)) {
                continue;
            }
            $allowed_recording_types[sanitize_key((string) $recording_type_term->slug)] = true;
        }

        $use_titles = !empty($existing_quiz_config['use_titles']);
        $prompt_type = !empty($validated_settings_request['prompt_present'])
            ? ll_tools_normalize_quiz_prompt_type((string) $validated_settings_request['prompt_type'], $use_titles)
            : (string) ($existing_quiz_config['prompt_type'] ?? 'audio');
        $option_type = !empty($validated_settings_request['option_present'])
            ? ll_tools_normalize_quiz_option_type(
                (string) $validated_settings_request['option_type'],
                $use_titles,
                $prompt_type
            )
            : (string) ($existing_quiz_config['option_type'] ?? 'image');
        $keep_word_titles = function_exists('ll_tools_should_keep_word_titles_for_audio_meta')
            ? ll_tools_should_keep_word_titles_for_audio_meta($prompt_type, $option_type, $use_titles)
            : $option_type === 'text_title';
        $lesson_grid_text_visibility = (string) $validated_settings_request['grid_visibility'];
        $enabled_games = ll_tools_normalize_category_enabled_games(
            (array) $validated_settings_request['enabled_games']
        );
        $selected_recording_types = [];
        foreach ((array) $validated_settings_request['recording_types'] as $raw_recording_type) {
        ...
        $desired_recording_types = empty($selected_recording_types)
        ...
        $settings_transaction_active = true;
        ...
        $settings_write_failed = !ll_tools_write_verified_vocab_lesson_category_setting_meta(
            ...
            $lesson_grid_text_visibility,
            !in_array($lesson_grid_text_visibility, ['show', 'hide'], true)
        ) || !ll_tools_write_verified_vocab_lesson_category_setting_meta(
            $category_id,
    ...
    try {
        ...
        // The durable settings write succeeded; later requests can refresh caches.
    }
    ...
    return [
        ...
        if ($settings_transaction_active) {
            try {
                if (is_array($settings_transaction) && function_exists('ll_tools_strict_term_insert_rollback_transaction')) {
                    ll_tools_strict_term_insert_rollback_transaction($settings_transaction);
                } else {
                ...
                // Closing the database connection is handled by the helper.
            }
            try {
                wp_cache_delete($category_id, 'term_meta');
            } catch (Throwable $throwable) {
                // SQL rollback is independent of cache eviction.
            }
        }
        ll_tools_release_vocab_lesson_category_settings_lock($settings_lock);
    }
}
...
function ll_tools_handle_vocab_lesson_category_settings_submit(): void {
    ...
    $action_valid = true;
    $action = sanitize_key(ll_tools_vocab_lesson_category_settings_bounded_scalar(
        $_POST['ll_vocab_lesson_category_settings_action'] ?? '',
        16,
        $action_valid
    ));
    if (!$action_valid) {
    ...
    $lesson_id_valid = true;
    $lesson_id = ll_tools_vocab_lesson_category_settings_positive_id(
        $_POST['ll_vocab_lesson_category_settings_lesson_id'] ?? '',
        $lesson_id_valid
    );
    if (!$lesson_id_valid) {
        $lesson_id = 0;
    ...
    $request = $_POST;
    if ($action === 'save') {
        $request['_ll_vocab_lesson_category_settings_require_revision'] = '1';
    }
    $result = ll_tools_save_vocab_lesson_category_settings_from_request($request);
    if (is_wp_error($result)) {
        if ($action === 'delete' && $lesson_id > 0 && get_post($lesson_id) === null) {
            $wordset_id_valid = true;
            $wordset_id = ll_tools_vocab_lesson_category_settings_positive_id(
                $_POST['ll_vocab_lesson_category_settings_wordset_id'] ?? '',
                $wordset_id_valid
            );
            if (!$wordset_id_valid) {
                $wordset_id = 0;
            }
            wp_safe_redirect(ll_tools_get_vocab_lesson_category_delete_error_redirect_url(
                $wordset_id,
                $base_redirect,
...
function ll_tools_save_vocab_lesson_category_settings_ajax_handler(): void {
    ...
    if (is_wp_error($result)) {
        ...
        if (is_array($data) && isset($data['current_revision'])) {
            $response['current_revision'] = max(0, (int) $data['current_revision']);
        }
        if (is_array($data) && isset($data['same_client_predecessor'])) {
            $response['same_client_predecessor'] = !empty($data['same_client_predecessor']);
        }
        ...
        if ($action === 'delete' && $lesson_id > 0 && get_post($lesson_id) === null) {
            ...
            $response['redirect_url'] = ll_tools_get_vocab_lesson_category_delete_error_redirect_url(
                $wordset_id,
                home_url('/'),
        ...
        wp_send_json_error($response, $status > 0 ? $status : 400);
    }
    ...
    $delete_result_status = (string) ($result['result'] ?? '');
    $redirect_url = in_array($delete_result_status, ['deleted', 'deleting'], true)
        ? ll_tools_get_vocab_lesson_category_delete_redirect_url($wordset_id, $fallback_url, $delete_result_status)
    ...
    wp_send_json_success([
        ...
        'wordset_id' => $wordset_id,
        'category_id' => $category_id,
        'revision' => max(0, (int) ($result['revision'] ?? 0)),
        'result' => (string) ($result['result'] ?? 'saved'),
...
function ll_tools_get_vocab_lesson_image_print_url(int $lesson_id): string {
    $url = ll_tools_get_vocab_lesson_print_url($lesson_id, [
        'auto_print' => true,
    ]);
    if ($url === '') {
        return '';
...
function ll_tools_current_user_can_access_vocab_lesson_image_prints(): bool {
    if (!is_user_logged_in()) {
        return false;
    }
...
function ll_tools_user_can_print_vocab_lesson_images(): bool {
    $allowed = ll_tools_current_user_can_access_vocab_lesson_image_prints();
    ...
    if (is_singular('ll_vocab_lesson')) {
        ...
        if ($lesson_id > 0) {
            $wordset_id = (int) get_post_meta($lesson_id, LL_TOOLS_VOCAB_LESSON_WORDSET_META, true);
            $category_id = (int) get_post_meta($lesson_id, LL_TOOLS_VOCAB_LESSON_CATEGORY_META, true);
            $allowed = $allowed && ll_tools_vocab_lesson_print_view_is_available($wordset_id, $category_id);
        }
    }
...
function ll_tools_get_vocab_lesson_print_posts(int $wordset_id, int $category_id): array {
    ...
    $category = ll_tools_resolve_vocab_lesson_category_for_wordset($category_id, $wordset_id);
    $wordset = get_term($wordset_id, 'wordset');
...
function ll_tools_vocab_lesson_grid_public_cache_lock_option(string $cache_key): string {
    $names = ll_tools_public_ajax_client_lease_option_names('ll_tools_vocab_grid_lock_', $cache_key, 1);
    return $names['value'];
}
...
function ll_tools_vocab_lesson_grid_public_cache_acquire_build_lock(string $cache_key): bool {
    $lease = ll_tools_public_ajax_acquire_client_lease(
        ...
        $cache_key,
        1,
        ll_tools_vocab_lesson_grid_public_cache_build_lock_ttl()
    );
    if (!empty($lease['acquired']) && !empty($lease['option_name'])) {
        $GLOBALS['ll_tools_vocab_lesson_grid_public_cache_leases'][$cache_key] = $lease;
    }
...
function ll_tools_vocab_lesson_grid_public_cache_release_build_lock(string $cache_key): void {
    $lease = $GLOBALS['ll_tools_vocab_lesson_grid_public_cache_leases'][$cache_key] ?? null;
...
function ll_tools_vocab_lesson_grid_public_cache_miss_counter_prefix(string $scope): string {
    return 'll_tools_vocab_grid_miss_' . sanitize_key($scope) . '_';
}
...
function ll_tools_vocab_lesson_grid_public_cache_miss_throttle_key(string $scope, string $identifier): string {
    ...
    $names = ll_tools_public_ajax_counter_option_names(
        ll_tools_vocab_lesson_grid_public_cache_miss_counter_prefix($scope),
        $identifier,
...
function ll_tools_vocab_lesson_grid_public_cache_miss_limited(int $lesson_id): bool {
    ...
    foreach ($checks as $check) {
        ...
        $status = ll_tools_public_ajax_counter_status(
            ll_tools_vocab_lesson_grid_public_cache_miss_counter_prefix((string) $check['scope']),
            $identifier,
            $limit,
            (int) ($config['window'] ?? (5 * MINUTE_IN_SECONDS))
        );
...
function ll_tools_vocab_lesson_grid_public_cache_reserve_miss(int $lesson_id): array {
    if (!ll_tools_vocab_lesson_grid_public_cache_enabled()) {
    ...
    foreach ($targets as $target) {
        ...
        $reservation = ll_tools_public_ajax_reserve_counter(
            ll_tools_vocab_lesson_grid_public_cache_miss_counter_prefix((string) $target['scope']),
            $identifier,
            $limit,
            $window
        );
...
function ll_tools_vocab_lesson_grid_public_cache_record_miss(int $lesson_id): void {
    ...
    ll_tools_vocab_lesson_grid_public_cache_reserve_miss($lesson_id);
}
...
function ll_tools_vocab_lesson_grid_paged_threshold(
    int $lesson_id = 0,
    int $wordset_id = 0,
    int $category_id = 0
): int {
    $threshold = (int) apply_filters(
        'll_tools_vocab_lesson_grid_paged_threshold',
        48,
        max(0, $lesson_id),
        max(0, $wordset_id),
        max(0, $category_id)
    ...
    return max(12, min(500, $threshold));
}

function ll_tools_vocab_lesson_grid_page_size(
    int $lesson_id = 0,
    int $wordset_id = 0,
    int $category_id = 0
): int {
    $page_size = (int) apply_filters(
        'll_tools_vocab_lesson_grid_page_size',
        24,
        max(0, $lesson_id),
        max(0, $wordset_id),
        max(0, $category_id)
...
function ll_tools_vocab_lesson_grid_scan_batch_size(
    int $lesson_id = 0,
    int $wordset_id = 0,
    int $category_id = 0
): int {
    $batch_size = (int) apply_filters(
        'll_tools_vocab_lesson_grid_scan_batch_size',
        128,
        max(0, $lesson_id),
        max(0, $wordset_id),
        max(0, $category_id)
    ...
    return max(24, min(256, $batch_size));
}

function ll_tools_vocab_lesson_grid_materialized_word_limit(
    int $lesson_id = 0,
    int $wordset_id = 0,
    int $category_id = 0
): int {
    $limit = (int) apply_filters(
        'll_tools_vocab_lesson_grid_materialized_word_limit',
        5000,
        max(0, $lesson_id),
        max(0, $wordset_id),
        max(0, $category_id)
    ...
    return max(500, min(20000, $limit));
}

function ll_tools_vocab_lesson_grid_should_use_paged_loading(
    int $lesson_id,
    int $wordset_id,
    $category,
    int $expected_count = 0,
    ?bool $has_prompt_cards = null
): bool {
    $lesson_id = max(0, $lesson_id);
    $wordset_id = max(0, $wordset_id);
    $category_id = ($category instanceof WP_Term && !is_wp_error($category))
        ? (int) $category->term_id
        : max(0, (int) $category);
    if ($lesson_id <= 0 || $wordset_id <= 0 || $category_id <= 0) {
        return false;
    }

    if ($has_prompt_cards === null && function_exists('ll_tools_get_vocab_lesson_prompt_card_summary')) {
        $summary = ll_tools_get_vocab_lesson_prompt_card_summary($wordset_id, $category, 1);
        $has_prompt_cards = !empty($summary['has_cards']);
    }
    if ($has_prompt_cards) {
        return false;
    }

    if ($expected_count <= 0 && function_exists('ll_tools_get_vocab_lesson_category_word_count_targeted')) {
        $count_complete = true;
        $expected_count = ll_tools_get_vocab_lesson_category_word_count_targeted(
            $category,
            $wordset_id,
            $count_complete
        );
    ...
    $should_page = $expected_count > ll_tools_vocab_lesson_grid_paged_threshold(
        $lesson_id,
        $wordset_id,
        $category_id
    );
    ...
    return (bool) apply_filters(
        'll_tools_vocab_lesson_grid_should_use_paged_loading',
        $should_page,
        $lesson_id,
        $wordset_id,
        $category_id,
...
function ll_tools_vocab_lesson_grid_order_cache_key(
    int $lesson_id,
    int $wordset_id,
    int $category_id,
    string $viewer_scope = 'public'
): string {
    $viewer_scope = trim($viewer_scope);
    if ($viewer_scope === '') {
        $viewer_scope = 'public';
    }
...
function ll_tools_vocab_lesson_grid_order_state_get(string $cache_key): ?array {
    $state = wp_cache_get($cache_key, 'll_tools_vocab_lesson');
    if ($state === false) {
...
function ll_tools_vocab_lesson_grid_order_state_set(string $cache_key, array $state): void {
    $ttl = ll_tools_vocab_lesson_grid_public_cache_ttl();
    wp_cache_set($cache_key, $state, 'll_tools_vocab_lesson', $ttl);
    set_transient($cache_key, $state, $ttl);
}

function ll_tools_vocab_lesson_grid_order_lock_option(string $cache_key): string {
    $names = ll_tools_public_ajax_client_lease_option_names('ll_tools_vocab_order_lock_', $cache_key, 1);
    return $names['value'];
}

function ll_tools_vocab_lesson_grid_acquire_order_lock(string $cache_key) {
    $ttl = max(5, min(120, (int) apply_filters('ll_tools_vocab_lesson_grid_order_lock_ttl', 60)));
    $lease = ll_tools_public_ajax_acquire_client_lease(
    ...
    if (empty($lease['acquired']) || empty($lease['lease_value'])) {
        return false;
    }

    $token = (string) $lease['lease_value'];
    $GLOBALS['ll_tools_vocab_lesson_grid_order_leases'][$cache_key][$token] = $lease;
    return $token;
}

function ll_tools_vocab_lesson_grid_release_order_lock(string $cache_key, string $token): void {
    if ($token === '') {
        return;
    }

    $lease = $GLOBALS['ll_tools_vocab_lesson_grid_order_leases'][$cache_key][$token] ?? null;
    unset($GLOBALS['ll_tools_vocab_lesson_grid_order_leases'][$cache_key][$token]);
    if (empty($GLOBALS['ll_tools_vocab_lesson_grid_order_leases'][$cache_key])) {
...
function ll_tools_vocab_lesson_grid_candidate_rows(
    int $wordset_id,
    int $category_id,
    int $after_word_id,
    int $limit,
    ...
    if (
        ...
        return new WP_Error('missing_terms', __('Lesson terms are missing.', 'll-tools-text-domain'));
    }
    ...
    $status_placeholders = $include_drafts ? '%s, %s' : '%s';
    $sql = "
        ...
        INNER JOIN {$wpdb->term_relationships} category_rel
           ...
           AND category_rel.term_taxonomy_id IN ({$category_placeholders})
        INNER JOIN {$wpdb->term_relationships} wordset_rel
            ON wordset_rel.object_id = p.ID
           AND wordset_rel.term_taxonomy_id = %d
        WHERE p.post_type = %s
        ...
        LIMIT %d
    ";
    $params = array_merge($category_tt_ids, [$wordset_tt_id, 'words', 'publish']);
    if ($include_drafts) {
        $params[] = 'draft';
    }
    $params[] = max(0, $after_word_id);
    $params[] = max(1, $limit);
    ...
    if ($wpdb->last_error !== '') {
        return new WP_Error('candidate_query_failed', __('Unable to load this lesson right now.', 'll-tools-text-domain'));
    }

    return array_values(array_filter((array) $rows, static function (array $row): bool {
        return (int) ($row['ID'] ?? 0) > 0;
    }));
}

function ll_tools_vocab_lesson_grid_context(
    int $lesson_id,
    WP_Term $wordset,
    WP_Term $category
): array {
    ...
    try {
        return ll_tools_word_grid_resolve_context([
...
function ll_tools_vocab_lesson_grid_filter_candidate_ids(
    array $candidate_ids,
    int $category_id,
    array $context,
    bool $show_staff_hidden_words = false,
    ?array &$presentation_hidden_reasons = null
) {
    $presentation_hidden_reasons = [];
    $word_ids = array_values(array_unique(array_filter(array_map('intval', $candidate_ids), static function (int $word_id): bool {
        return $word_id > 0;
    })));
    if (empty($word_ids)) {
        return [];
    ...
    if (!empty($word_ids) && empty($context['is_text_based'])) {
        if (function_exists('ll_tools_get_word_effective_image_presence_map')) {
            $image_complete = true;
            $image_map = ll_tools_get_word_effective_image_presence_map($word_ids, $image_complete);
            if (!$image_complete) {
                return new WP_Error('image_filter_failed', __('Unable to load this lesson right now.', 'll-tools-text-domain'));
            }
            if ($show_staff_hidden_words) {
                foreach ($word_ids as $word_id) {
                    if (empty($image_map[$word_id]) && get_post_status($word_id) === 'publish') {
                        $presentation_hidden_reasons[$word_id] = 'missing_image';
                    }
                }
            } else {
                $word_ids = array_values(array_filter($word_ids, static function (int $word_id) use ($image_map): bool {
                    return !empty($image_map[$word_id]);
                }));
            }
        } elseif (function_exists('ll_tools_filter_word_ids_with_effective_images')) {
            $visible_word_ids = ll_tools_filter_word_ids_with_effective_images($word_ids, true);
            if ($show_staff_hidden_words) {
                $visible_lookup = array_fill_keys($visible_word_ids, true);
                foreach ($word_ids as $word_id) {
                    if (!isset($visible_lookup[$word_id]) && get_post_status($word_id) === 'publish') {
                        $presentation_hidden_reasons[$word_id] = 'missing_image';
                    }
                }
            } else {
                $word_ids = $visible_word_ids;
            }
        }
    }

    if (!empty($word_ids) && !empty($context['category_quiz_requires_audio'])) {
        if (function_exists('ll_tools_get_word_audio_presence_map')) {
            $audio_complete = true;
            $audio_map = ll_tools_get_word_audio_presence_map($word_ids, $audio_complete);
            if (!$audio_complete) {
                return new WP_Error('audio_filter_failed', __('Unable to load this lesson right now.', 'll-tools-text-domain'));
            }
            if ($show_staff_hidden_words) {
                foreach ($word_ids as $word_id) {
                    if (empty($audio_map[$word_id]) && get_post_status($word_id) === 'publish') {
                        $presentation_hidden_reasons[$word_id] = 'missing_audio';
                    }
                }
            } else {
                $word_ids = array_values(array_filter($word_ids, static function (int $word_id) use ($audio_map): bool {
                    return !empty($audio_map[$word_id]);
                }));
            }
        } elseif (function_exists('ll_tools_word_has_published_audio')) {
            if ($show_staff_hidden_words) {
                foreach ($word_ids as $word_id) {
                    if (!ll_tools_word_has_published_audio($word_id) && get_post_status($word_id) === 'publish') {
                        $presentation_hidden_reasons[$word_id] = 'missing_audio';
                    }
                }
            } else {
...
function ll_tools_vocab_lesson_grid_finalize_order_state(
    array $state,
    int $lesson_id,
    int $wordset_id,
    int $category_id,
    ...
    $rows = isset($state['rows']) && is_array($state['rows']) ? array_values($state['rows']) : [];
    usort($rows, static function (array $left, array $right): int {
        $date_comparison = strcmp((string) ($left['d'] ?? ''), (string) ($right['d'] ?? ''));
    ...
    foreach ($rows as $row) {
        ...
        $posts[] = new WP_Post((object) [
            'ID' => $word_id,
            'post_title' => (string) ($row['t'] ?? ''),
            'post_date' => (string) ($row['d'] ?? ''),
        ]);
        $display_values_cache[$word_id] = [
            'word_text' => (string) ($row['w'] ?? ''),
            'translation_text' => (string) ($row['x'] ?? ''),
            'store_in_title' => !empty($row['s']),
        ];
        $image_id_cache[$word_id] = (int) ($row['i'] ?? 0);
    }
    ...
    $manual_word_order = function_exists('ll_tools_get_vocab_lesson_manual_word_order')
        ...
        : [];
    $manual_order_applied = !empty($manual_word_order);
    if ($manual_order_applied && function_exists('ll_tools_reorder_posts_by_word_id_order')) {
        $posts = ll_tools_reorder_posts_by_word_id_order($posts, $manual_word_order);
    }
    ...
    if (!$manual_order_applied && function_exists('ll_tools_word_grid_group_same_name_or_image')) {
        $posts = ll_tools_word_grid_group_same_name_or_image($posts, $display_values_cache, $image_id_cache);
    }
    if (
        !$manual_order_applied
        && function_exists('ll_tools_word_grid_should_sort_visible_titles')
        && ll_tools_word_grid_should_sort_visible_titles($context)
        && function_exists('ll_tools_word_grid_sort_posts_by_display_title')
    ) {
        $posts = ll_tools_word_grid_sort_posts_by_display_title($posts, $display_values_cache);
    }
    $presentation_hidden_lookup = [];
    foreach ($rows as $row) {
        $word_id = (int) ($row['id'] ?? 0);
        if ($word_id > 0 && !empty($row['h'])) {
            $presentation_hidden_lookup[$word_id] = true;
        }
    }
    if (!empty($presentation_hidden_lookup) && function_exists('ll_tools_word_grid_move_lookup_posts_to_end')) {
        $posts = ll_tools_word_grid_move_lookup_posts_to_end($posts, $presentation_hidden_lookup);
    }
    ...
    $state['complete'] = true;
    $state['rows'] = [];
    $state['ordered_ids'] = $ordered_word_ids;
    $state['total'] = count($ordered_word_ids);
    $state['completed_at'] = time();

    return $state;
}

function ll_tools_vocab_lesson_grid_advance_order_state(
    string $cache_key,
    int $lesson_id,
    WP_Term $wordset,
    WP_Term $category
) {
    $state = ll_tools_vocab_lesson_grid_order_state_get($cache_key);
    if (is_array($state) && !empty($state['complete'])) {
        return $state;
    }

    $lock_token = ll_tools_vocab_lesson_grid_acquire_order_lock($cache_key);
    ...
    try {
        ...
        $context = ll_tools_vocab_lesson_grid_context(
            $lesson_id,
            $wordset,
            $category
        );
        if (!empty($context['access_denied'])) {
            return new WP_Error('access_denied', __('Lesson not found.', 'll-tools-text-domain'));
        }
        $show_staff_hidden_words = function_exists('ll_tools_word_grid_should_show_staff_hidden_words')
            && ll_tools_word_grid_should_show_staff_hidden_words($context);

        $batch_size = ll_tools_vocab_lesson_grid_scan_batch_size(
            $lesson_id,
            (int) $wordset->term_id,
            (int) $category->term_id
        );
        $candidate_rows = ll_tools_vocab_lesson_grid_candidate_rows(
            (int) $wordset->term_id,
            (int) $category->term_id,
            max(0, (int) ($state['cursor_id'] ?? 0)),
            $batch_size,
            $show_staff_hidden_words
        );
        if (is_wp_error($candidate_rows)) {
            return $candidate_rows;
        }

        $candidate_ids = array_values(array_filter(array_map(static function (array $row): int {
            return (int) ($row['ID'] ?? 0);
        }, $candidate_rows)));
        $presentation_hidden_reasons = [];
        $visible_word_ids = ll_tools_vocab_lesson_grid_filter_candidate_ids(
            $candidate_ids,
            (int) $category->term_id,
            $context,
            $show_staff_hidden_words,
        ...
        if (is_wp_error($visible_word_ids)) {
            return $visible_word_ids;
        }
        ...
        foreach ($candidate_rows as $candidate_row) {
            ...
            if ($candidate_id > 0) {
                $row_by_id[$candidate_id] = $candidate_row;
            }
        }
        if (!empty($visible_word_ids)) {
            update_meta_cache('post', $visible_word_ids);
        }
        $display_values = function_exists('ll_tools_word_grid_get_category_display_text_map')
            ? ll_tools_word_grid_get_category_display_text_map($visible_word_ids, $context)
            : [];
        $needs_image_identity = !function_exists('ll_tools_word_grid_should_sort_visible_titles')
            || !ll_tools_word_grid_should_sort_visible_titles($context);
        foreach ($visible_word_ids as $word_id) {
            $word_id = (int) $word_id;
            if ($word_id <= 0 || !isset($row_by_id[$word_id])) {
                continue;
            }
            if (!isset($display_values[$word_id]) || !is_array($display_values[$word_id])) {
                $display_values[$word_id] = function_exists('ll_tools_word_grid_resolve_display_text')
                    ? ll_tools_word_grid_resolve_display_text($word_id)
                    : [
                        'word_text' => (string) ($row_by_id[$word_id]['post_title'] ?? ''),
                        'translation_text' => '',
                        'store_in_title' => true,
                    ];
            }
            $display = $display_values[$word_id];
            $image_id = 0;
            if ($needs_image_identity) {
            ...
            $state['rows'][] = [
                'id' => $word_id,
                't' => (string) ($row_by_id[$word_id]['post_title'] ?? ''),
                'd' => (string) ($row_by_id[$word_id]['post_date'] ?? ''),
                'w' => (string) ($display['word_text'] ?? ''),
                'x' => (string) ($display['translation_text'] ?? ''),
                's' => !empty($display['store_in_title']),
                'i' => $image_id,
                'h' => isset($presentation_hidden_reasons[$word_id])
                    ? (string) $presentation_hidden_reasons[$word_id]
                    : '',
            ];
        }

        $state['scanned'] = max(0, (int) ($state['scanned'] ?? 0)) + count($candidate_rows);
        if (!empty($candidate_ids)) {
            $state['cursor_id'] = max($candidate_ids);
        }

        $materialized_limit = ll_tools_vocab_lesson_grid_materialized_word_limit(
            $lesson_id,
            (int) $wordset->term_id,
            (int) $category->term_id
        );
        if (count((array) ($state['rows'] ?? [])) > $materialized_limit) {
            return new WP_Error(
                'lesson_grid_limit_exceeded',
                __('Unable to load this lesson right now.', 'll-tools-text-domain')
            );
        }

        if (count($candidate_rows) < $batch_size) {
            $state = ll_tools_vocab_lesson_grid_finalize_order_state(
                $state,
                $lesson_id,
                (int) $wordset->term_id,
                (int) $category->term_id,
        ...
        ll_tools_vocab_lesson_grid_order_state_set($cache_key, $state);

        return $state;
    } finally {
        ll_tools_vocab_lesson_grid_release_order_lock($cache_key, $lock_token);
    }
}

function ll_tools_vocab_lesson_grid_cursor(
    int $lesson_id,
    string $order_cache_key,
    int $offset
): string {
    $offset = max(0, $offset);
    $message = $lesson_id . '|' . $order_cache_key . '|' . $offset;
    $signature = hash_hmac('sha256', $message, wp_salt('nonce'));

    return $offset . '.' . $signature;
}

function ll_tools_vocab_lesson_grid_cursor_offset(
    string $cursor,
    int $lesson_id,
    string $order_cache_key
) {
    ...
    if (strlen($cursor) > 96 || strpos($cursor, '.') === false) {
        return new WP_Error('invalid_cursor', __('Invalid request.', 'll-tools-text-domain'));
    }

    [$offset_raw, $signature] = array_pad(explode('.', $cursor, 2), 2, '');
    if (!ctype_digit($offset_raw) || strlen($signature) !== 64 || !ctype_xdigit($signature)) {
        return new WP_Error('invalid_cursor', __('Invalid request.', 'll-tools-text-domain'));
    }
    $offset = (int) $offset_raw;
    $expected = ll_tools_vocab_lesson_grid_cursor($lesson_id, $order_cache_key, $offset);
    [, $expected_signature] = explode('.', $expected, 2);
...
function ll_tools_vocab_lesson_grid_page_cache_key(
    string $order_cache_key,
    int $offset,
    int $page_size
): string {
    return 'll_vl_grid_page_' . md5($order_cache_key . '|' . max(0, $offset) . '|' . max(1, $page_size));
}

function ll_tools_vocab_lesson_grid_page_cache_get(string $cache_key): ?array {
    $response = wp_cache_get($cache_key, 'll_tools_vocab_lesson');
    if ($response === false) {
...
function ll_tools_vocab_lesson_grid_page_cache_set(string $cache_key, array $response): void {
    $ttl = ll_tools_vocab_lesson_grid_public_cache_ttl();
    wp_cache_set($cache_key, $response, 'll_tools_vocab_lesson', $ttl);
    set_transient($cache_key, $response, $ttl);
}

function ll_tools_vocab_lesson_grid_paged_response(
    int $lesson_id,
    WP_Term $wordset,
    WP_Term $category,
    int $expected_count,
    ...
    $can_edit_words = function_exists('ll_tools_user_can_edit_vocab_words')
        && ll_tools_user_can_edit_vocab_words((int) $wordset->term_id);
    $viewer_scope = $can_edit_words
        ? 'staff:' . max(0, get_current_user_id())
        : 'public';
    $order_cache_key = ll_tools_vocab_lesson_grid_order_cache_key(
        $lesson_id,
        (int) $wordset->term_id,
        (int) $category->term_id,
        $viewer_scope
    );
    $offset = ll_tools_vocab_lesson_grid_cursor_offset($cursor, $lesson_id, $order_cache_key);
    if (is_wp_error($offset)) {
        return $offset;
    }

    $state = ll_tools_vocab_lesson_grid_order_state_get($order_cache_key);
    if (!is_array($state) || empty($state['complete'])) {
        $state_result = ll_tools_vocab_lesson_grid_advance_order_state(
            $order_cache_key,
            $lesson_id,
            $wordset,
            $category
        ...
        if (!empty($state_result['preparing'])) {
            $state = isset($state_result['state']) && is_array($state_result['state'])
                ? $state_result['state']
                : [];
        } else {
            $state = $state_result;
    ...
    if (empty($state['complete'])) {
        ll_tools_vocab_lesson_grid_public_cache_send_header('PREPARING');
        return [
            'paged' => true,
            'preparing' => true,
            'html' => '',
            'has_more' => true,
            'next_cursor' => $cursor,
            'loaded' => max(0, (int) $offset),
            'total' => max(0, $expected_count),
            'scanned' => max(0, (int) ($state['scanned'] ?? 0)),
            'retry_after_ms' => 120,
        ];
    }

    $ordered_word_ids = isset($state['ordered_ids']) && is_array($state['ordered_ids'])
        ? array_values(array_filter(array_map('intval', $state['ordered_ids'])))
        : [];
    $total = count($ordered_word_ids);
    $offset = max(0, (int) $offset);
    if ($offset > $total) {
        return new WP_Error('invalid_cursor', __('Invalid request.', 'll-tools-text-domain'));
    }

    $page_size = ll_tools_vocab_lesson_grid_page_size(
        $lesson_id,
        (int) $wordset->term_id,
        (int) $category->term_id
    );
    $page_cache_key = ll_tools_vocab_lesson_grid_page_cache_key($order_cache_key, $offset, $page_size);
    if (ll_tools_vocab_lesson_grid_public_cache_enabled()) {
        $cached_response = ll_tools_vocab_lesson_grid_page_cache_get($page_cache_key);
        if (is_array($cached_response)) {
            ll_tools_vocab_lesson_grid_public_cache_send_header('PAGE_HIT');
            return $cached_response;
        }
    }

    $page_word_ids = array_slice($ordered_word_ids, $offset, $page_size);
    if (empty($page_word_ids)) {
        $html = sprintf(
            '<div class="word-grid ll-word-grid ll-vocab-lesson-grid-empty" data-ll-word-grid data-ll-wordset-id="%d" data-ll-category-id="%d"></div>',
            (int) $wordset->term_id,
            (int) $category->term_id
        );
    } else {
    ...
    $loaded = min($total, $offset + count($page_word_ids));
    ...
    $response = [
        ...
        'next_cursor' => $has_more
            ? ll_tools_vocab_lesson_grid_cursor($lesson_id, $order_cache_key, $loaded)
            : '',
        'loaded' => $loaded,
        'total' => $total,
        'scanned' => max(0, (int) ($state['scanned'] ?? 0)),
    ...
    if (ll_tools_vocab_lesson_grid_public_cache_enabled()) {
        ll_tools_vocab_lesson_grid_page_cache_set($page_cache_key, $response);
    }
    ll_tools_vocab_lesson_grid_public_cache_send_header('PAGE_MISS');
...
function ll_tools_get_vocab_lesson_grid_handler() {
    ...
    $prompt_card_summary = function_exists('ll_tools_get_vocab_lesson_prompt_card_summary')
        ? ll_tools_get_vocab_lesson_prompt_card_summary($wordset_id, $category, 1)
        : ['has_cards' => false];
    $count_complete = true;
    $expected_count = function_exists('ll_tools_get_vocab_lesson_category_word_count_targeted')
        ? ll_tools_get_vocab_lesson_category_word_count_targeted($category, $wordset_id, $count_complete)
        : 0;
    if (!$count_complete) {
        $retry_after = max(1, min(60, (int) apply_filters(
            'll_tools_vocab_lesson_grid_source_incomplete_retry_after',
            5,
            $lesson_id,
            $wordset_id,
            $category_id
        ...
        wp_send_json_error([
            ...
            'message' => __('Unable to load this lesson right now.', 'll-tools-text-domain'),
            'retryable' => true,
    ...
    if (
        ...
        $category_content_summary = ll_tools_wordset_page_get_category_content_summary($category_id, $wordset_id);
        $expected_count = max($expected_count, max(0, (int) ($category_content_summary['total'] ?? 0)));
    }
    $uses_paged_loading = $count_complete && ll_tools_vocab_lesson_grid_should_use_paged_loading(
        $lesson_id,
        $wordset_id,
        $category,
        $expected_count,
        !empty($prompt_card_summary['has_cards'])
    );
    if ($uses_paged_loading) {
        $cursor = isset($_POST['cursor'])
            ? sanitize_text_field(wp_unslash((string) $_POST['cursor']))
            : '';
        $paged_response = ll_tools_vocab_lesson_grid_paged_response(
            $lesson_id,
            $wordset,
            $category,
            $expected_count,
        ...
        if (is_wp_error($paged_response)) {
            $error_code = (string) $paged_response->get_error_code();
            $status = $error_code === 'invalid_cursor' ? 400 : 500;
            wp_send_json_error([
                'code' => $error_code,
                'message' => $paged_response->get_error_message(),
            ], $status);
        }
        wp_send_json_success($paged_response);
    }
    ...
    $cache_miss_reservation = $public_cache_enabled
        ? ll_tools_vocab_lesson_grid_public_cache_reserve_miss($lesson_id)
    ...
    try {
        ...
        $html = ll_tools_render_vocab_lesson_prompt_cards_grid(
            $wordset_id,
            $category,
            $lesson_id,
            $render_complete
        ...
        if ($public_cache_enabled && $render_complete) {
    ...
    ll_tools_vocab_lesson_grid_public_cache_send_header(
        ($public_cache_enabled && $render_complete) ? 'MISS' : 'BYPASS'
    );
    wp_send_json_success([
        'html' => is_string($html) ? $html : '',
    ]);
...
function ll_tools_render_vocab_lesson_trash_notice(): void {
    if (!is_admin() || !current_user_can('view_ll_tools')) {
        return;
    }
    if (function_exists('wp_doing_ajax') && wp_doing_ajax()) {
    ...
    $restore_status = isset($_GET['ll_vocab_lesson_restore'])
        ? sanitize_key(wp_unslash((string) $_GET['ll_vocab_lesson_restore']))
        : '';
    $restored_count = isset($_GET['ll_vocab_lesson_restored'])
        ? max(0, (int) wp_unslash((string) $_GET['ll_vocab_lesson_restored']))
        : 0;
    $failed_count = isset($_GET['ll_vocab_lesson_restore_failed'])
        ? max(0, (int) wp_unslash((string) $_GET['ll_vocab_lesson_restore_failed']))
        : 0;

    if ($restore_status !== '') {
        $notice_class = 'notice notice-success is-dismissible';
        if ($restore_status === 'partial' || $restore_status === 'error') {
            $notice_class = 'notice notice-warning is-dismissible';
        } elseif ($restore_status === 'none') {
            $notice_class = 'notice notice-info is-dismissible';
        }

        if ($restore_status === 'ok') {
            $message = sprintf(
                _n(
                    '%d vocab lesson page was restored.',
                    '%d vocab lesson pages were restored.',
                    $restored_count,
                    'll-tools-text-domain'
                ),
                $restored_count
            );
        } elseif ($restore_status === 'partial') {
            $message = sprintf(
                /* translators: 1: restored lesson page count, 2: failed lesson page count */
                __('Restored %1$d vocab lesson pages. %2$d could not be restored.', 'll-tools-text-domain'),
                $restored_count,
                $failed_count
            );
        } elseif ($restore_status === 'none') {
            $message = __('No trashed vocab lesson pages were waiting to be restored.', 'll-tools-text-domain');
        } else {
            $message = __('Vocab lesson pages could not be restored right now.', 'll-tools-text-domain');
        }
    ...
    $trashed_post_ids = ll_tools_get_trashed_vocab_lesson_notice_post_ids();
...
add_action('admin_notices', 'll_tools_render_vocab_lesson_trash_notice', 5);

function ll_tools_handle_restore_vocab_lessons_request(): void {
    if (!current_user_can('view_ll_tools')) {
        wp_die(esc_html__('You do not have permission to restore vocab lesson pages.', 'll-tools-text-domain'), 403);
    }

    check_admin_referer('ll_tools_restore_vocab_lessons');
    ...
    foreach ($trashed_post_ids as $post_id) {
        ...
        if ($category_id > 0 && $wordset_id > 0) {
            ...
            $restored = !is_wp_error($result) && !empty($result['post_id']);
        }

        if (!$restored) {
            $restored = (bool) wp_untrash_post($post_id);
        }
    ...
    ll_tools_get_trashed_vocab_lesson_notice_post_ids();
    ...
    wp_safe_redirect(add_query_arg([
        'll_vocab_lesson_restore' => $status,
        'll_vocab_lesson_restored' => $restored_count,
        'll_vocab_lesson_restore_failed' => $failed_count,
    ], $redirect_to));
    exit;
}
