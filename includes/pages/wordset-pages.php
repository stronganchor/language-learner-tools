...
function ll_tools_wordset_page_get_cached_payload(string $cache_key, array &$request_cache, string $cache_group = 'll_tools') {
...
function ll_tools_wordset_page_current_user_can_preview_inactive_categories(int $wordset_id): bool {
    ...
    if (function_exists('ll_tools_current_user_can_manage_wordset_content')) {
        return (bool) ll_tools_current_user_can_manage_wordset_content($wordset_id);
    }
...
function ll_tools_wordset_page_get_category_word_ids(int $category_id, int $wordset_id): array {
    ...
    return array_values(array_filter(array_map('intval', (array) $ids), static function (int $word_id): bool {
        return $word_id > 0;
    }));
}

function ll_tools_wordset_page_uncategorized_virtual_category_id(): int {
    return 2147483001;
}

function ll_tools_wordset_page_is_uncategorized_virtual_category(array $category): bool {
    return !empty($category['is_virtual_category']) && (string) ($category['virtual_category_type'] ?? '') === 'uncategorized';
}
...
function ll_tools_wordset_page_get_uncategorized_editor_url(int $wordset_id): string {
    ...
    $url = function_exists('ll_tools_get_wordset_settings_tool_url')
        ? ll_tools_get_wordset_settings_tool_url($wordset_term, 'editor')
...
function ll_tools_wordset_page_build_uncategorized_virtual_category(int $wordset_id, int $preview_limit = 2): ?array {
    if (!ll_tools_wordset_page_current_user_can_preview_inactive_categories($wordset_id)) {
        return null;
    }
    ...
    $search_text_parts = [
        $uncategorized_label,
        __('No category', 'll-tools-text-domain'),
    ];
    foreach ($word_ids as $word_id) {
        $label = trim((string) get_the_title($word_id));
        if ($label !== '') {
            $search_text_parts[] = $label;
        }
        ...
        if ($translation !== '') {
            $search_text_parts[] = $translation;
        }
    }
    foreach (array_slice($word_ids, 0, $preview_limit) as $word_id) {
        $label = trim((string) get_the_title($word_id));
        if ($label === '') {
            $label = sprintf(__('Word #%d', 'll-tools-text-domain'), $word_id);
        }
        $preview[] = [
            'type' => 'text',
            'label' => $label,
    ...
    return [
        ...
        'slug' => 'uncategorized',
        'raw_name' => $uncategorized_label,
        'name' => $uncategorized_label,
        'translation' => $uncategorized_label,
        'mode' => 'text_title',
        ...
        'preview_requires_images' => false,
        'url' => ll_tools_wordset_page_get_uncategorized_editor_url($wordset_id),
        'is_public' => false,
        'public_note_label' => __('Manager view', 'll-tools-text-domain'),
        ...
        'can_preview' => false,
        'can_delete' => false,
        'delete_reason' => __('The uncategorized lesson is a system view and cannot be deleted.', 'll-tools-text-domain'),
...
function ll_tools_wordset_page_get_staff_category_terms(int $wordset_id, array $counts = []): array {
    $wordset_id = (int) $wordset_id;
    if ($wordset_id <= 0) {
        return [];
    }
    ...
    $terms_by_id = [];
    $add_term = static function ($term) use (&$terms_by_id, $wordset_id): void {
        ...
        if (
            ...
            && (int) ll_tools_get_category_wordset_owner_id($term) > 0
    ...
    $owned_terms = function_exists('ll_tools_wordset_page_get_owned_category_terms')
        ? ll_tools_wordset_page_get_owned_category_terms($wordset_id)
        : [];
    $owned_category_ids = [];
    foreach ((array) $owned_terms as $owned_term) {
        if ($owned_term instanceof WP_Term && !is_wp_error($owned_term)) {
            $owned_category_id = (int) $owned_term->term_id;
            $candidate_ids[] = $owned_category_id;
            $owned_category_ids[] = $owned_category_id;
        }
    }
 ...
 * @return int[]
 */
function ll_tools_wordset_page_get_category_word_image_ids(int $category_id, int $wordset_id): array {
    $category_id = (int) $category_id;
    $wordset_id = (int) $wordset_id;
    if ($category_id <= 0 || $wordset_id <= 0) {
        return [];
    ...
    return array_values(array_filter(array_map('intval', (array) get_posts($query_args)), static function (int $image_id): bool {
        return $image_id > 0;
    }));
}
 ...
 * @return int[]
 */
function ll_tools_wordset_page_get_category_prompt_card_ids(int $category_id, int $wordset_id): array {
    $category_id = (int) $category_id;
    $wordset_id = (int) $wordset_id;
    if ($category_id <= 0 || $wordset_id <= 0) {
        return [];
    ...
    return array_values(array_filter(array_map('intval', (array) $ids), static function (int $prompt_card_id): bool {
...
function ll_tools_wordset_page_category_delete_blocker(WP_Term $category, int $wordset_id, array $summary = []): string {
    ...
    if ($category_id <= 0 || $wordset_id <= 0) {
        return __('Choose a valid category.', 'll-tools-text-domain');
    }
    ...
    if (!is_wp_error($child_terms) && !empty($child_terms)) {
        return __('Delete child categories first.', 'll-tools-text-domain');
    }

    if (empty($summary)) {
        $summary = ll_tools_wordset_page_get_category_content_summary($category_id, $wordset_id);
    }
    if (max(0, (int) ($summary['prompt_card_count'] ?? 0)) > 0) {
        return __('Remove prompt cards in this category first.', 'll-tools-text-domain');
    }
 ...
 * @return int|WP_Error
 */
function ll_tools_wordset_page_detach_category_from_words(int $category_id, int $wordset_id) {
    $category_id = (int) $category_id;
    $wordset_id = (int) $wordset_id;
    if ($category_id <= 0 || $wordset_id <= 0) {
    ...
    $word_ids = ll_tools_wordset_page_get_category_word_ids($category_id, $wordset_id);
    if (empty($word_ids)) {
        return 0;
    }
    ...
    $detached = 0;
    foreach ($word_ids as $word_id) {
        $category_ids = wp_get_object_terms((int) $word_id, 'word-category', ['fields' => 'ids']);
        if (is_wp_error($category_ids)) {
            return new WP_Error('category_delete', __('Unable to remove this category from its words right now.', 'll-tools-text-domain'));
        }

        $remaining_category_ids = array_values(array_filter(array_map('intval', (array) $category_ids), static function (int $existing_category_id) use ($category_id): bool {
            return $existing_category_id > 0 && $existing_category_id !== $category_id;
        }));

        $updated = wp_set_object_terms((int) $word_id, $remaining_category_ids, 'word-category', false);
...
/**
 * @return array{deleted_lesson_count:int,detached_word_count:int}|WP_Error
 */
function ll_tools_wordset_page_delete_category_for_wordset(int $category_id, int $wordset_id, string $blocked_error_code = 'category_delete') {
    $category_id = (int) $category_id;
    $wordset_id = (int) $wordset_id;
    ...
    $lesson_delete_result = ll_tools_wordset_page_delete_vocab_lessons_for_category($category_id, $wordset_id);
    if (is_wp_error($lesson_delete_result)) {
        return $lesson_delete_result;
    }

    $detached_word_count = ll_tools_wordset_page_detach_category_from_words($category_id, $wordset_id);
    if (is_wp_error($detached_word_count)) {
        return $detached_word_count;
    }
    ...
    return [
        'deleted_lesson_count' => (int) $lesson_delete_result,
        'detached_word_count' => (int) $detached_word_count,
    ];
}

/**
 * @return array{can_manage:bool,can_hide:bool,can_preview:bool,can_delete:bool,delete_reason:string,nonce:string,action_url:string}
 */
function ll_tools_wordset_page_get_inactive_category_action_state(WP_Term $category, int $wordset_id, array $summary = []): array {
    ...
    if ($wordset_term instanceof WP_Term && !is_wp_error($wordset_term) && function_exists('ll_tools_get_wordset_page_view_url')) {
        $action_url = ll_tools_get_wordset_page_view_url($wordset_term);
    }
    if ($action_url === '') {
        $action_url = function_exists('ll_tools_wordset_page_current_url') ? ll_tools_wordset_page_current_url() : home_url('/');
    }
...
function ll_tools_wordset_page_prepare_word_images_for_lesson_preview(int $category_id, int $wordset_id) {
    ...
    $wordset_id = (int) $wordset_id;
    if ($category_id <= 0 || $wordset_id <= 0) {
        return new WP_Error('invalid_preview_context', __('Unable to prepare this category preview.', 'll-tools-text-domain'));
    }
    if (!function_exists('ll_find_or_create_word_for_image')) {
...
function ll_tools_wordset_page_handle_inactive_category_action(): void {
    ...
    $redirect_error = static function (string $error, string $message = '') use ($base_redirect): void {
        $args = [
            ...
            'll_wordset_inactive_category_error' => $error,
        ];
        if ($message !== '') {
            $args['ll_wordset_inactive_category_message'] = $message;
        }
        wp_safe_redirect(add_query_arg($args, $base_redirect));
        exit;
    };
    $redirect_success = static function (string $result) use ($base_redirect): void {
...
function ll_tools_wordset_page_get_inactive_category_public_note(WP_Term $category, int $wordset_id, int $quiz_ready_count, int $min_words, array $counts): string {
    ...
    if ((string) $category->slug === 'uncategorized') {
        return __('Reserved category.', 'll-tools-text-domain');
    }
    ...
    $unpublished_words = max(0, (int) ($status_summary['draft'] ?? 0))
        ...
        + max(0, (int) ($status_summary['future'] ?? 0))
        + max(0, (int) ($status_summary['private'] ?? 0));

    if (max(0, (int) ($status_summary['total'] ?? 0)) <= 0) {
        $content_summary = ll_tools_wordset_page_get_category_content_summary($category_id, $wordset_id);
        if (max(0, (int) ($content_summary['word_image_count'] ?? 0)) > 0) {
            return __('Needs word records.', 'll-tools-text-domain');
        }
        if (max(0, (int) ($content_summary['prompt_card_count'] ?? 0)) > 0) {
            return __('Needs published prompt cards.', 'll-tools-text-domain');
        }
        return __('No words yet.', 'll-tools-text-domain');
    }

    if ($requires_audio) {
        if ($published_words <= 0 && max(0, (int) ($status_summary['unpublished_audio'] ?? 0)) > 0) {
            return __('Audio is not published yet.', 'll-tools-text-domain');
        }
        if (
            $published_words <= 0
            && $unpublished_words > 0
            && max(0, (int) ($status_summary['missing_audio'] ?? 0)) > 0
        ) {
            return __('Needs audio.', 'll-tools-text-domain');
        }
        if (
            $published_words > 0
            && max(0, (int) ($status_summary['published_audio'] ?? 0)) < min($published_words, max(1, $min_words))
            && $quiz_ready_count < $min_words
        ) {
            return __('Needs published audio.', 'll-tools-text-domain');
        }
    }

    if ($published_words <= 0 && $unpublished_words > 0) {
        return __('No published words yet.', 'll-tools-text-domain');
    }

    if ($requires_images) {
        $all_count = max(0, (int) ($counts['all'][$category_id] ?? 0));
        $image_count = max(0, (int) ($counts['with_images'][$category_id] ?? 0));
        if ($all_count >= $min_words && $image_count < $min_words) {
            return __('Needs images.', 'll-tools-text-domain');
        }
    }

    if ($quiz_ready_count < $min_words) {
        return __('Needs more quiz-ready words.', 'll-tools-text-domain');
    }

    return __('Not public yet.', 'll-tools-text-domain');
}

function ll_tools_get_wordset_page_category_rows(int $wordset_id, int $preview_limit = 2, bool $include_inactive = false): array {
    static $request_cache = [];
    ...
    $cache_key = ll_tools_wordset_page_build_cache_key('category_rows', [
        'wordset_id' => $wordset_id,
        'min_words' => $min_words,
        'preview_limit' => $preview_limit_sig,
    ...
    if (empty($eligible_ids) && !$include_inactive) {
    ...
    if ($include_inactive) {
        ...
        foreach (ll_tools_wordset_page_get_staff_category_terms($wordset_id, $counts) as $term) {
            ...
            $rows[] = [
                ...
                'can_manage_inactive' => !empty($action_state['can_manage']),
                'can_hide' => !empty($action_state['can_hide']),
                'can_preview' => !empty($action_state['can_preview']),
                'can_delete' => !empty($action_state['can_delete']),
                'delete_reason' => (string) ($action_state['delete_reason'] ?? ''),
                'inactive_action_nonce' => (string) ($action_state['nonce'] ?? ''),
                'inactive_action_url' => (string) ($action_state['action_url'] ?? ''),
            ];
            $row_lookup_ids[$term_id] = true;
        }
    }
...
function ll_tools_get_wordset_category_preview(int $wordset_id, int $category_id, int $limit = 2, ?bool $requires_images = null): array {
    ...
    if ($requires_images === null && function_exists('ll_tools_vocab_lesson_category_requires_images')) {
        $use_images = ll_tools_vocab_lesson_category_requires_images($category_id, $wordset_id);
    ...
    $wordset_epoch = function_exists('ll_tools_get_wordset_cache_epoch')
        ? max(1, (int) ll_tools_get_wordset_cache_epoch())
        : 1;
    $preview_cache_key = ll_tools_wordset_page_build_cache_key('category_preview', [
        'wordset_id' => $wordset_id,
        'category_id' => $category_id,
        'limit' => $limit,
        'query_limit' => $query_limit,
        'use_images' => $use_images ? 1 : 0,
    ...
    $cache_ttl = ll_tools_wordset_page_normalize_cache_ttl('ll_tools_wordset_page_preview_cache_ttl', 30 * MINUTE_IN_SECONDS);
    $preview_aspect_ratio = '';
    $ratio_cache_key = '';
    if ($use_images) {
    ...
    if ($prompt_card_option_preview) {
        ...
        foreach ($preview_pairs as $pair) {
            if (!is_array($pair) || count($items) >= $limit) {
                break;
            }
    ...
    return ll_tools_wordset_page_store_cached_payload($preview_cache_key, [
        'items' => $items,
...
function ll_tools_get_wordset_page_categories(int $wordset_id, int $preview_limit = 2, array $args = []): array {
    ...
    $uncategorized_virtual_category = ll_tools_wordset_page_build_uncategorized_virtual_category($wordset_id, $preview_limit);
    ...
    if (!empty($rows)) {
        ...
        foreach ($lesson_posts as $lesson_id) {
            if (function_exists('ll_tools_vocab_lesson_is_preview_only') && ll_tools_vocab_lesson_is_preview_only((int) $lesson_id)) {
                continue;
            }
    ...
    foreach ($rows as $row) {
        ...
        $public_note = isset($row['public_note']) ? trim((string) $row['public_note']) : '';
        if ($lesson_post_id <= 0 && $is_public) {
            continue;
        }
        ...
        $item = [
            ...
            'preview_deferred' => $defer_previews,
            'preview_requires_images' => $requires_images,
            'url'        => ($is_public && $lesson_post_id > 0) ? get_permalink($lesson_post_id) : '',
            'is_public'  => $is_public,
            'public_note' => $is_public ? '' : ($public_note !== '' ? $public_note : __('Not public yet.', 'll-tools-text-domain')),
            'word_image_count' => max(0, (int) ($row['word_image_count'] ?? 0)),
            'prompt_card_count' => max(0, (int) ($row['prompt_card_count'] ?? 0)),
            'content_count' => max(0, (int) ($row['content_count'] ?? 0)),
            'can_manage_inactive' => !empty($row['can_manage_inactive']),
            'can_hide' => !empty($row['can_hide']),
            'can_preview' => !empty($row['can_preview']),
    ...
    if (is_array($uncategorized_virtual_category)) {
        $items[] = $uncategorized_virtual_category;
    }
...
function ll_tools_wordset_page_get_lazy_card_batch_size(): int {
    $batch_size = (int) apply_filters('ll_tools_wordset_page_lazy_card_batch_size', 18);
    if ($batch_size < 6) {
        $batch_size = 6;
    }
    if ($batch_size > 48) {
...
function ll_tools_wordset_page_lazy_cards_cache_key(string $token): string {
    return ll_tools_wordset_page_build_cache_key('lazy_cards', [
        'token' => sanitize_key($token),
    ]);
}

function ll_tools_wordset_page_build_lazy_cards_token(array $signature = []): string {
    if (empty($signature)) {
        return '';
    }
...
function ll_tools_wordset_page_get_lazy_cards_payload(string $token): ?array {
    ...
    $cache_key = ll_tools_wordset_page_lazy_cards_cache_key($token);
    $payload = ll_tools_wordset_page_get_cached_payload($cache_key, $request_cache);
    if (!is_array($payload)) {
        return null;
    }
...
function ll_tools_wordset_page_category_search_payload_cache_key(string $token): string {
    return ll_tools_wordset_page_build_cache_key('category_search_payload', [
        'token' => sanitize_key($token),
    ]);
}

function ll_tools_wordset_page_build_category_search_token(array $signature = []): string {
    if (empty($signature)) {
        return '';
    }

    return 'search_' . md5((string) wp_json_encode($signature));
}

function ll_tools_wordset_page_store_category_search_payload(array $payload, int $ttl = 0, string $preferred_token = ''): string {
    static $request_cache = [];
...
function ll_tools_wordset_page_get_category_search_payload(string $token): ?array {
    static $request_cache = [];
    ...
    $payload = ll_tools_wordset_page_get_cached_payload(
        ll_tools_wordset_page_category_search_payload_cache_key($token),
        $request_cache
    );
    if (!is_array($payload)) {
        return null;
...
function ll_tools_wordset_page_lazy_cards_request_id_cap(): int {
    $cap = (int) apply_filters('ll_tools_wordset_page_lazy_cards_request_id_cap', 96);
    return max(1, min(500, $cap));
}

function ll_tools_wordset_page_normalize_lazy_cards_requested_ids(array $ids): array {
    $ids = array_values(array_unique(array_filter(array_map('intval', $ids), static function (int $id): bool {
        return $id > 0;
    })));
    sort($ids, SORT_NUMERIC);

    $cap = ll_tools_wordset_page_lazy_cards_request_id_cap();
    if (count($ids) > $cap) {
...
function ll_tools_wordset_page_filter_lazy_cards_requested_ids(array $ids, array $allowed_ids): array {
    if (empty($ids) || empty($allowed_ids)) {
        return [];
    }

    $allowed_lookup = array_fill_keys(array_map('intval', $allowed_ids), true);
    return array_values(array_filter($ids, static function (int $id) use ($allowed_lookup): bool {
        return isset($allowed_lookup[$id]);
    }));
}
...
function ll_tools_wordset_page_wait_for_lazy_cards_ajax_cached_response(array $args, int $wait_ms = 1000) {
    ...
    do {
        $cached = ll_tools_wordset_page_lazy_cards_ajax_cache_get($args);
        if (is_array($cached)) {
            return $cached;
        }
...
function ll_tools_wordset_page_lazy_cards_cache_miss_throttle_config(): array {
    $config = [
        'window' => apply_filters('ll_tools_wordset_page_lazy_cards_cache_miss_window', 5 * MINUTE_IN_SECONDS),
        'token_limit' => apply_filters('ll_tools_wordset_page_lazy_cards_cache_miss_token_limit', 60),
        'ip_limit' => apply_filters('ll_tools_wordset_page_lazy_cards_cache_miss_ip_limit', 180),
    ];

    return [
        'window' => max(MINUTE_IN_SECONDS, (int) ($config['window'] ?? (5 * MINUTE_IN_SECONDS))),
        'token_limit' => max(0, (int) ($config['token_limit'] ?? 60)),
...
function ll_tools_wordset_page_get_client_ip(): string {
    $candidates = apply_filters('ll_tools_wordset_page_client_ip_candidates', [
...
function ll_tools_wordset_page_category_search_ajax_cache_enabled(string $token): bool {
    if (is_user_logged_in()) {
        return false;
    }

    return preg_match('/^search_[a-f0-9]{32}$/', sanitize_key($token)) === 1;
}

function ll_tools_wordset_page_category_search_ajax_cache_ttl(): int {
    $default_ttl = function_exists('ll_tools_public_static_cache_ttl')
        ? ll_tools_public_static_cache_ttl()
        : HOUR_IN_SECONDS;

    return ll_tools_wordset_page_normalize_cache_ttl('ll_tools_wordset_page_category_search_ajax_cache_ttl', $default_ttl);
}

function ll_tools_wordset_page_category_search_ajax_cache_key(array $args): string {
    $category_epoch = function_exists('ll_tools_get_category_cache_epoch')
        ? max(1, (int) ll_tools_get_category_cache_epoch())
        : 1;
    $wordset_epoch = function_exists('ll_tools_get_wordset_cache_epoch')
...
function ll_tools_wordset_page_category_search_cache_miss_throttle_config(): array {
    $config = [
        'window' => apply_filters('ll_tools_wordset_page_category_search_cache_miss_window', 5 * MINUTE_IN_SECONDS),
        'token_limit' => apply_filters('ll_tools_wordset_page_category_search_cache_miss_token_limit', 90),
        'ip_limit' => apply_filters('ll_tools_wordset_page_category_search_cache_miss_ip_limit', 240),
    ];

    return [
        'window' => max(MINUTE_IN_SECONDS, (int) ($config['window'] ?? (5 * MINUTE_IN_SECONDS))),
        'token_limit' => max(0, (int) ($config['token_limit'] ?? 90)),
        'ip_limit' => max(0, (int) ($config['ip_limit'] ?? 240)),
    ];
}

function ll_tools_wordset_page_category_search_cache_miss_throttle_key(string $scope, string $identifier): string {
    return 'll_tools_wsp_search_miss_' . sanitize_key($scope) . '_' . substr(hash('sha256', $identifier), 0, 24);
}

function ll_tools_wordset_page_category_search_cache_miss_limited(string $token): bool {
    if (!ll_tools_wordset_page_category_search_ajax_cache_enabled($token)) {
        return false;
    }

    $config = ll_tools_wordset_page_category_search_cache_miss_throttle_config();
    $checks = [
        [
            'scope' => 'token',
    ...
    foreach ($checks as $check) {
        ...
        $attempts = (int) get_transient(ll_tools_wordset_page_category_search_cache_miss_throttle_key((string) $check['scope'], $identifier));
        if ($attempts >= $limit) {
            return true;
        }
    }
...
function ll_tools_wordset_page_record_category_search_cache_miss(string $token): void {
    if (!ll_tools_wordset_page_category_search_ajax_cache_enabled($token)) {
        return;
    }
    ...
    foreach ($targets as $scope => $identifier) {
        ...
        $key = ll_tools_wordset_page_category_search_cache_miss_throttle_key((string) $scope, $identifier);
        $attempts = (int) get_transient($key);
        set_transient($key, $attempts + 1, $window);
    }
}

function ll_tools_wordset_page_category_search_ajax_cache_get(array $args) {
    static $request_cache = [];
    ...
    $cached = ll_tools_wordset_page_get_cached_payload(
        ll_tools_wordset_page_category_search_ajax_cache_key($args),
        $request_cache
    );
...
function ll_tools_wordset_page_category_search_ajax_cache_set(array $args, array $payload): void {
    static $request_cache = [];
    ...
    ll_tools_wordset_page_store_cached_payload(
        ll_tools_wordset_page_category_search_ajax_cache_key($args),
        $payload,
        ll_tools_wordset_page_category_search_ajax_cache_ttl(),
        $request_cache
    );
}

function ll_tools_wordset_page_category_search_ajax_send_cache_header(string $status): void {
    if (!headers_sent()) {
        header('X-LL-Wordset-Category-Search-Cache: ' . strtoupper($status));
    }
}

function ll_tools_wordset_page_lazy_cards_cache_miss_limited(string $token): bool {
...
function ll_tools_wordset_page_build_lazy_cards_fallback_payload(int $wordset_id, int $preview_limit = 2): array {
    ...
    foreach ((array) $categories as $cat) {
        ...
        $is_virtual_category = is_array($cat) && !empty($cat['is_virtual_category']);
        ...
        if (!$is_virtual_category && $enhanced['aspect_bucket'] === '' && function_exists('ll_tools_get_category_aspect_bucket_key')) {
    ...
    foreach ((array) $wordset_content_lessons as $content_lesson) {
        if (is_array($content_lesson) && !empty($content_lesson['show_in_mix'])) {
    ...
    return [
        ...
        'render_context' => [
            ...
            'category_starred_lookup' => $category_starred_lookup,
            'summary_counts_deferred' => $summary_counts_deferred,
...
function ll_tools_wordset_page_hydrate_category_preview(array $category, int $preview_limit = 0): array {
    if (empty($category['preview_deferred'])) {
        return $category;
    }

    $category_id = isset($category['id']) ? (int) $category['id'] : 0;
    $wordset_id = isset($category['wordset_id']) ? (int) $category['wordset_id'] : 0;
    if ($category_id <= 0 || $wordset_id <= 0) {
        $category['preview_deferred'] = false;
        return $category;
    }

    $stored_preview_limit = max(0, (int) ($category['preview_limit'] ?? 0));
    $limit = $stored_preview_limit > 0
        ? $stored_preview_limit
        : max(1, (int) ($preview_limit > 0 ? $preview_limit : 2));
    $requires_images = array_key_exists('preview_requires_images', $category)
        ? !empty($category['preview_requires_images'])
        : !empty($category['has_images']);
    ...
    $category['preview'] = isset($preview['items']) && is_array($preview['items'])
        ? $preview['items']
        : [];
    $category['has_images'] = !empty($preview['has_images']);
    $category['preview_aspect_ratio'] = isset($preview['preview_aspect_ratio'])
        ? (string) $preview['preview_aspect_ratio']
        : '';
    $category['preview_deferred'] = false;
...
function ll_tools_wordset_page_hydrate_mixed_card_previews(array $cards, int $preview_limit = 0): array {
    foreach ($cards as $index => $card) {
        if (!is_array($card) || ($card['type'] ?? '') !== 'category' || !isset($card['data']) || !is_array($card['data'])) {
            continue;
        }

        $cards[$index]['data'] = ll_tools_wordset_page_hydrate_category_preview($card['data'], $preview_limit);
    }
...
function ll_tools_wordset_page_render_category_card(array $cat, array $context = []): string {
    ...
    $category_progress_lookup = (isset($context['category_progress_lookup']) && is_array($context['category_progress_lookup']))
        ? $context['category_progress_lookup']
        : [];
    $category_starred_lookup = (isset($context['category_starred_lookup']) && is_array($context['category_starred_lookup']))
        ? $context['category_starred_lookup']
        : [];
    $summary_counts_deferred = !empty($context['summary_counts_deferred']);
    $is_study_user = !empty($context['is_study_user']);
    $gender_enabled = !empty($context['gender_enabled']);
    $mode_ui = (isset($context['mode_ui']) && is_array($context['mode_ui'])) ? $context['mode_ui'] : [];
    $mode_labels = (isset($context['mode_labels']) && is_array($context['mode_labels'])) ? $context['mode_labels'] : [];
    $mode_fallback_icons = (isset($context['mode_fallback_icons']) && is_array($context['mode_fallback_icons'])) ? $context['mode_fallback_icons'] : [];

    $preview_style = '';
    if (!empty($cat['preview_aspect_ratio'])) {
        $preview_style = ' style="--ll-wordset-preview-aspect: ' . esc_attr((string) $cat['preview_aspect_ratio']) . ';"';
    }
    ...
    $public_note_label = (!$is_public && isset($cat['public_note_label']) && trim((string) $cat['public_note_label']) !== '')
        ? trim((string) $cat['public_note_label'])
        : __('Not public', 'll-tools-text-domain');
    ...
    <article class="ll-wordset-card<?php echo $is_public ? '' : ' ll-wordset-card--inactive'; ?><?php echo $can_preview_inactive ? ' ll-wordset-card--inactive-previewable' : ''; ?><?php echo $is_virtual_category ? ' ll-wordset-card--virtual' : ''; ?>" role="listitem" data-cat-id="<?php echo esc_attr($cat_id); ?>" data-word-count="<?php echo esc_attr((int) ($cat['count'] ?? 0)); ?>" data-ll-wordset-public="<?php echo $is_public ? '1' : '0'; ?>"<?php echo $can_preview_inactive ? ' data-ll-wordset-inactive-preview-card="true"' : ''; ?><?php echo $is_virtual_category ? ' data-ll-wordset-virtual-category="' . esc_attr($virtual_category_type) . '"' : ''; ?>>
        <div class="ll-wordset-card__top">
            ...
            <?php endif; ?>
            <?php if ($is_public && $is_study_user) : ?>
            ...
            <?php elseif ($can_manage_inactive_actions) : ?>
                <div class="ll-wordset-card__inactive-actions" role="group" aria-label="<?php echo esc_attr(sprintf(__('Category management for %s', 'll-tools-text-domain'), $cat_name)); ?>">
                    ...
                    echo ll_tools_wordset_page_render_inactive_category_icon_action_form(
                        ...
                        __('Delete this category and any linked vocab lesson? This cannot be undone.', 'll-tools-text-domain')
                    ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
                    ?>
                </div>
            <?php else : ?>
        ...
        <?php endif; ?>
                <div class="ll-wordset-card__preview <?php echo !empty($cat['has_images']) ? 'has-images' : 'has-text'; ?>"<?php echo $preview_style; ?>>
                    <?php if ($is_public && $card_starred_count > 0) : ?>
                        <span
                            class="ll-wordset-card__starred-pill ll-wordset-progress-pill ll-wordset-progress-pill--starred"
                            aria-label="<?php echo esc_attr(sprintf(_n('%1$d starred word in %2$s', '%1$d starred words in %2$s', $card_starred_count, 'll-tools-text-domain'), $card_starred_count, $cat_name)); ?>">
                        ...
                        <?php foreach (array_slice($preview_items, 0, $current_preview_limit) as $preview) : ?>
                            <?php if (($preview['type'] ?? '') === 'image') : ?>
                                ...
                                if (!empty($preview['ratio'])) {
                                    $ratio_value = esc_attr((string) $preview['ratio']);
                                ...
                                <span class="ll-wordset-preview-item ll-wordset-preview-item--image"<?php echo $preview_ratio_style; ?>>
                                    <img src="<?php echo esc_url((string) ($preview['url'] ?? '')); ?>" alt="<?php echo esc_attr((string) ($preview['alt'] ?? '')); ?>"<?php echo $preview_width_attr . $preview_height_attr; ?> loading="lazy" decoding="async" fetchpriority="low" />
                                </span>
                            <?php else : ?>
                                <span class="ll-wordset-preview-item ll-wordset-preview-item--text">
                                    <span class="ll-wordset-preview-text" dir="auto"><?php echo esc_html((string) ($preview['label'] ?? '')); ?></span>
                                </span>
                            <?php endif; ?>
                        <?php endforeach; ?>
        ...
        <?php if (!$is_public) : ?>
            <div class="ll-wordset-card__public-note" role="note">
                <span class="ll-wordset-card__public-note-label"><?php echo esc_html($public_note_label); ?></span>
        ...
        <?php else : ?>
            ...
            <div class="ll-wordset-card__quiz-actions" role="group" aria-label="<?php echo esc_attr(sprintf(__('Quiz modes for %s', 'll-tools-text-domain'), $cat_name)); ?>">
                <?php
                $card_modes = ll_tools_get_study_launch_mode_order($gender_enabled && !empty($cat['gender_supported']));
                foreach ($card_modes as $mode) :
                ?>
                    <button type="button" class="ll-wordset-card__quiz-btn" data-ll-wordset-category-mode data-mode="<?php echo esc_attr($mode); ?>" data-cat-id="<?php echo esc_attr($cat_id); ?>" aria-label="<?php echo esc_attr(sprintf(__('%1$s: %2$s', 'll-tools-text-domain'), $mode_labels[$mode] ?? ucfirst($mode), $cat_name)); ?>">
...
function ll_tools_wordset_page_render_mixed_card(array $card, array $context = []): string {
    ...
    $category = (isset($card['data']) && is_array($card['data'])) ? $card['data'] : [];
    return ll_tools_wordset_page_render_category_card($category, $context);
}

function ll_tools_wordset_page_render_mixed_cards(array $cards, array $context = []): string {
    if (empty($cards)) {
        return '';
    }
    ...
    foreach ($cards as $card) {
        ...
        $html .= ll_tools_wordset_page_render_mixed_card($card, $context);
    }
...
function ll_tools_wordset_page_normalize_category_search_match_text(string $value): string {
    $text = ll_tools_wordset_page_normalize_search_text($value);
    if ($text === '') {
        return '';
    }
...
function ll_tools_wordset_page_prepare_category_search_query(string $query): string {
    $query = ll_tools_wordset_page_normalize_category_search_match_text($query);
    if ($query === '') {
        return '';
    }
...
function ll_tools_wordset_page_get_category_search_matches(int $wordset_id, array $allowed_category_ids, string $query, int $per_category_limit = 1): array {
    ...
    $index = ll_tools_get_wordset_page_category_search_index($wordset_id, $allowed_category_ids);
    if (empty($index)) {
        return [];
    }
...
function ll_tools_wordset_page_inactive_category_notice(): ?array {
    $status = isset($_GET['ll_wordset_inactive_category'])
        ? sanitize_key(wp_unslash((string) $_GET['ll_wordset_inactive_category']))
        : '';
    if ($status === '') {
        return null;
    }

    $message = isset($_GET['ll_wordset_inactive_category_message'])
        ? sanitize_text_field(wp_unslash((string) $_GET['ll_wordset_inactive_category_message']))
        : '';

    if ($status === 'ok') {
        $result = isset($_GET['ll_wordset_inactive_category_result'])
            ? sanitize_key(wp_unslash((string) $_GET['ll_wordset_inactive_category_result']))
            : '';
        if ($result === 'hidden') {
            return [
                'type' => 'success',
    ...
    $error = isset($_GET['ll_wordset_inactive_category_error'])
        ? sanitize_key(wp_unslash((string) $_GET['ll_wordset_inactive_category_error']))
        : '';
    if ($message !== '') {
        return [
            'type' => 'error',
    ...
    if ($error === 'permission') {
        return [
            'type' => 'error',
            'message' => __('You do not have permission to manage this category.', 'll-tools-text-domain'),
        ];
    }
    if ($error === 'nonce') {
        return [
    ...
    if ($error === 'delete_blocked') {
        return [
            'type' => 'error',
            'message' => __('This category still has content attached.', 'll-tools-text-domain'),
        ];
    }
    if ($error === 'preview_create' || $error === 'preview_prepare') {
        return [
            'type' => 'error',
            'message' => __('Unable to open that lesson preview right now.', 'll-tools-text-domain'),
        ];
    }

    return [
        'type' => 'error',
        'message' => __('Unable to update that category right now.', 'll-tools-text-domain'),
    ];
}
...
function ll_tools_wordset_page_manager_recorder_queue_notice(): ?array {
    $status = isset($_GET['ll_wordset_manager_recorder_queue'])
        ? sanitize_key(wp_unslash((string) $_GET['ll_wordset_manager_recorder_queue']))
        : '';
    if ($status === '') {
        return null;
    }
    ...
    if ($status === 'ok') {
        $result = isset($_GET['ll_wordset_manager_recorder_queue_result'])
            ? sanitize_key(wp_unslash((string) $_GET['ll_wordset_manager_recorder_queue_result']))
            : '';

        return [
            'type' => 'success',
            'message' => ll_tools_wordset_page_manager_recorder_queue_result_message($result),
        ];
    }

    $error = isset($_GET['ll_wordset_manager_recorder_queue_error'])
        ? sanitize_key(wp_unslash((string) $_GET['ll_wordset_manager_recorder_queue_error']))
        : '';

    return [
        'type' => 'error',
        'message' => ll_tools_wordset_page_manager_recorder_queue_error_message($error),
    ];
}

function ll_tools_wordset_page_manager_recorder_queue_result_message(string $result): string {
    if ($result === 'prompts') {
    ...
    if ($result === 'unhidden') {
        return __('Recorder queue updated. Hidden word restored.', 'll-tools-text-domain');
    }

    return __('Recorder queue updated. Word hidden for that recorder.', 'll-tools-text-domain');
}

function ll_tools_wordset_page_manager_recorder_queue_error_message(string $error): string {
    if ($error === 'permission') {
        return __('You do not have permission to update recorder queues for this word set.', 'll-tools-text-domain');
    }
    if ($error === 'nonce') {
        return __('Your session expired. Please try again.', 'll-tools-text-domain');
    }
    if ($error === 'wordset') {
        return __('Unable to find that word set.', 'll-tools-text-domain');
    }
    if ($error === 'user') {
        return __('Unable to find that recorder user.', 'll-tools-text-domain');
    }
    if ($error === 'role') {
        return __('The selected user does not have the Audio Recorder role.', 'll-tools-text-domain');
    }
    if ($error === 'assignment') {
        return __('That recorder is not assigned to this word set.', 'll-tools-text-domain');
    }
    if ($error === 'key') {
        return __('Unable to identify that queue item.', 'll-tools-text-domain');
    }
    if ($error === 'target') {
        return __('Unable to save prompts for that queue item.', 'll-tools-text-domain');
    }

    return __('Unable to update the recorder queue right now.', 'll-tools-text-domain');
}

function ll_tools_wordset_page_is_recorder_assigned_to_wordset(int $recorder_user_id, WP_Term $wordset_term): bool {
    if ($recorder_user_id <= 0) {
        return false;
    }

    if (function_exists('ll_tools_get_assigned_recorder_wordset_ids_for_user')) {
        $assigned_ids = array_map('intval', (array) ll_tools_get_assigned_recorder_wordset_ids_for_user($recorder_user_id));
        return in_array((int) $wordset_term->term_id, $assigned_ids, true);
    }
    ...
    return is_array($config)
        && sanitize_title((string) ($config['wordset'] ?? '')) === sanitize_title((string) $wordset_term->slug);
}
...
function ll_tools_wordset_page_recorder_queue_request_scalar(array $request, string $key): string {
    ...
    return (string) wp_unslash($request[$key]);
}

/**
 * @return array{result:string,wordset_id:int,recorder_user_id:int}|WP_Error
 */
function ll_tools_wordset_page_process_manager_recorder_queue_action(array $request, WP_Term $wordset_term) {
    ...
    $wordset_id = (int) $wordset_term->term_id;
    $submitted_wordset_id = (int) ll_tools_wordset_page_recorder_queue_request_scalar($request, 'll_wordset_manager_recorder_queue_wordset_id');
    $recorder_user_id = (int) ll_tools_wordset_page_recorder_queue_request_scalar($request, 'll_wordset_manager_recorder_queue_user_id');
    $nonce = ll_tools_wordset_page_recorder_queue_request_scalar($request, 'll_wordset_manager_recorder_queue_nonce');

    if ($submitted_wordset_id !== $wordset_id) {
        return new WP_Error('wordset', ll_tools_wordset_page_manager_recorder_queue_error_message('wordset'));
    }
    if (!function_exists('ll_tools_current_user_can_manage_wordset_content') || !ll_tools_current_user_can_manage_wordset_content($wordset_id)) {
        return new WP_Error('permission', ll_tools_wordset_page_manager_recorder_queue_error_message('permission'));
    }
    ...
    if (!in_array('audio_recorder', (array) $recorder_user->roles, true)) {
        return new WP_Error('role', ll_tools_wordset_page_manager_recorder_queue_error_message('role'));
    }
    if (!ll_tools_wordset_page_is_recorder_assigned_to_wordset($recorder_user_id, $wordset_term)) {
        return new WP_Error('assignment', ll_tools_wordset_page_manager_recorder_queue_error_message('assignment'));
    }
    ...
    if ($action === 'save_settings') {
        ...
        $all_type_slugs = function_exists('ll_tools_get_all_recording_type_slugs')
            ? ll_tools_get_all_recording_type_slugs()
            : [];
        $allowed_type_map = array_fill_keys(array_values(array_filter(array_map('sanitize_title', $all_type_slugs))), true);
        $normalize_type_values = static function ($raw_values) use ($allowed_type_map): array {
            ...
            foreach ($values as $raw_value) {
                ...
                $slug = sanitize_title((string) wp_unslash($raw_value));
                if ($slug === '' || (!empty($allowed_type_map) && empty($allowed_type_map[$slug]))) {
                    continue;
                }
            ...
            if (function_exists('ll_sort_recording_type_slugs')) {
                return ll_sort_recording_type_slugs($normalized);
            }
        ...
        $include_types = $normalize_type_values($request['ll_wordset_manager_recorder_queue_include_types'] ?? []);
        $exclude_types = $normalize_type_values($request['ll_wordset_manager_recorder_queue_exclude_types'] ?? []);

        $config['wordset'] = (string) $wordset_term->slug;
        $config['category'] = '';
        $config['include_recording_types'] = implode(',', $include_types);
        $config['exclude_recording_types'] = implode(',', $exclude_types);
        $config['allow_new_words'] = !empty($request['ll_wordset_manager_recorder_queue_allow_new_words']) ? '1' : '0';
        $config['auto_process_recordings'] = !empty($request['ll_wordset_manager_recorder_queue_auto_process_recordings']) ? '1' : '0';
    ...
    $hide_key = ll_tools_sanitize_recording_hide_key(ll_tools_wordset_page_recorder_queue_request_scalar($request, 'll_wordset_manager_recorder_queue_hide_key'));
    $word_id = absint(ll_tools_wordset_page_recorder_queue_request_scalar($request, 'll_wordset_manager_recorder_queue_word_id'));
    $image_id = absint(ll_tools_wordset_page_recorder_queue_request_scalar($request, 'll_wordset_manager_recorder_queue_image_id'));
    $prompt_card_id = absint(ll_tools_wordset_page_recorder_queue_request_scalar($request, 'll_wordset_manager_recorder_queue_prompt_card_id'));
    $title = sanitize_text_field(ll_tools_wordset_page_recorder_queue_request_scalar($request, 'll_wordset_manager_recorder_queue_title'));
    $category_name = sanitize_text_field(ll_tools_wordset_page_recorder_queue_request_scalar($request, 'll_wordset_manager_recorder_queue_category_name'));
    $category_slug = sanitize_title(ll_tools_wordset_page_recorder_queue_request_scalar($request, 'll_wordset_manager_recorder_queue_category_slug'));

    if ($hide_key === '') {
        $hide_key = ll_tools_build_recording_hide_key($word_id, $image_id, $title, $prompt_card_id);
    }
    if ($hide_key === '') {
        return new WP_Error('key', ll_tools_wordset_page_manager_recorder_queue_error_message('key'));
    }

    if ($action === 'save_prompts') {
        $target_post_id = function_exists('ll_tools_get_recording_prompt_target_post_id')
            ? ll_tools_get_recording_prompt_target_post_id($word_id, $image_id, $prompt_card_id)
            : 0;
        if ($target_post_id <= 0 || !ll_tools_wordset_page_recorder_prompt_target_belongs_to_wordset($target_post_id, $wordset_id)) {
            return new WP_Error('target', ll_tools_wordset_page_manager_recorder_queue_error_message('target'));
        }

        $raw_prompts = [];
        if (isset($request['ll_wordset_manager_recorder_queue_prompts']) && is_array($request['ll_wordset_manager_recorder_queue_prompts'])) {
            $raw_prompts = wp_unslash($request['ll_wordset_manager_recorder_queue_prompts']);
        }

        if (function_exists('ll_tools_save_recording_prompt_hints_for_post')) {
            ll_tools_save_recording_prompt_hints_for_post($target_post_id, (array) $raw_prompts);
        }
    ...
    if ($action === 'hide') {
        ...
        ll_tools_remove_hidden_recording_word($recorder_user_id, $hide_key, $word_id, $image_id, $title, $prompt_card_id);
        $result = 'unhidden';
    }
...
function ll_tools_wordset_page_recorder_queue_ajax_status_code(WP_Error $error): int {
    $code = $error->get_error_code();
    if (in_array($code, ['permission', 'nonce', 'role', 'assignment'], true)) {
        return 403;
    }
...
function ll_tools_wordset_page_handle_manager_recorder_queue_action(): void {
    if (strtoupper((string) ($_SERVER['REQUEST_METHOD'] ?? 'GET')) !== 'POST') {
        return;
    }
    if (!ll_tools_is_wordset_page_context()) {
    ...
    $base_redirect = ll_tools_get_wordset_settings_tool_url(
        $wordset_term,
        $submitted_tool !== '' ? $submitted_tool : 'recorder-queues',
        ll_tools_wordset_page_resolve_back_url($wordset_term)
    );

    $redirect_error = static function (string $error) use ($base_redirect): void {
        wp_safe_redirect(add_query_arg([
            'll_wordset_manager_recorder_queue' => 'error',
            'll_wordset_manager_recorder_queue_error' => $error,
        ], $base_redirect));
        exit;
    };

    $result = ll_tools_wordset_page_process_manager_recorder_queue_action($_POST, $wordset_term);
    if (is_wp_error($result)) {
        $redirect_error($result->get_error_code());
    }

    wp_safe_redirect(add_query_arg([
        'll_wordset_manager_recorder_queue' => 'ok',
        'll_wordset_manager_recorder_queue_result' => (string) ($result['result'] ?? 'updated'),
    ], $base_redirect));
    exit;
}
add_action('template_redirect', 'll_tools_wordset_page_handle_manager_recorder_queue_action', 6);

function ll_tools_wordset_page_handle_manager_recorder_queue_action_ajax(): void {
    $wordset_id = (int) ll_tools_wordset_page_recorder_queue_request_scalar($_POST, 'll_wordset_manager_recorder_queue_wordset_id');
    $wordset_term = $wordset_id > 0 ? get_term($wordset_id, 'wordset') : null;
    if (!($wordset_term instanceof WP_Term) || is_wp_error($wordset_term)) {
        wp_send_json_error([
            'code' => 'wordset',
            'message' => ll_tools_wordset_page_manager_recorder_queue_error_message('wordset'),
        ], 400);
    }
    ...
    if (is_wp_error($result)) {
        ...
        wp_send_json_error([
            'code' => $code,
            'message' => $result->get_error_message() ?: ll_tools_wordset_page_manager_recorder_queue_error_message($code),
        ], ll_tools_wordset_page_recorder_queue_ajax_status_code($result));
    }

    $result_key = (string) ($result['result'] ?? 'updated');
    wp_send_json_success([
        'result' => $result_key,
        'wordset_id' => (int) ($result['wordset_id'] ?? $wordset_id),
        'recorder_user_id' => (int) ($result['recorder_user_id'] ?? 0),
        'message' => ll_tools_wordset_page_manager_recorder_queue_result_message($result_key),
    ]);
}
add_action('wp_ajax_ll_tools_wordset_recorder_queue_save', 'll_tools_wordset_page_handle_manager_recorder_queue_action_ajax');
...
function ll_tools_wordset_settings_tool_label(string $tool): string {
    ...
    if ($tool === 'recorder-queues') {
        return __('Recorder Queues', 'll-tools-text-domain');
    }
...
function ll_tools_wordset_settings_tool_description(string $tool): string {
    ...
    if ($tool === 'recorder-queues') {
        return __('Review each recorder\'s live queue and manage hidden words without leaving the front end.', 'll-tools-text-domain');
    }
    if ($tool === 'transcription') {
...
function ll_tools_wordset_page_render_settings_tool_icon(string $tool, string $class = 'll-wordset-settings-tool-card__icon-svg'): string {
    ...
    if ($tool === 'recorder-queues') {
        return '<svg class="' . esc_attr($class) . '" viewBox="0 0 24 24" fill="none" aria-hidden="true" focusable="false">'
            . '<rect x="4.75" y="5" width="14.5" height="14.25" rx="2.25" stroke="currentColor" stroke-width="1.8"/>'
            . '<path d="M8.25 9h7.5M8.25 12h7.5M8.25 15h4.5" stroke="currentColor" stroke-width="1.8" stroke-linecap="round"/>'
            . '<path d="M16.25 6.75a3 3 0 1 1 0 6 3 3 0 0 1 0-6Z" fill="#eef5fb" stroke="currentColor" stroke-width="1.4"/>'
            . '<path d="M17.1 8.8 15.75 10.1l-.6-.6" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"/>'
            . '</svg>';
    }
    if ($tool === 'transcription') {
        return '<svg class="' . esc_attr($class) . '" viewBox="0 0 24 24" fill="none" aria-hidden="true" focusable="false">'
...
function ll_tools_wordset_page_get_managed_category_rows(int $wordset_id, array $enhanced_categories = []): array {
    ...
    foreach ($ordered_rows as $index => $row) {
        ...
        } elseif (isset($term_lookup[$category_id]) && $term_lookup[$category_id] instanceof WP_Term) {
...
function ll_tools_wordset_page_get_vocab_lesson_ids_for_category(int $category_id, int $wordset_id): array {
    ...
    return array_values(array_unique(array_filter(array_map('intval', (array) $lesson_ids), static function (int $lesson_id): bool {
        return $lesson_id > 0;
    })));
}
 ...
 * @return int|WP_Error
 */
function ll_tools_wordset_page_delete_vocab_lessons_for_category(int $category_id, int $wordset_id) {
    $lesson_ids = ll_tools_wordset_page_get_vocab_lesson_ids_for_category($category_id, $wordset_id);
    if (empty($lesson_ids)) {
        return 0;
    }
    ...
    foreach ($lesson_ids as $lesson_id) {
        $result = wp_delete_post((int) $lesson_id, true);
        if (!($result instanceof WP_Post)) {
            return new WP_Error('delete_lesson_failed', __('Unable to delete the linked vocab lesson right now.', 'll-tools-text-domain'));
        }
        $deleted++;
    }
    ...
    <section class="ll-wordset-settings-page ll-wordset-settings-page--tool" data-ll-wordset-settings-page>
        <div class="ll-wordset-settings-card">
            ...
            <div class="ll-wordset-settings-card__group">
                ...
                <p class="description" style="margin-top:0;">
                    <?php echo esc_html__('Rename categories, update translated names, move them under a different parent, or delete categories.', 'll-tools-text-domain'); ?>
                </p>
...
function ll_tools_wordset_page_get_recorder_queue_primary_label(array $item): string {
    $word_title = trim((string) ($item['word_title'] ?? ''));
    $title = trim((string) ($item['title'] ?? ''));
    $translation = trim((string) ($item['word_translation'] ?? ''));

    if (!empty($item['use_word_display']) && $word_title !== '') {
        return $word_title;
    }
    if ($title !== '') {
        return $title;
    ...
    return __('Untitled word', 'll-tools-text-domain');
}

function ll_tools_wordset_page_get_recorder_queue_secondary_label(array $item): string {
    $primary = ll_tools_wordset_page_get_recorder_queue_primary_label($item);
    $candidates = [
        trim((string) ($item['word_translation'] ?? '')),
        trim((string) ($item['title'] ?? '')),
        trim((string) ($item['word_title'] ?? '')),
    ];
...
function ll_tools_wordset_page_get_recorder_queue_type_labels(array $slugs): array {
    $slugs = function_exists('ll_sort_recording_type_slugs')
        ...
        : array_values(array_unique(array_filter(array_map('sanitize_key', $slugs))));
    ...
    $labels = [];
    if (function_exists('ll_build_recording_type_payload')) {
        foreach ((array) ll_build_recording_type_payload($slugs) as $type_row) {
            if (!is_array($type_row)) {
                continue;
            }
            $name = trim((string) ($type_row['name'] ?? ''));
            if ($name !== '') {
                $labels[] = $name;
            }
        }
    ...
    if (empty($labels)) {
        foreach ($slugs as $slug) {
            $labels[] = ucfirst(str_replace('-', ' ', (string) $slug));
        }
    }
...
function ll_tools_wordset_page_get_recorder_queue_type_label_map(array $slugs): array {
    $slugs = function_exists('ll_sort_recording_type_slugs')
        ? ll_sort_recording_type_slugs($slugs)
        : array_values(array_unique(array_filter(array_map('sanitize_key', $slugs))));
    if (empty($slugs)) {
    ...
    if (function_exists('ll_build_recording_type_payload')) {
        foreach ((array) ll_build_recording_type_payload($slugs) as $type_row) {
            ...
            $slug = sanitize_title((string) ($type_row['slug'] ?? ''));
            $name = trim((string) ($type_row['name'] ?? ''));
            if ($slug !== '' && $name !== '') {
                $map[$slug] = $name;
            }
    ...
    foreach ($slugs as $slug) {
        ...
        if (!isset($map[$slug])) {
            $map[$slug] = ucwords(str_replace('-', ' ', $slug));
        }
    }
...
function ll_tools_wordset_page_build_recorder_queue_item_lookup(array $items): array {
    $lookup = [];
    ...
    foreach ($items as $item) {
        ...
        $keys = function_exists('ll_tools_get_recording_item_hide_keys')
            ? ll_tools_get_recording_item_hide_keys($item)
            : [(string) ($item['hide_key'] ?? '')];

        foreach ((array) $keys as $key) {
            $key = function_exists('ll_tools_sanitize_recording_hide_key')
                ? ll_tools_sanitize_recording_hide_key($key)
...
function ll_tools_wordset_page_recorder_prompt_target_belongs_to_wordset(int $target_post_id, int $wordset_id): bool {
    $post = $target_post_id > 0 ? get_post($target_post_id) : null;
    if (!($post instanceof WP_Post) || $wordset_id <= 0) {
        return false;
    }

    if ($post->post_type === 'words') {
        return has_term($wordset_id, 'wordset', $target_post_id);
    }

    if ($post->post_type === 'word_images') {
        if (function_exists('ll_tools_get_word_image_wordset_owner_id')) {
            $owner_wordset_id = (int) ll_tools_get_word_image_wordset_owner_id($target_post_id);
            if ($owner_wordset_id > 0) {
                return $owner_wordset_id === $wordset_id;
            }
        }

        return function_exists('ll_get_word_for_image_in_wordset')
            && (int) ll_get_word_for_image_in_wordset($target_post_id, [$wordset_id]) > 0;
    }

    if (defined('LL_TOOLS_PROMPT_CARD_POST_TYPE') && $post->post_type === LL_TOOLS_PROMPT_CARD_POST_TYPE) {
        return function_exists('ll_tools_prompt_card_matches_recording_wordsets')
            && ll_tools_prompt_card_matches_recording_wordsets($target_post_id, [$wordset_id]);
    }
...
function ll_tools_wordset_page_hidden_entry_belongs_to_wordset(array $entry, int $wordset_id, array $queue_lookup): bool {
    ...
    $primary_key = function_exists('ll_tools_sanitize_recording_hide_key')
        ...
        : sanitize_text_field((string) ($entry['key'] ?? ''));
    if ($primary_key !== '') {
    ...
    if (function_exists('ll_tools_build_recording_hide_key')) {
        $fallback_key = ll_tools_build_recording_hide_key(
            (int) ($entry['word_id'] ?? 0),
            (int) ($entry['image_id'] ?? 0),
            (string) ($entry['title'] ?? ''),
            (int) ($entry['prompt_card_id'] ?? 0)
        );
        if ($fallback_key !== '') {
            $possible_keys[] = $fallback_key;
        }
    }
    ...
    if ($word_id > 0) {
        ...
        if (!is_wp_error($wordset_ids)) {
            return in_array($wordset_id, array_map('intval', (array) $wordset_ids), true);
        }
    }
    ...
    if ($image_id > 0) {
        if (function_exists('ll_tools_get_word_image_wordset_owner_id')) {
            $owner_wordset_id = (int) ll_tools_get_word_image_wordset_owner_id($image_id);
            if ($owner_wordset_id > 0) {
                return $owner_wordset_id === $wordset_id;
            }
        }

        if (function_exists('ll_get_word_for_image_in_wordset')) {
            return (int) ll_get_word_for_image_in_wordset($image_id, [$wordset_id]) > 0;
        }
    }

    $prompt_card_id = (int) ($entry['prompt_card_id'] ?? 0);
    if ($prompt_card_id > 0 && defined('LL_TOOLS_PROMPT_CARD_POST_TYPE')) {
        return function_exists('ll_tools_prompt_card_matches_recording_wordsets')
            && ll_tools_prompt_card_matches_recording_wordsets($prompt_card_id, [$wordset_id]);
    }
 ...
 * @return string[]
 */
function ll_tools_wordset_page_recorder_queue_type_csv_to_slugs(string $csv): array {
    $slugs = $csv !== '' ? array_map('trim', explode(',', $csv)) : [];
    return function_exists('ll_sort_recording_type_slugs')
        ? ll_sort_recording_type_slugs($slugs)
 ...
 * @return array<string,string>
 */
function ll_tools_wordset_page_get_recorder_queue_recording_type_options(): array {
    $slugs = function_exists('ll_tools_get_all_recording_type_slugs')
        ? ll_tools_get_all_recording_type_slugs()
        : [];
    if (empty($slugs)) {
        return [];
    ...
    return ll_tools_wordset_page_get_recorder_queue_type_label_map($slugs);
}

/**
 * @return array<int,array{name:string,slug:string,items:array<int,array<string,mixed>>}>
 */
function ll_tools_wordset_page_group_recorder_queue_items_by_category(array $items): array {
    $groups = [];
    $uncategorized_label = __('Uncategorized', 'll-tools-text-domain');
    ...
    foreach ($items as $item) {
        ...
        if (!isset($groups[$category_slug])) {
            $groups[$category_slug] = [
                'name' => $category_name,
                'slug' => $category_slug,
                'items' => [],
            ];
        ...
        $groups[$category_slug]['items'][] = $item;
    }

    return array_values($groups);
}

function ll_tools_wordset_page_get_recorder_queue_rows(int $wordset_id, WP_Term $wordset_term, array $assigned_audio_recorders): array {
    if ($wordset_id <= 0 || empty($assigned_audio_recorders)) {
        return [];
    }

    $rows = [];
    foreach ($assigned_audio_recorders as $recorder_user) {
        if (!$recorder_user instanceof WP_User) {
            continue;
        }
        ...
        $all_items = function_exists('ll_tools_get_recording_queue_items')
            ...
            : (function_exists('ll_get_images_needing_audio')
                ? ll_get_images_needing_audio('', [$wordset_id], $include_types, $exclude_types, true, $recorder_user_id)
                : []);
        $visible_items = function_exists('ll_tools_filter_hidden_recording_items')
            ? ll_tools_filter_hidden_recording_items($all_items, $recorder_user_id)
            : $all_items;
        $queue_lookup = ll_tools_wordset_page_build_recorder_queue_item_lookup($all_items);
        ...
        $queue_notes = [];
        if ($include_types !== '') {
            $include_labels = ll_tools_wordset_page_get_recorder_queue_type_labels(array_map('trim', explode(',', $include_types)));
            $queue_notes[] = !empty($include_labels)
                ? sprintf(
                    /* translators: %s: recording type labels */
                    __('Types: %s', 'll-tools-text-domain'),
                    implode(', ', $include_labels)
                )
                : __('Types: filtered', 'll-tools-text-domain');
        } elseif ($exclude_types !== '') {
            $exclude_labels = ll_tools_wordset_page_get_recorder_queue_type_labels(array_map('trim', explode(',', $exclude_types)));
            $queue_notes[] = !empty($exclude_labels)
                ? sprintf(
                    /* translators: %s: recording type labels */
                    __('Skipping: %s', 'll-tools-text-domain'),
                    implode(', ', $exclude_labels)
                )
                : __('Types: filtered', 'll-tools-text-domain');
        } else {
            $queue_notes[] = __('Types: all', 'll-tools-text-domain');
        }
        if ($allow_new_words) {
        ...
        if ($display_name === '') {
            $display_name = (string) $recorder_user->user_login;
        }
        ...
        $rows[] = [
            ...
            'display_name' => $display_name,
            'user_login' => (string) $recorder_user->user_login,
            'user_email' => (string) $recorder_user->user_email,
...
function ll_tools_wordset_page_render_recorder_queue_item(array $item, array $args): string {
    ...
    $recorder_user_id = isset($args['recorder_user_id']) ? (int) $args['recorder_user_id'] : 0;
    $back_url = (string) ($args['back_url'] ?? '');
    $wordset_slug = (string) ($args['wordset_slug'] ?? '');
    $tool = sanitize_key((string) ($args['tool'] ?? 'recorder-queues'));
    $editor_base_url = (string) ($args['editor_base_url'] ?? '');
    $prompts_open = !empty($args['prompts_open']);
    $show_category = array_key_exists('show_category', $args) ? (bool) $args['show_category'] : true;

    if ($action_url === '' || $wordset_id <= 0 || $recorder_user_id <= 0 || $wordset_slug === '' || !in_array($action, ['hide', 'unhide'], true)) {
        return '';
    }

    $hide_key = function_exists('ll_tools_sanitize_recording_hide_key')
        ? ll_tools_sanitize_recording_hide_key((string) ($item['hide_key'] ?? ''))
        : sanitize_text_field((string) ($item['hide_key'] ?? ''));
    if ($hide_key === '' && function_exists('ll_tools_build_recording_hide_key')) {
        $hide_key = ll_tools_build_recording_hide_key(
            (int) ($item['word_id'] ?? 0),
            (int) ($item['image_id'] ?? ($item['id'] ?? 0)),
            (string) ($item['word_title'] ?? ($item['title'] ?? ''))
        );
    }
    if ($hide_key === '') {
        return '';
    ...
    $primary_label = ll_tools_wordset_page_get_recorder_queue_primary_label($item);
    $secondary_label = ll_tools_wordset_page_get_recorder_queue_secondary_label($item);
    $category_name = trim((string) ($item['category_name'] ?? ''));
    if ($category_name === '') {
        $category_name = __('Uncategorized', 'll-tools-text-domain');
    }
    $type_labels = ll_tools_wordset_page_get_recorder_queue_type_labels((array) ($item['missing_types'] ?? []));
    $image_url = trim((string) ($item['image_url'] ?? ''));
    $is_hidden = ($action === 'unhide');
    $action_label = $is_hidden ? __('Unhide', 'll-tools-text-domain') : __('Hide', 'll-tools-text-domain');
    $word_id = (int) ($item['word_id'] ?? 0);
    $image_id = (int) ($item['image_id'] ?? ($item['id'] ?? 0));
    $prompt_card_id = (int) ($item['prompt_card_id'] ?? 0);
    $title_value = trim((string) ($item['word_title'] ?? ($item['title'] ?? '')));
    $category_slug = sanitize_title((string) ($item['category_slug'] ?? ''));
    $is_stale = !empty($item['is_stale']);
    $prompt_type_slugs = function_exists('ll_sort_recording_type_slugs')
        ? ll_sort_recording_type_slugs(array_merge((array) ($item['prompt_types'] ?? []), (array) ($item['missing_types'] ?? [])))
        : array_values(array_unique(array_filter(array_map('sanitize_key', array_merge((array) ($item['prompt_types'] ?? []), (array) ($item['missing_types'] ?? []))))));
    $prompt_type_labels = ll_tools_wordset_page_get_recorder_queue_type_label_map($prompt_type_slugs);
    $prompt_hints = function_exists('ll_tools_get_recording_prompt_hints_for_item')
        ? ll_tools_get_recording_prompt_hints_for_item($item)
        : (is_array($item['recording_prompts'] ?? null) ? (array) $item['recording_prompts'] : []);
    $prompt_target_post_id = function_exists('ll_tools_get_recording_prompt_target_post_id')
        ? ll_tools_get_recording_prompt_target_post_id($word_id, $image_id, $prompt_card_id)
        : 0;
    $can_save_prompts = $prompt_target_post_id > 0 && !empty($prompt_type_slugs);
    $edit_url = '';
    if ($word_id > 0 && $editor_base_url !== '') {
        $editor_search = trim((string) get_the_title($word_id));
        if ($editor_search !== '') {
            $edit_url = add_query_arg([
                'll_wordset_tool' => 'editor',
                'll_editor_q' => $editor_search,
                'll_editor_exact' => '1',
            ], $editor_base_url);
        }
    }
    ...
    ob_start();
    ?>
    <li class="ll-wordset-recorder-queue-item<?php echo $is_hidden ? ' is-hidden-item' : ''; ?>">
        <div class="ll-wordset-recorder-queue-item__media">
            <?php if ($image_url !== '') : ?>
                <span class="ll-wordset-recorder-queue-item__thumb ll-wordset-recorder-queue-item__thumb--image">
                    <img src="<?php echo esc_url($image_url); ?>" alt="" loading="lazy" decoding="async" />
                </span>
            <?php else : ?>
                <span class="ll-wordset-recorder-queue-item__thumb ll-wordset-recorder-queue-item__thumb--text" aria-hidden="true">
                    <?php echo esc_html(function_exists('mb_substr') ? mb_substr($primary_label, 0, 1) : substr($primary_label, 0, 1)); ?>
                </span>
            <?php endif; ?>
        </div>
        <div class="ll-wordset-recorder-queue-item__content">
            <div class="ll-wordset-recorder-queue-item__title-row">
                <span class="ll-wordset-recorder-queue-item__title"><?php echo esc_html($primary_label); ?></span>
                <?php if ($show_category) : ?>
                    <span class="ll-wordset-settings-card__pill ll-wordset-recorder-queue-item__category"><?php echo esc_html($category_name); ?></span>
                <?php endif; ?>
            </div>
            <?php if ($secondary_label !== '') : ?>
                <p class="ll-wordset-recorder-queue-item__secondary"><?php echo esc_html($secondary_label); ?></p>
            <?php endif; ?>
            <?php if (!empty($type_labels)) : ?>
                <div class="ll-wordset-recorder-queue-item__types">
                    <?php foreach ($type_labels as $type_label) : ?>
                        <span class="ll-wordset-settings-card__pill ll-wordset-recorder-queue-item__type"><?php echo esc_html($type_label); ?></span>
                    <?php endforeach; ?>
                </div>
            <?php endif; ?>
            <?php if ($can_save_prompts) : ?>
                <details class="ll-wordset-recorder-queue-prompts"<?php echo $prompts_open ? ' open' : ''; ?>>
                    <summary><?php echo esc_html__('Recording prompts', 'll-tools-text-domain'); ?></summary>
                    <form method="post" action="<?php echo esc_url($action_url); ?>" class="ll-wordset-recorder-queue-prompts__form" data-ll-recorder-queue-autosave="prompts">
                        ...
                        <input type="hidden" name="ll_wordset_manager_recorder_queue_user_id" value="<?php echo esc_attr($recorder_user_id); ?>" />
                        <input type="hidden" name="ll_wordset_manager_recorder_queue_hide_key" value="<?php echo esc_attr($hide_key); ?>" />
                        <input type="hidden" name="ll_wordset_manager_recorder_queue_word_id" value="<?php echo esc_attr($word_id); ?>" />
                        <input type="hidden" name="ll_wordset_manager_recorder_queue_image_id" value="<?php echo esc_attr($image_id); ?>" />
                        <input type="hidden" name="ll_wordset_manager_recorder_queue_prompt_card_id" value="<?php echo esc_attr($prompt_card_id); ?>" />
                        <input type="hidden" name="ll_wordset_manager_recorder_queue_title" value="<?php echo esc_attr($title_value); ?>" />
                        <input type="hidden" name="ll_wordset_manager_recorder_queue_category_name" value="<?php echo esc_attr($category_name); ?>" />
                        <input type="hidden" name="ll_wordset_manager_recorder_queue_category_slug" value="<?php echo esc_attr($category_slug); ?>" />
                        <input type="hidden" name="ll_wordset_back" value="<?php echo esc_attr($back_url); ?>" />
                        <input type="hidden" name="ll_wordset_page" value="<?php echo esc_attr($wordset_slug); ?>" />
                        <input type="hidden" name="ll_wordset_view" value="settings" />
                        <input type="hidden" name="ll_wordset_tool" value="<?php echo esc_attr($tool); ?>" />
                        ...
                        <div class="ll-wordset-recorder-queue-prompts__grid">
                            <?php foreach ($prompt_type_slugs as $prompt_type_slug) : ?>
                                <?php
                                $prompt_type_slug = sanitize_title((string) $prompt_type_slug);
                                if ($prompt_type_slug === '') {
                                    continue;
                                }
                                $prompt_label = (string) ($prompt_type_labels[$prompt_type_slug] ?? ucwords(str_replace('-', ' ', $prompt_type_slug)));
                                ?>
                                <label class="ll-wordset-recorder-queue-prompts__field">
                                    <span><?php echo esc_html($prompt_label); ?></span>
                                    <textarea name="ll_wordset_manager_recorder_queue_prompts[<?php echo esc_attr($prompt_type_slug); ?>]" rows="2" placeholder="<?php echo esc_attr__('Suggested phrasing for the recorder', 'll-tools-text-domain'); ?>"><?php echo esc_textarea((string) ($prompt_hints[$prompt_type_slug] ?? '')); ?></textarea>
                                </label>
                            <?php endforeach; ?>
                        </div>
                        ...
                        <button type="submit" class="ll-wordset-settings-action ll-wordset-settings-action--secondary ll-wordset-recorder-queue-prompts__save">
                            <?php echo esc_html__('Save prompts', 'll-tools-text-domain'); ?>
                        </button>
                    </form>
                </details>
            <?php endif; ?>
            <?php if ($edit_url !== '') : ?>
                <a class="ll-wordset-recorder-queue-item__edit-link" href="<?php echo esc_url($edit_url); ?>">
                    <?php echo esc_html__('Edit word', 'll-tools-text-domain'); ?>
                </a>
            <?php endif; ?>
            <?php if ($is_hidden && $is_stale) : ?>
                <p class="ll-wordset-recorder-queue-item__note"><?php echo esc_html__('Not in the current queue right now.', 'll-tools-text-domain'); ?></p>
            <?php endif; ?>
        </div>
        <form method="post" action="<?php echo esc_url($action_url); ?>" class="ll-wordset-recorder-queue-item__form">
            <input type="hidden" name="ll_wordset_manager_recorder_queue_action" value="<?php echo esc_attr($action); ?>" />
            <input type="hidden" name="ll_wordset_manager_recorder_queue_wordset_id" value="<?php echo esc_attr($wordset_id); ?>" />
            <input type="hidden" name="ll_wordset_manager_recorder_queue_user_id" value="<?php echo esc_attr($recorder_user_id); ?>" />
            <input type="hidden" name="ll_wordset_manager_recorder_queue_hide_key" value="<?php echo esc_attr($hide_key); ?>" />
            <input type="hidden" name="ll_wordset_manager_recorder_queue_word_id" value="<?php echo esc_attr($word_id); ?>" />
            ...
            <input type="hidden" name="ll_wordset_tool" value="<?php echo esc_attr($tool); ?>" />
            <?php wp_nonce_field('ll_wordset_manager_recorder_queue_' . $wordset_id, 'll_wordset_manager_recorder_queue_nonce'); ?>
            <button type="submit" class="ll-study-btn ll-vocab-lesson-mode-button ll-wordset-recorder-queue-item__action">
                <?php echo esc_html($action_label); ?>
            </button>
        </form>
    </li>
...
function ll_tools_wordset_page_render_recorder_queue_category_group(array $category_group, array $args): string {
    ...
    <section class="ll-wordset-recorder-queue-category-view" aria-labelledby="<?php echo esc_attr($list_id . '-title'); ?>">
        ...
        <ul id="<?php echo esc_attr($list_id); ?>" class="ll-wordset-recorder-queue-list">
            <?php foreach ($items as $item) : ?>
                <?php
                echo ll_tools_wordset_page_render_recorder_queue_item($item, $args + [
                    'show_category' => false,
                    'prompts_open' => true,
                ]); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
                ?>
...
function ll_tools_wordset_page_render_recorder_queue_settings_form(array $queue_row, array $args): string {
    ...
    if ($action_url === '' || $wordset_id <= 0 || $queue_user_id <= 0 || $wordset_slug === '') {
        return '';
    }

    $type_options = (array) ($args['recording_type_options'] ?? []);
    $include_slugs = ll_tools_wordset_page_recorder_queue_type_csv_to_slugs((string) ($queue_row['include_recording_types'] ?? ''));
    $exclude_slugs = ll_tools_wordset_page_recorder_queue_type_csv_to_slugs((string) ($queue_row['exclude_recording_types'] ?? ''));
    $include_lookup = array_fill_keys($include_slugs, true);
    $exclude_lookup = array_fill_keys($exclude_slugs, true);
    $settings_id = 'll-recorder-queue-settings-' . $queue_user_id;

    ob_start();
    ?>
    <details class="ll-wordset-recorder-queue-settings" id="<?php echo esc_attr($settings_id); ?>">
        <summary class="ll-wordset-recorder-queue-settings__summary">
            <?php echo esc_html__('Change queue settings', 'll-tools-text-domain'); ?>
        </summary>
        <form method="post" action="<?php echo esc_url($action_url); ?>" class="ll-wordset-recorder-queue-settings__form" data-ll-recorder-queue-autosave="settings">
            <input type="hidden" name="ll_wordset_manager_recorder_queue_action" value="save_settings" />
            <input type="hidden" name="ll_wordset_manager_recorder_queue_wordset_id" value="<?php echo esc_attr($wordset_id); ?>" />
            <input type="hidden" name="ll_wordset_manager_recorder_queue_user_id" value="<?php echo esc_attr($queue_user_id); ?>" />
            <input type="hidden" name="ll_wordset_back" value="<?php echo esc_attr($back_url); ?>" />
            <input type="hidden" name="ll_wordset_page" value="<?php echo esc_attr($wordset_slug); ?>" />
            <input type="hidden" name="ll_wordset_view" value="settings" />
            <input type="hidden" name="ll_wordset_tool" value="recorder-queues" />
            ...
            <div class="ll-wordset-recorder-queue-settings__grid">
                <fieldset class="ll-wordset-recorder-queue-settings__fieldset">
                    <legend><?php echo esc_html__('Only include types', 'll-tools-text-domain'); ?></legend>
                    <p class="description"><?php echo esc_html__('Leave all unchecked to include every recording type.', 'll-tools-text-domain'); ?></p>
                    <div class="ll-wordset-recorder-queue-settings__checks">
                        <?php foreach ($type_options as $type_slug => $type_label) : ?>
                            <?php $type_slug = sanitize_title((string) $type_slug); ?>
                            <?php if ($type_slug === '') { continue; } ?>
                            <label>
                                <input type="checkbox" name="ll_wordset_manager_recorder_queue_include_types[]" value="<?php echo esc_attr($type_slug); ?>" <?php checked(!empty($include_lookup[$type_slug])); ?> />
                ...
                <fieldset class="ll-wordset-recorder-queue-settings__fieldset">
                    <legend><?php echo esc_html__('Skipped types', 'll-tools-text-domain'); ?></legend>
                    <p class="description"><?php echo esc_html__('These are hidden from the recorder queue unless the only-include list is set.', 'll-tools-text-domain'); ?></p>
                    <div class="ll-wordset-recorder-queue-settings__checks">
                        <?php foreach ($type_options as $type_slug => $type_label) : ?>
                            <?php $type_slug = sanitize_title((string) $type_slug); ?>
                            <?php if ($type_slug === '') { continue; } ?>
                            <label>
                                <input type="checkbox" name="ll_wordset_manager_recorder_queue_exclude_types[]" value="<?php echo esc_attr($type_slug); ?>" <?php checked(!empty($exclude_lookup[$type_slug])); ?> />
                                <span><?php echo esc_html((string) $type_label); ?></span>
                            </label>
                        <?php endforeach; ?>
                    </div>
...
function ll_tools_wordset_page_render_settings_recorder_queues_tool(
    WP_Term $wordset_term,
    int $wordset_id,
    string $back_url,
    array $recorder_queue_rows
): string {
    $action_url = ll_tools_get_wordset_settings_tool_url($wordset_term, 'recorder-queues', $back_url);
    $recorder_access_url = ll_tools_get_wordset_settings_tool_url($wordset_term, 'recorder', $back_url);
    $editor_base_url = ll_tools_get_wordset_settings_tool_url($wordset_term, 'editor', $back_url);
    $hidden_view = isset($_GET['ll_recorder_queue_view'])
    ...
    foreach ($recorder_queue_rows as $queue_row) {
        if (!is_array($queue_row)) {
            continue;
        }
        $visible_total += count((array) ($queue_row['visible_items'] ?? []));
        $hidden_total += count((array) ($queue_row['hidden_items'] ?? []));
    }
    ...
    <section class="ll-wordset-settings-page ll-wordset-settings-page--tool" data-ll-wordset-settings-page>
        <div class="ll-wordset-settings-card">
            <h2 class="ll-wordset-settings-card__title"><?php echo esc_html__('Recorder Queues', 'll-tools-text-domain'); ?></h2>
            <p class="description" style="margin-top:0;">
                <?php
                echo esc_html($hidden_view
                    ? __('Review hidden recorder queue items and restore anything that should be recordable again.', 'll-tools-text-domain')
                    : __('Review each assigned recorder\'s queue by category, edit prompt wording, and tune their recording settings from one place.', 'll-tools-text-domain'));
                ?>
            </p>
            <div class="ll-wordset-settings-card__meta">
                <span class="ll-wordset-settings-card__pill">
                    <?php
                    echo esc_html(sprintf(
                        _n('%d recorder', '%d recorders', $recorder_count, 'll-tools-text-domain'),
                        $recorder_count
                    ));
                    ?>
                </span>
                <span class="ll-wordset-settings-card__pill">
                    <?php
                    echo esc_html(sprintf(
                        _n('%d queued word', '%d queued words', $visible_total, 'll-tools-text-domain'),
                        $visible_total
                    ));
                    ?>
                </span>
                <a class="ll-wordset-settings-card__pill ll-wordset-recorder-queue-view-link" href="<?php echo esc_url($hidden_view ? $queue_url : $hidden_url); ?>">
                    <?php
                    echo esc_html($hidden_view
                        ? __('Back to queue', 'll-tools-text-domain')
                        : sprintf(
                            /* translators: %d: hidden queue item count */
                            _n('Hidden (%d)', 'Hidden (%d)', $hidden_total, 'll-tools-text-domain'),
                            $hidden_total
                        ));
                    ?>
                </a>
        ...
        <?php if (empty($recorder_queue_rows)) : ?>
            <div class="ll-wordset-settings-card">
                <p class="ll-wordset-settings-empty"><?php echo esc_html__('No audio recorder users are assigned to this word set yet.', 'll-tools-text-domain'); ?></p>
                <div class="ll-wordset-empty__actions">
                    <a class="ll-study-btn ll-vocab-lesson-mode-button" href="<?php echo esc_url($recorder_access_url); ?>">
                        <?php echo esc_html__('Open Recorder Access', 'll-tools-text-domain'); ?>
                    </a>
                </div>
            </div>
        <?php else : ?>
            <?php foreach ($recorder_queue_rows as $queue_row) : ?>
                <?php
                if (!is_array($queue_row)) {
                    continue;
                }
                ...
                $queue_notes = array_values(array_filter(array_map('strval', (array) ($queue_row['queue_notes'] ?? []))));
                $visible_items = array_values(array_filter((array) ($queue_row['visible_items'] ?? []), 'is_array'));
                $hidden_items = array_values(array_filter((array) ($queue_row['hidden_items'] ?? []), 'is_array'));
                ...
                <article class="ll-wordset-settings-card ll-wordset-recorder-queue-card" id="<?php echo esc_attr('ll-recorder-queue-' . $queue_user_id); ?>">
                    <div class="ll-wordset-recorder-queue-card__head">
                        <div class="ll-wordset-recorder-queue-card__identity">
                            <h3 class="ll-wordset-recorder-queue-card__title"><?php echo esc_html($queue_display_name); ?></h3>
                            <p class="ll-wordset-recorder-queue-card__identity-meta">
                                <?php if ($queue_login !== '') : ?>
                                    <span><?php echo esc_html('@' . $queue_login); ?></span>
                                <?php endif; ?>
                                <?php if ($queue_email !== '') : ?>
                                    <span><?php echo esc_html($queue_email); ?></span>
                                <?php endif; ?>
                            </p>
                        </div>
                        <div class="ll-wordset-settings-card__meta ll-wordset-recorder-queue-card__summary">
                            <span class="ll-wordset-settings-card__pill">
                                <?php
                                echo esc_html(sprintf(
                                    _n('%d queued word', '%d queued words', count($visible_items), 'll-tools-text-domain'),
                                    count($visible_items)
                                ));
                                ?>
                            </span>
                            <?php if (!$hidden_view) : ?>
                                <a class="ll-wordset-settings-card__pill ll-wordset-recorder-queue-view-link" href="<?php echo esc_url($row_hidden_url); ?>">
                                    <?php
                                    echo esc_html(sprintf(
                                        /* translators: %d: hidden queue item count */
                                        _n('Hidden (%d)', 'Hidden (%d)', count($hidden_items), 'll-tools-text-domain'),
                                        count($hidden_items)
                                    ));
                                    ?>
                                </a>
                    ...
                    <?php if (!empty($queue_notes)) : ?>
                        <div class="ll-wordset-settings-card__meta ll-wordset-recorder-queue-card__notes">
                            <?php foreach ($queue_notes as $queue_note) : ?>
                                <span class="ll-wordset-settings-card__pill"><?php echo esc_html($queue_note); ?></span>
                            <?php endforeach; ?>
                        </div>
                    <?php endif; ?>
                    ...
                    <?php else : ?>
                        <section class="ll-wordset-recorder-queue-column ll-wordset-recorder-queue-column--hidden-view">
                            ...
                            <?php if (empty($hidden_groups)) : ?>
                                <p class="ll-wordset-settings-empty"><?php echo esc_html__('No hidden words for this recorder.', 'll-tools-text-domain'); ?></p>
                            <?php else : ?>
                                <div class="ll-wordset-recorder-queue-categories">
                                    <?php foreach ($hidden_groups as $hidden_group) : ?>
                                        <?php
                                        echo ll_tools_wordset_page_render_recorder_queue_category_group($hidden_group, [
                                            'action' => 'unhide',
                                            'action_url' => $action_url,
                                            'wordset_id' => $wordset_id,
                                            'recorder_user_id' => $queue_user_id,
                                            ...
                                            'tool' => 'recorder-queues',
                                            'editor_base_url' => $editor_base_url,
                                            'category_open' => count($hidden_groups) === 1,
                                        ]); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
                                        ?>
                                    <?php endforeach; ?>
                                </div>
...
function ll_tools_render_wordset_page_content($wordset, array $args = []): string {
    ...
    $categories = ll_tools_get_wordset_page_categories($wordset_id, $preview_limit, [
        'defer_previews' => $defer_main_category_previews,
    ]);
    ...
    $settings_tool_urls = [
        ...
        'recorder-queues' => ll_tools_get_wordset_settings_tool_url($wordset_term, 'recorder-queues', $settings_navigation_back_url),
    ...
    if ($is_main_view && count($mixed_lesson_cards) > ll_tools_wordset_page_get_lazy_card_batch_size()) {
        ...
        if ($lazy_cards_user_id <= 0) {
            ...
            $wordset_epoch = function_exists('ll_tools_get_wordset_cache_epoch')
                ? max(1, (int) ll_tools_get_wordset_cache_epoch())
                : 1;
            $lazy_cards_token_hint = ll_tools_wordset_page_build_lazy_cards_token([
                'schema' => 2,
                'wordset_id' => $wordset_id,
                'preview_limit' => max(1, (int) $preview_limit),
                'batch_size' => $lazy_batch_size,
                'locale' => sanitize_key((string) get_locale()),
                'main_sort' => sanitize_key((string) $saved_main_category_sort),
                'translation_language' => sanitize_key((string) $translation_language),
                'category_translation_enabled' => $category_translation_enabled ? '1' : '0',
                'gender_enabled' => $gender_enabled ? '1' : '0',
                'card_count' => count($mixed_lesson_cards),
                'category_epoch' => $category_epoch,
                'wordset_epoch' => $wordset_epoch,
            ]);
        }
        $lazy_payload = [
            'cards' => $remaining_lazy_cards,
    ...
    if ($view === 'settings' && $can_manage_wordset_content) {
        ...
        $settings_hub_cards[] = [
            'tool' => 'recorder-queues',
            'label' => ll_tools_wordset_settings_tool_label('recorder-queues'),
            'description' => ll_tools_wordset_settings_tool_description('recorder-queues'),
            'status' => $assigned_recorder_count > 0
                ? sprintf(
                    _n('%d recorder ready', '%d recorders ready', $assigned_recorder_count, 'll-tools-text-domain'),
                    $assigned_recorder_count
                )
                : __('No recorders assigned', 'll-tools-text-domain'),
            'url' => $assigned_recorder_count > 0 ? $settings_tool_urls['recorder-queues'] : '',
            'enabled' => ($assigned_recorder_count > 0),
        ];
    ...
    $initial_category_preview_lookup = [];
    foreach ((array) $initial_mixed_lesson_cards as $initial_card) {
        if (!is_array($initial_card) || ($initial_card['type'] ?? '') !== 'category' || !isset($initial_card['data']) || !is_array($initial_card['data'])) {
            continue;
        }

        $initial_category = $initial_card['data'];
        $initial_category_id = isset($initial_category['id']) ? (int) $initial_category['id'] : 0;
        if ($initial_category_id <= 0) {
            continue;
        }

        $initial_category_preview_lookup[$initial_category_id] = [
            'preview' => isset($initial_category['preview']) && is_array($initial_category['preview']) ? $initial_category['preview'] : [],
            'has_images' => !empty($initial_category['has_images']),
            'preview_aspect_ratio' => isset($initial_category['preview_aspect_ratio']) ? (string) $initial_category['preview_aspect_ratio'] : '',
        ];
    }
    ...
    foreach (array_values($enhanced_categories) as $index => $category_row) {
        ...
        $category_id = isset($category_row['id']) ? (int) $category_row['id'] : 0;
        if ($category_id > 0 && isset($initial_category_preview_lookup[$category_id])) {
            $category_row = array_merge($category_row, $initial_category_preview_lookup[$category_id]);
        }
    ...
    $script_categories = array_map(function (array $cat) use ($category_metrics_lookup): array {
        ...
        return [
            ...
            'public_note_label' => (string) ($cat['public_note_label'] ?? ''),
            'word_image_count' => max(0, (int) ($cat['word_image_count'] ?? 0)),
            'prompt_card_count' => max(0, (int) ($cat['prompt_card_count'] ?? 0)),
            'content_count' => max(0, (int) ($cat['content_count'] ?? 0)),
            'can_manage_inactive' => !empty($cat['can_manage_inactive']),
            'can_hide' => !empty($cat['can_hide']),
            'can_preview' => !empty($cat['can_preview']),
            'can_delete' => !empty($cat['can_delete']),
    ...
    wp_localize_script('ll-wordset-pages-js', 'llWordsetPageData', [
        ...
        'i18n' => [
            ...
            'recorderQueueSaving' => __('Saving...', 'll-tools-text-domain'),
            'recorderQueueSaved' => __('Saved!', 'll-tools-text-domain'),
            'recorderQueueSaveError' => __('Unable to save right now.', 'll-tools-text-domain'),
            ...$games_i18n,
    ...
    <<?php echo esc_html($wrapper_tag); ?>
        ...
        <?php elseif ($view === 'settings') : ?>
            ...
            <?php elseif ($settings_tool === 'recorder-queues' && $can_manage_wordset_content) : ?>
                <?php echo ll_tools_wordset_page_render_settings_recorder_queues_tool($wordset_term, $wordset_id, $back_url, $recorder_queue_rows); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>
        ...
        <?php else : ?>
            ...
            <?php endif; ?>
            <?php if (is_array($inactive_category_notice) && !empty($inactive_category_notice['message'])) : ?>
                <div
                    class="ll-wordset-progress-reset-notice ll-wordset-progress-reset-notice--<?php echo esc_attr(($inactive_category_notice['type'] ?? 'error') === 'success' ? 'success' : 'error'); ?>"
                    role="<?php echo esc_attr(($inactive_category_notice['type'] ?? 'error') === 'success' ? 'status' : 'alert'); ?>">
                    <?php echo esc_html((string) $inactive_category_notice['message']); ?>
                </div>
            <?php endif; ?>
...
function ll_tools_wordset_page_handle_category_search_ajax(): void {
    check_ajax_referer('ll_tools_wordset_page_category_search', 'nonce');
    ...
    $query = isset($_POST['query'])
        ? ll_tools_wordset_page_prepare_category_search_query((string) wp_unslash($_POST['query']))
        : '';

    $payload = ll_tools_wordset_page_get_category_search_payload($token);
    if (!is_array($payload)) {
        wp_send_json_error([
            'message' => __('Could not search this word set right now.', 'll-tools-text-domain'),
        ], 410);
    }

    $payload_wordset_id = isset($payload['wordset_id']) ? (int) $payload['wordset_id'] : 0;
    if ($wordset_id <= 0 || $payload_wordset_id !== $wordset_id) {
        wp_send_json_error([
            'message' => __('Could not search this word set right now.', 'll-tools-text-domain'),
        ], 403);
    }
    ...
    if ($payload_user_id !== get_current_user_id()) {
        wp_send_json_error([
            'message' => __('Could not search this word set right now.', 'll-tools-text-domain'),
        ], 403);
    }

    $allowed_category_ids = isset($payload['category_ids']) && is_array($payload['category_ids'])
        ? array_values(array_filter(array_unique(array_map('intval', $payload['category_ids'])), static function (int $id): bool {
            return $id > 0;
        }))
        : [];
    sort($allowed_category_ids, SORT_NUMERIC);
    ...
    $cached_response = ll_tools_wordset_page_category_search_ajax_cache_get($cache_args);
    if (is_array($cached_response)) {
        ll_tools_wordset_page_category_search_ajax_send_cache_header('HIT');
        wp_send_json_success($cached_response);
    }

    if (ll_tools_wordset_page_category_search_cache_miss_limited($token)) {
        wp_send_json_error([
            'message' => __('Too many search requests. Please wait a few minutes and try again.', 'll-tools-text-domain'),
        ], 429);
    }
    ...
    ll_tools_wordset_page_category_search_ajax_cache_set($cache_args, $response);
    ll_tools_wordset_page_category_search_ajax_send_cache_header('MISS');
    wp_send_json_success($response);
}
add_action('wp_ajax_ll_tools_wordset_page_category_search', 'll_tools_wordset_page_handle_category_search_ajax');
add_action('wp_ajax_nopriv_ll_tools_wordset_page_category_search', 'll_tools_wordset_page_handle_category_search_ajax');

function ll_tools_wordset_page_handle_lazy_cards_ajax(): void {
    check_ajax_referer('ll_tools_wordset_page_lazy_cards', 'nonce');
    ...
    $payload_user_id = isset($payload['user_id']) ? (int) $payload['user_id'] : 0;
    if (!$used_fallback_payload && $payload_user_id !== get_current_user_id()) {
        wp_send_json_error([
            'message' => __('Could not load more cards right now.', 'll-tools-text-domain'),
        ], 403);
    }
    ...
    if ($specific_card_request) {
        $payload_ids = ll_tools_wordset_page_collect_lazy_card_ids($cards);
        $requested_category_ids = ll_tools_wordset_page_filter_lazy_cards_requested_ids(
            $requested_category_ids,
            (array) ($payload_ids['category_ids'] ?? [])
        );
        $requested_content_ids = ll_tools_wordset_page_filter_lazy_cards_requested_ids(
            $requested_content_ids,
            (array) ($payload_ids['content_ids'] ?? [])
        );
    }

    $public_cache_args = [
        'mode' => $specific_card_request ? 'ids' : 'offset',
        'token' => $token,
        'wordset_id' => $wordset_id,
        'preview_limit' => $preview_limit,
        'offset' => $offset,
    ...
    if (ll_tools_wordset_page_lazy_cards_ajax_cache_enabled($token)) {
        $lock_ttl = max(5, (int) apply_filters('ll_tools_wordset_page_lazy_cards_ajax_cache_build_lock_ttl', 15));
        $public_cache_lock_acquired = ll_tools_wordset_page_acquire_lazy_cards_ajax_cache_lock($public_cache_args, $lock_ttl);
        if (!$public_cache_lock_acquired) {
            $wait_ms = max(0, (int) apply_filters('ll_tools_wordset_page_lazy_cards_ajax_cache_build_wait_ms', 1000));
            $cached_after_wait = ll_tools_wordset_page_wait_for_lazy_cards_ajax_cached_response($public_cache_args, $wait_ms);
            if (is_array($cached_after_wait)) {
                ll_tools_wordset_page_lazy_cards_ajax_send_cache_header('WAIT_HIT');
