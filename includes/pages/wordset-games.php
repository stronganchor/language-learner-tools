...
function ll_tools_wordset_games_lineup_launch_sequence_cap(): int {
    return max(1, min(10, (int) apply_filters('ll_tools_wordset_games_lineup_launch_sequence_cap', 3)));
}

function ll_tools_wordset_games_lineup_launch_word_cap(): int {
    $minimum = ll_tools_wordset_games_lineup_min_sequence_length();
    return max($minimum, min(60, (int) apply_filters('ll_tools_wordset_games_lineup_launch_word_cap', 24)));
}
...
function ll_tools_wordset_games_unscramble_max_unit_count(): int {
    return max(
        ll_tools_wordset_games_unscramble_min_tile_count(),
        (int) apply_filters('ll_tools_wordset_games_unscramble_max_unit_count', 14)
    );
}
...
function ll_tools_wordset_games_default_catalog(): array {
    ...
    if ($wordset_term instanceof WP_Term && !is_wp_error($wordset_term)) {
        return ll_tools_wordset_games_build_catalog((int) $wordset_term->term_id, get_current_user_id(), false);
    }
...
function ll_tools_wordset_games_supported_image_word_lookup(array $word_ids, array $has_image_by_word): array {
    ...
    foreach ($word_ids as $word_id) {
        if (!empty($has_image_by_word[$word_id])) {
            $lookup[$word_id] = true;
        }
    }
    if (empty($lookup) || !function_exists('ll_tools_get_effective_word_image_attachment_id_for_word') || !function_exists('ll_tools_is_attachment_animated_webp')) {
        return $lookup;
    }

    foreach (array_keys($lookup) as $word_id) {
        $attachment_id = (int) ll_tools_get_effective_word_image_attachment_id_for_word((int) $word_id, true);
        if ($attachment_id <= 0 || !ll_tools_is_attachment_animated_webp($attachment_id)) {
            continue;
        }

        $allow_animated_webp = (bool) apply_filters('ll_tools_wordset_games_allow_animated_webp_images', false, [
            'id' => (int) $word_id,
            'image_is_animated_webp' => true,
        ]);
        if (!$allow_animated_webp) {
            unset($lookup[$word_id]);
        }
    }
...
function ll_tools_wordset_games_unscramble_normalize_compare_text(string $text): string {
    $text = trim((string) preg_replace('/[\p{P}\p{S}]+/u', '', $text));
...
function ll_tools_wordset_games_prepare_unscramble_answer_text(string $text): string {
    $text = trim((string) preg_replace('/[\p{P}\p{S}]+/u', '', $text));
    $text = trim((string) preg_replace('/\s+/u', ' ', $text));
    return trim($text);
}
...
function ll_tools_wordset_games_resolve_unscramble_display_texts(array $word): array {
    $word_id = (int) ($word['id'] ?? 0);
    $wordset_ids = isset($word['wordset_ids']) && is_array($word['wordset_ids'])
        ? array_values(array_filter(array_map('intval', $word['wordset_ids']), static function (int $id): bool {
            return $id > 0;
        }))
        : [];
    if (empty($wordset_ids) && $word_id > 0 && function_exists('ll_tools_get_post_wordset_ids')) {
        $wordset_ids = array_values(array_filter(array_map('intval', (array) ll_tools_get_post_wordset_ids($word_id)), static function (int $id): bool {
            return $id > 0;
        }));
    }
...
function ll_tools_wordset_games_resolve_unscramble_prompt(array $word, string $answer_text = ''): array {
    ...
    foreach ($candidates as $candidate) {
        ...
        if (ll_tools_wordset_games_unscramble_normalize_compare_text($candidate) === $normalized_answer) {
            continue;
        }
...
function ll_tools_wordset_games_build_unscramble_word_entry(array $word, int $wordset_id): ?array {
    ...
    $maximum_unit_count = ll_tools_wordset_games_unscramble_max_unit_count();
    if (
        $movable_unit_count < $minimum_tile_count
        || $movable_unit_count > $maximum_tile_count
        || $unit_count > $maximum_unit_count
    ) {
        return null;
    }

    $prompt = ll_tools_wordset_games_resolve_unscramble_prompt($word, $answer_text);
    if (($prompt['type'] ?? '') === '') {
        return null;
    }
    ...
    $word['unscramble_unit_count'] = $unit_count;
...
function ll_tools_wordset_games_unscramble_candidate_word_cap(int $launch_word_cap): int {
    $minimum = ll_tools_wordset_games_min_word_count();
    $default = max($minimum, $launch_word_cap * 3);
    return max(
        $minimum,
        min(500, (int) apply_filters('ll_tools_wordset_games_unscramble_candidate_word_cap', $default, $launch_word_cap))
    );
}

function ll_tools_wordset_games_count_wordset_category_words(int $wordset_id, array $category_ids): int {
    global $wpdb;
    ...
    $sql = $wpdb->prepare(
        ...
        array_merge([$wordset_id], $category_ids)
    );

    return max(0, (int) $wpdb->get_var($sql));
}

function ll_tools_wordset_games_collect_unscramble_candidate_rows(
    array $word_ids,
    int $user_id,
    array $visible_category_ids
): array {
    $word_ids = array_values(array_unique(array_filter(array_map('intval', $word_ids), static function (int $word_id): bool {
        return $word_id > 0;
    })));
    $visible_category_ids = array_values(array_filter(array_map('intval', $visible_category_ids), static function (int $category_id): bool {
        return $category_id > 0;
    }));
    if (empty($word_ids) || empty($visible_category_ids)) {
        return [];
    }

    $visible_category_lookup = array_fill_keys($visible_category_ids, true);
    $category_ids_by_word = function_exists('ll_tools_get_object_term_ids_map')
        ? ll_tools_get_object_term_ids_map($word_ids, 'word-category')
        : [];
    $progress_rows = ($user_id > 0 && function_exists('ll_tools_get_user_word_progress_rows'))
        ? ll_tools_get_user_word_progress_rows($user_id, $word_ids)
        : [];

    $rows = [];
    foreach ($word_ids as $word_id) {
        $word_category_ids = isset($category_ids_by_word[$word_id]) && is_array($category_ids_by_word[$word_id])
            ? array_values(array_filter(array_map('intval', $category_ids_by_word[$word_id]), static function (int $category_id) use ($visible_category_lookup): bool {
                return $category_id > 0 && !empty($visible_category_lookup[$category_id]);
            }))
            : [];
        ...
        $rows[] = [
            'id' => $word_id,
            'category_id' => (int) reset($word_category_ids),
            'category_ids' => $word_category_ids,
            'progress_status' => function_exists('ll_tools_user_progress_word_status')
...
function ll_tools_wordset_games_prepare_unscramble_candidates(
    int $wordset_id,
    int $user_id,
    ...
    $collected = ll_tools_wordset_games_collect_visible_speaking_words_for_candidate_pool(
        $wordset_id,
        $user_id,
        'unscramble',
        $candidate_rows
    ...
    $prepared = [];
    foreach ((array) ($collected['words'] ?? []) as $word) {
        if (!is_array($word)) {
            continue;
        }
        $entry = ll_tools_wordset_games_build_unscramble_word_entry($word, $wordset_id);
        if (is_array($entry)) {
            $prepared[] = $entry;
        }
    }
...
function ll_tools_wordset_games_collect_unscramble_frontier_candidate_rows(
    int $wordset_id,
    int $user_id,
    array $visible_categories,
    array $visible_category_ids,
    int $candidate_cap,
    array $exclude_word_ids = []
): array {
    $frontier_category_ids = ll_tools_wordset_games_lowest_frontier_category_ids($wordset_id, $visible_categories);
    $ordered_category_ids = array_values(array_unique(array_merge($frontier_category_ids, $visible_category_ids)));
    $exclude_lookup = array_fill_keys(array_map('intval', $exclude_word_ids), true);
    $candidate_ids = [];
    $scan_cap = max($candidate_cap, min(1500, $candidate_cap * 3));
    $scanned = 0;

    foreach ($ordered_category_ids as $category_id) {
        $offset = 0;
        while (count($candidate_ids) < $candidate_cap && $scanned < $scan_cap) {
            $batch_limit = min($candidate_cap, $scan_cap - $scanned);
            $batch_ids = ll_tools_wordset_games_query_word_ids_for_category_batch(
                $wordset_id,
                (int) $category_id,
                $batch_limit,
                $offset
            ...
            foreach ($batch_ids as $word_id) {
                $word_id = (int) $word_id;
                if ($word_id <= 0 || isset($exclude_lookup[$word_id])) {
                    continue;
                }
                $candidate_ids[] = $word_id;
                $exclude_lookup[$word_id] = true;
                if (count($candidate_ids) >= $candidate_cap) {
                    break 2;
                }
            }
            if (count($batch_ids) < $batch_limit) {
                break;
            }
        }
        if ($scanned >= $scan_cap) {
    ...
    return ll_tools_wordset_games_collect_unscramble_candidate_rows(
        $candidate_ids,
        $user_id,
...
function ll_tools_wordset_games_build_unscramble_pool(int $wordset_id, int $user_id = 0): array {
    ...
    $categories = ll_tools_wordset_games_visible_categories($wordset_id, $uid, 'unscramble');
    $visible_category_ids = array_values(array_filter(array_map(static function ($row): int {
        return is_array($row) ? (int) ($row['id'] ?? 0) : 0;
    }, $categories), static function (int $category_id): bool {
        return $category_id > 0;
    }));
    if (empty($visible_category_ids)) {
        return [
            ...
            'launch_word_cap' => $launch_word_cap,
            'launch_word_count' => 0,
            'pool_source' => 'no_visible_categories',
            'category_ids' => [],
            'enabled_category_count' => 0,
            'words' => [],
        ];
    ...
    $candidate_cap = ll_tools_wordset_games_unscramble_candidate_word_cap($launch_word_cap);
    $progress_word_ids = ll_tools_wordset_games_query_progress_word_ids_for_wordset(
        $wordset_id,
        $uid,
        $candidate_cap,
        0
    );
    $progress_candidates = ll_tools_wordset_games_collect_unscramble_candidate_rows(
        $progress_word_ids,
        $uid,
        $visible_category_ids
    );
    $prepared_progress_words = ll_tools_wordset_games_prepare_unscramble_candidates(
        $wordset_id,
        $uid,
        $progress_candidates
    );
    ...
    $studied_words = [];
    $mastered_words = [];
    foreach ($prepared_progress_words as $prepared_word) {
        $status = (string) ($prepared_word['progress_status'] ?? 'new');
        if ($status === 'studied') {
    ...
    $has_recorded_progress = !empty($prepared_progress_words);
    ...
    if (count($pool) < $minimum_word_count && !$has_recorded_progress) {
        $frontier_candidates = ll_tools_wordset_games_collect_unscramble_frontier_candidate_rows(
            $wordset_id,
            $uid,
            $categories,
            $visible_category_ids,
        ...
        $frontier_candidate_count = count($frontier_candidates);
        $frontier_truncated = $frontier_candidate_count >= $candidate_cap;
        $eligible_words = ll_tools_wordset_games_prepare_unscramble_candidates(
            $wordset_id,
            $uid,
            $frontier_candidates
        );
    ...
    if ($pool_source === 'frontier_new' && $frontier_truncated && $available_word_count >= $minimum_word_count) {
        $available_word_count = max(
            $available_word_count,
            ll_tools_wordset_games_count_wordset_category_words($wordset_id, $visible_category_ids)
        );
    }
    $launch_words = ll_tools_wordset_games_limit_launch_words((array) $pool, $launch_word_cap);
    if (!empty($launch_words) && function_exists('ll_tools_attach_user_practice_progress_to_words')) {
        $launch_words = ll_tools_attach_user_practice_progress_to_words($launch_words, $uid);
    }
    ...
    return [
        ...
        'candidate_word_cap' => $candidate_cap,
        'candidate_word_count' => $has_recorded_progress ? count($progress_candidates) : $frontier_candidate_count,
...
function ll_tools_wordset_games_build_lineup_pool(int $wordset_id, int $user_id = 0): array {
    ...
    $sequence_cap = ll_tools_wordset_games_lineup_launch_sequence_cap();
    ...
    foreach ($visible_category_ids as $category_id) {
        ...
        if (count($sequences) >= $sequence_cap) {
            continue;
        }
...
function ll_tools_wordset_games_collect_visible_words_for_candidate_pool(
    int $wordset_id,
    int $user_id,
    string $game_slug,
    array $candidate_words
): array {
    $wordset_id = max(0, $wordset_id);
    $uid = (int) ($user_id ?: get_current_user_id());
    $candidate_words = array_values(array_filter($candidate_words, 'is_array'));
    if ($wordset_id <= 0 || empty($candidate_words)) {
        return [
            'categories' => [],
            'category_ids' => [],
            'words' => [],
    ...
    foreach ($visible_categories as $category_row) {
        ...
        if ($category_id > 0) {
            $visible_category_names[$category_id] = (string) ($category_row['name'] ?? '');
        }
    }

    $candidate_by_id = [];
    $candidate_ids_by_category = [];
    $candidate_order = [];
    foreach ($candidate_words as $candidate) {
        $word_id = (int) ($candidate['id'] ?? 0);
        if ($word_id <= 0) {
            continue;
        }

        $category_ids = isset($candidate['category_ids']) && is_array($candidate['category_ids'])
            ? array_values(array_filter(array_map('intval', $candidate['category_ids']), static function (int $category_id) use ($visible_category_lookup): bool {
                return $category_id > 0 && !empty($visible_category_lookup[$category_id]);
            }))
            : [];
        if (empty($category_ids) && !empty($candidate['category_id']) && !empty($visible_category_lookup[(int) $candidate['category_id']])) {
            $category_ids = [(int) $candidate['category_id']];
        }
        if (empty($category_ids)) {
            continue;
        }

        $candidate['category_id'] = (int) reset($category_ids);
        $candidate['category_ids'] = $category_ids;
        $candidate_by_id[$word_id] = $candidate;
        $candidate_order[] = $word_id;
        foreach ($category_ids as $category_id) {
            if (!isset($candidate_ids_by_category[$category_id])) {
                $candidate_ids_by_category[$category_id] = [];
            }
            $candidate_ids_by_category[$category_id][$word_id] = $word_id;
        }
    }
    if (empty($candidate_by_id)) {
        return [
            'categories' => $visible_categories,
            'category_ids' => $visible_category_ids,
            'words' => [],
    ...
    foreach ($visible_categories as $category_row) {
        $category_id = is_array($category_row) ? (int) ($category_row['id'] ?? 0) : 0;
        if ($category_id <= 0 || empty($candidate_ids_by_category[$category_id])) {
            continue;
        }
        ...
        $merged_config = array_merge((array) $config, [
            'prompt_type' => 'audio',
            'option_type' => 'image',
            '__candidate_word_ids' => array_values($candidate_ids_by_category[$category_id]),
        ]);

        $words = ll_get_words_by_category($term, 'image', [$wordset_id], $merged_config);
        foreach ((array) $words as $word) {
            if (!is_array($word)) {
                continue;
            }
            ...
            $word_id = (int) ($word['id'] ?? 0);
            if ($word_id <= 0 || !isset($candidate_by_id[$word_id]) || isset($words_by_id[$word_id])) {
                continue;
            }

            $candidate = $candidate_by_id[$word_id];
            $category_ids = isset($candidate['category_ids']) && is_array($candidate['category_ids'])
                ? array_values(array_filter(array_map('intval', $candidate['category_ids']), static function (int $id): bool {
                    return $id > 0;
                }))
                : [$category_id];
            $category_names = [];
            foreach ($category_ids as $candidate_category_id) {
                $name = (string) ($visible_category_names[$candidate_category_id] ?? '');
                if ($name === '') {
                    $candidate_term = get_term($candidate_category_id, 'word-category');
                    $name = ($candidate_term instanceof WP_Term && !is_wp_error($candidate_term)) ? (string) $candidate_term->name : '';
                }
                if ($name !== '') {
                    $category_names[] = $name;
                }
            ...
            $word['category_id'] = (int) ($candidate['category_id'] ?? $category_id);
            $word['category_ids'] = $category_ids;
            $word['category_name'] = (string) ($category_names[0] ?? $term->name);
            $word['category_names'] = array_values(array_unique($category_names));
            $word['progress_status'] = (string) ($candidate['progress_status'] ?? 'new');
            $words_by_id[$word_id] = $word;
        }
    }
    ...
    $ordered_words = [];
    $seen = [];
    foreach ($candidate_order as $word_id) {
        if (!isset($words_by_id[$word_id]) || isset($seen[$word_id])) {
            continue;
        }
...
function ll_tools_wordset_games_collect_visible_speaking_words_for_candidate_pool(
    int $wordset_id,
    int $user_id,
    string $game_slug,
    array $candidate_words
    ...
    foreach ($visible_categories as $category_row) {
        ...
        $words = ll_get_words_by_category($term, 'text_title', [$wordset_id], $merged_config);
        foreach ((array) $words as $word) {
            if (!is_array($word)) {
                continue;
            }
            ...
            $word['category_name'] = (string) ($category_names[0] ?? $term->name);
            $word['category_names'] = array_values(array_unique($category_names));
            $word['progress_status'] = (string) ($candidate['progress_status'] ?? 'mastered');
            $words_by_id[$word_id] = $word;
        }
    }
    ...
    foreach ($candidate_order as $word_id) {
        ...
        $ordered_words[] = $words_by_id[$word_id];
        $seen[$word_id] = true;
    }
...
function ll_tools_wordset_games_build_practice_launch_pool(int $wordset_id, int $user_id = 0, string $game_slug = ''): array {
    $game_slug = sanitize_key($game_slug);
    $launch_word_cap = $game_slug === 'bubble-pop'
        ? ll_tools_wordset_games_bubble_pop_launch_word_cap()
        : ll_tools_wordset_games_space_shooter_launch_word_cap();
    $count_pool = ll_tools_wordset_games_build_practice_deferred_count_pool($wordset_id, $user_id, $game_slug, $launch_word_cap);
    $minimum_word_count = max(1, (int) ($count_pool['minimum_word_count'] ?? ll_tools_wordset_games_min_word_count()));
    $available_word_count = max(0, (int) ($count_pool['available_word_count'] ?? 0));
    $resolved_launch_word_cap = max($minimum_word_count, $launch_word_cap);
    $candidate_words = isset($count_pool['launch_candidate_words']) && is_array($count_pool['launch_candidate_words'])
        ? array_values(array_filter($count_pool['launch_candidate_words'], 'is_array'))
        : [];
    $collected = !empty($candidate_words)
        ? ll_tools_wordset_games_collect_visible_words_for_candidate_pool($wordset_id, $user_id, $game_slug, $candidate_words)
        : [
            'category_ids' => isset($count_pool['category_ids']) && is_array($count_pool['category_ids']) ? $count_pool['category_ids'] : [],
            'words' => [],
        ];
    $launch_words = isset($collected['words']) && is_array($collected['words'])
        ? array_values(array_filter($collected['words'], 'is_array'))
        : [];
    if (!empty($launch_words) && function_exists('ll_tools_attach_user_practice_progress_to_words')) {
        $launch_words = ll_tools_attach_user_practice_progress_to_words($launch_words, (int) ($user_id ?: get_current_user_id()));
    }
    $pool_source = (string) ($count_pool['pool_source'] ?? 'deferred_count');
    $deferred_suffix = '_deferred_count';
    if (substr($pool_source, -strlen($deferred_suffix)) === $deferred_suffix) {
        $pool_source = substr($pool_source, 0, -strlen($deferred_suffix));
    }
    if ($pool_source === '') {
        $pool_source = 'deferred_count';
    }
...
function ll_tools_wordset_games_build_speaking_word_entry(array $word, string $target_field, string $target_label): ?array {
    $word_id = (int) ($word['id'] ?? 0);
    if ($word_id <= 0) {
        return null;
    }
    ...
    $word['speaking_prompt_text'] = $prompt_text;
    $word['speaking_prompt_type'] = !empty($word['image']) ? 'image' : 'text';
    $word['speaking_display_texts'] = ll_tools_wordset_games_get_speaking_display_texts($word_id, $target_field, $word);
    $word['speaking_best_correct_audio_url'] = $best_correct_audio_url;
    $word['progress_status'] = (string) ($word['progress_status'] ?? 'mastered');

    return $word;
}

function ll_tools_wordset_games_build_speaking_launch_pool(int $wordset_id, int $user_id = 0, string $game_slug = 'speaking-practice', bool $require_image = false): array {
    $game_slug = sanitize_key($game_slug);
    $launch_word_cap = $game_slug === 'speaking-stack'
        ? ll_tools_wordset_games_speaking_stack_launch_word_cap()
        : ll_tools_wordset_games_speaking_practice_launch_word_cap();
    $candidate_word_cap = max($launch_word_cap, $launch_word_cap * 3);
    $count_pool = ll_tools_wordset_games_build_speaking_deferred_count_pool($wordset_id, $user_id, $game_slug, $require_image, $candidate_word_cap);
    $minimum_word_count = max(1, (int) ($count_pool['minimum_word_count'] ?? ll_tools_wordset_games_min_word_count()));
    $available_word_count = max(0, (int) ($count_pool['available_word_count'] ?? 0));
    $target_field = (string) ($count_pool['target_field'] ?? '');
    $target_label = (string) ($count_pool['target_label'] ?? '');
    $base = [
        'minimum_word_count' => $minimum_word_count,
        'pool_source' => $require_image ? 'speaking_stack' : 'mastered',
        'category_ids' => isset($count_pool['category_ids']) && is_array($count_pool['category_ids']) ? $count_pool['category_ids'] : [],
        'available_word_count' => $available_word_count,
        'launch_word_cap' => $launch_word_cap,
        'launch_word_count' => 0,
        ...
        'target_field' => $target_field,
        'target_label' => $target_label,
        'enabled' => !empty($count_pool['enabled']),
        'reason_code' => (string) ($count_pool['reason_code'] ?? ($available_word_count >= $minimum_word_count ? '' : 'not_enough_learned_words')),
        'provider' => (string) ($count_pool['provider'] ?? ''),
        'provider_label' => (string) ($count_pool['provider_label'] ?? ''),
        'service_enabled' => !empty($count_pool['service_enabled']),
        'local_endpoint' => ll_tools_wordset_games_public_local_endpoint((string) ($count_pool['provider'] ?? ''), (string) ($count_pool['local_endpoint'] ?? '')),
    ];
    if (empty($base['enabled']) || empty($base['service_enabled']) || $target_field === '') {
        return $base;
    }
    ...
    $collected = ll_tools_wordset_games_collect_visible_speaking_words_for_candidate_pool($wordset_id, $user_id, $game_slug, $candidate_words);
    $rows = isset($collected['words']) && is_array($collected['words'])
        ? array_values(array_filter($collected['words'], 'is_array'))
        : [];
    $launch_words = [];
    foreach ($rows as $word) {
        if ($require_image && !ll_tools_wordset_games_word_has_supported_image($word)) {
            continue;
        }

        $prepared_word = ll_tools_wordset_games_build_speaking_word_entry($word, $target_field, $target_label);
        if (is_array($prepared_word)) {
            $launch_words[] = $prepared_word;
        }
    }
    if (count($launch_words) > $launch_word_cap) {
        $launch_words = array_slice($launch_words, 0, $launch_word_cap);
    }
    ...
    $base['words'] = array_values($launch_words);
    $base['launch_word_count'] = count($launch_words);
...
function ll_tools_wordset_games_build_speaking_stack_pool(int $wordset_id, int $user_id = 0): array {
    ...
    $eligible_words = isset($speaking_pool['words']) && is_array($speaking_pool['words'])
        ? array_values(array_filter($speaking_pool['words'], static function ($word): bool {
            return is_array($word) && ll_tools_wordset_games_word_has_supported_image($word);
        }))
        : [];
...
function ll_tools_wordset_games_build_space_shooter_pool(int $wordset_id, int $user_id = 0): array {
    return ll_tools_wordset_games_build_practice_launch_pool($wordset_id, $user_id, 'space-shooter');
}

function ll_tools_wordset_games_build_bubble_pop_pool(int $wordset_id, int $user_id = 0): array {
    return ll_tools_wordset_games_build_practice_launch_pool($wordset_id, $user_id, 'bubble-pop');
}

function ll_tools_wordset_games_pool_category_ids(array $pool): array {
    return array_values(array_filter(array_map('intval', (array) ($pool['category_ids'] ?? [])), static function (int $id): bool {
        return $id > 0;
    }));
}

function ll_tools_wordset_games_count_visible_category_words(
    int $wordset_id,
    int $user_id,
    string $game_slug,
    string $display_mode,
    ...
    if (empty($visible_categories) || empty($visible_category_ids) || !function_exists('ll_get_words_by_category_count')) {
        return [
            'category_ids' => $visible_category_ids,
            'available_word_count' => 0,
        ];
    ...
    foreach ($visible_categories as $category_row) {
        ...
        $config = function_exists('ll_tools_get_category_quiz_config')
            ? ll_tools_get_category_quiz_config($term)
            : $forced_config;
        $merged_config = array_merge((array) $config, $forced_config);
        $available_word_count += max(0, (int) ll_get_words_by_category_count(
            $term,
            $display_mode,
            [$wordset_id],
            $merged_config
...
function ll_tools_wordset_games_build_deferred_count_pool(
    int $wordset_id,
    int $user_id,
    string $game_slug,
    string $display_mode,
    ...
    $counted = ll_tools_wordset_games_count_visible_category_words(
        $wordset_id,
        $user_id,
        $game_slug,
        $display_mode,
    ...
    return [
        'minimum_word_count' => ll_tools_wordset_games_min_word_count(),
        'pool_source' => 'deferred_count',
        'category_ids' => isset($counted['category_ids']) && is_array($counted['category_ids']) ? $counted['category_ids'] : [],
        'available_word_count' => (int) ($counted['available_word_count'] ?? 0),
        'words' => [],
    ];
}

function ll_tools_wordset_games_finalize_deferred_count_pool(array $source_pool, string $slug, int $launch_word_cap): array {
    $minimum_word_count = max(1, (int) ($source_pool['minimum_word_count'] ?? ll_tools_wordset_games_min_word_count()));
    $available_word_count = max(0, (int) ($source_pool['available_word_count'] ?? 0));
    $resolved_launch_word_cap = max($minimum_word_count, $launch_word_cap);
    ...
    return [
        ...
        'available_word_count' => $available_word_count,
        'launch_word_cap' => $resolved_launch_word_cap,
        'launch_word_count' => min($available_word_count, $resolved_launch_word_cap),
        'launchable' => $available_word_count >= $minimum_word_count,
        'pool_source' => isset($source_pool['pool_source']) ? (string) $source_pool['pool_source'] : 'deferred_count',
        'category_ids' => ll_tools_wordset_games_pool_category_ids($source_pool),
        'words' => [],
    ];
}
...
function ll_tools_wordset_games_query_word_ids_for_category_batch(int $wordset_id, int $category_id, int $limit, int $offset = 0): array {
    global $wpdb;
...
function ll_tools_wordset_games_collect_practice_candidate_rows_for_word_ids(
    array $word_ids,
    int $user_id,
    array $visible_category_ids
): array {
    ...
    $word_category_ids_map = function_exists('ll_tools_get_object_term_ids_map')
        ? ll_tools_get_object_term_ids_map($word_ids, 'word-category')
        : [];
    $has_image_by_word = function_exists('ll_tools_get_word_effective_image_presence_map')
        ? ll_tools_get_word_effective_image_presence_map($word_ids)
        : [];
    $supported_image_word_lookup = ll_tools_wordset_games_supported_image_word_lookup($word_ids, $has_image_by_word);
    $recording_types_by_word = function_exists('ll_tools_get_word_practice_recording_types_map')
        ? ll_tools_get_word_practice_recording_types_map($word_ids)
        : [];
    $progress_rows = ($user_id > 0 && function_exists('ll_tools_get_user_word_progress_rows'))
        ? ll_tools_get_user_word_progress_rows($user_id, $word_ids)
        : [];
    $allowed_recording_types = array_flip(ll_tools_wordset_games_prompt_recording_types());

    $rows = [];
    foreach ($word_ids as $word_id) {
        if (empty($supported_image_word_lookup[$word_id])) {
            continue;
        }

        $recording_types = isset($recording_types_by_word[$word_id]) && is_array($recording_types_by_word[$word_id])
            ? $recording_types_by_word[$word_id]
            : [];
        $has_prompt_recording = false;
        foreach ($recording_types as $recording_type) {
            $recording_type = ll_tools_normalize_practice_recording_type_slug((string) $recording_type);
            if ($recording_type !== '' && isset($allowed_recording_types[$recording_type])) {
                $has_prompt_recording = true;
                break;
            }
        }
        if (!$has_prompt_recording) {
            continue;
        }

        $word_category_ids = isset($word_category_ids_map[$word_id]) && is_array($word_category_ids_map[$word_id])
            ? array_values(array_filter(array_map('intval', $word_category_ids_map[$word_id]), static function (int $category_id) use ($visible_category_lookup): bool {
                return $category_id > 0 && !empty($visible_category_lookup[$category_id]);
            }))
            : [];
        if (empty($word_category_ids)) {
...
function ll_tools_wordset_games_collect_frontier_new_practice_candidates(
    int $wordset_id,
    int $user_id,
    array $visible_categories,
    array $visible_category_ids,
    int $target_count,
    int $minimum_source_count = 0
): array {
    $target_count = max(1, $target_count);
    $minimum_source_count = max(1, $minimum_source_count > 0 ? $minimum_source_count : $target_count);
    $frontier_category_ids = ll_tools_wordset_games_lowest_frontier_category_ids($wordset_id, $visible_categories);
    $ordered_category_ids = array_values(array_unique(array_merge($frontier_category_ids, $visible_category_ids)));
    ...
    $counted_lookup = [];
    $available_word_count = 0;
    $batch_size = max(50, min(500, $target_count * 4));
    foreach ($ordered_category_ids as $category_id) {
        ...
        $offset = 0;
        $category_available_word_count = 0;
        while (true) {
            $batch_ids = ll_tools_wordset_games_query_word_ids_for_category_batch($wordset_id, $category_id, $batch_size, $offset);
            if (empty($batch_ids)) {
                break;
            }

            $candidate_rows = ll_tools_wordset_games_collect_practice_candidate_rows_for_word_ids($batch_ids, $user_id, $visible_category_ids);
            foreach ($candidate_rows as $candidate) {
                ...
                $available_word_count++;
                $category_available_word_count++;
                $counted_lookup[$word_id] = true;

                if (count($selected) < $target_count && !isset($selected_lookup[$word_id])) {
                    $selected[] = $candidate;
                    $selected_lookup[$word_id] = true;
                }
            }
        ...
        if ($category_available_word_count > 0 && $available_word_count >= $minimum_source_count) {
            break;
        }
    }
    ...
    return [
        'available_word_count' => $available_word_count,
        'launch_candidate_words' => $selected,
    ];
}

function ll_tools_wordset_games_build_practice_deferred_count_pool(int $wordset_id, int $user_id = 0, string $game_slug = '', int $launch_word_cap = 0): array {
    $wordset_id = max(0, $wordset_id);
    $uid = (int) ($user_id ?: get_current_user_id());
    $minimum_word_count = ll_tools_wordset_games_min_word_count();
    if ($wordset_id <= 0) {
    ...
    if ($pool_available_word_count < $minimum_word_count && !$has_recorded_progress) {
        ...
        $frontier_words = isset($frontier_pool['launch_candidate_words']) && is_array($frontier_pool['launch_candidate_words'])
            ? array_values(array_filter($frontier_pool['launch_candidate_words'], 'is_array'))
            : [];
        if (!empty($frontier_words)) {
            $pool = $frontier_words;
            $pool_available_word_count = max(count($frontier_words), (int) ($frontier_pool['available_word_count'] ?? 0));
            $pool_source = 'frontier_new';
        }
    }

    $launch_candidate_words = $launch_word_cap > 0
        ? ll_tools_wordset_games_limit_launch_words($pool, max($minimum_word_count, $launch_word_cap))
        : [];
    $launch_candidate_word_ids = array_values(array_filter(array_map(static function ($word): int {
        return is_array($word) ? (int) ($word['id'] ?? 0) : 0;
    }, $launch_candidate_words), static function (int $word_id): bool {
        return $word_id > 0;
    }));
...
function ll_tools_wordset_games_build_unscramble_deferred_count_pool(int $wordset_id, int $user_id = 0): array {
    $pool = ll_tools_wordset_games_build_deferred_count_pool(
        $wordset_id,
        $user_id,
        'unscramble',
        'text_title',
    ...
    $launch_word_cap = ll_tools_wordset_games_unscramble_launch_word_cap();
    $minimum_word_count = max(1, (int) ($pool['minimum_word_count'] ?? ll_tools_wordset_games_min_word_count()));
    $available_word_count = max(0, (int) ($pool['available_word_count'] ?? 0));

    return array_merge($pool, [
        'launch_word_cap' => max($minimum_word_count, $launch_word_cap),
        'launch_word_count' => min($available_word_count, max($minimum_word_count, $launch_word_cap)),
        'enabled_category_count' => count(ll_tools_wordset_games_pool_category_ids($pool)),
    ]);
}

function ll_tools_wordset_games_build_lineup_deferred_count_pool(int $wordset_id, int $user_id = 0): array {
    $wordset_id = max(0, $wordset_id);
    $uid = (int) ($user_id ?: get_current_user_id());
    $minimum_sequence_length = ll_tools_wordset_games_lineup_min_sequence_length();
    ...
    return [
        ...
        'minimum_sequence_count' => 1,
        'minimum_sequence_length' => $minimum_sequence_length,
        'pool_source' => 'lineup_deferred_count',
        'category_ids' => $visible_category_ids,
        'enabled_category_count' => count($visible_category_ids),
        'available_sequence_count' => $available_sequence_count,
...
function ll_tools_wordset_games_filter_visible_candidate_word_ids(array $word_ids, int $wordset_id, array $category_ids): array {
    $wordset_id = max(0, $wordset_id);
    $word_ids = array_values(array_unique(array_filter(array_map('intval', $word_ids), static function (int $word_id): bool {
        return $word_id > 0;
    })));
    ...
    if ($wordset_id <= 0 || empty($word_ids) || empty($category_ids)) {
        return [];
    }
    ...
    foreach (array_chunk($word_ids, 500) as $chunk) {
        ...
        foreach ((array) $query->posts as $word_id) {
            $word_id = (int) $word_id;
            if ($word_id > 0) {
                $visible_lookup[$word_id] = true;
            }
        }
    }
...
function ll_tools_wordset_games_query_speaking_mastered_word_id_batch(
    ...
    foreach ((array) $rows as $row) {
        ...
        $row['practice_required_recording_types_json'] = (string) ($row['practice_required_recording_types'] ?? '');
        $row['practice_correct_recording_types_json'] = (string) ($row['practice_correct_recording_types'] ?? '');
        $status = function_exists('ll_tools_user_progress_word_status')
            ? ll_tools_user_progress_word_status($row)
            : (!empty($row['mastery_unlocked']) ? 'mastered' : 'studied');
        if ($status === 'mastered') {
            $mastered_ids[] = $word_id;
        }
    }
...
function ll_tools_wordset_games_word_ids_with_speaking_isolation_audio(array $word_ids, string $target_field): array {
    ...
    foreach (array_chunk($word_ids, 500) as $chunk) {
        ...
        $sql = $wpdb->prepare(
            "
            SELECT p.ID, p.post_parent, path.meta_value AS audio_path,
                   text_meta.meta_value AS recording_text,
                   ipa_meta.meta_value AS recording_ipa,
                   duration_meta.meta_value AS duration_seconds
            FROM {$wpdb->posts} p
            ...
            LEFT JOIN {$wpdb->postmeta} type_meta
               ...
               AND type_meta.meta_key = %s
            INNER JOIN {$wpdb->postmeta} path
                ON path.post_id = p.ID
               AND path.meta_key = %s
               AND path.meta_value <> ''
            LEFT JOIN {$wpdb->postmeta} text_meta
                ON text_meta.post_id = p.ID
               AND text_meta.meta_key = %s
            LEFT JOIN {$wpdb->postmeta} ipa_meta
                ON ipa_meta.post_id = p.ID
               AND ipa_meta.meta_key = %s
            LEFT JOIN {$wpdb->postmeta} duration_meta
                ON duration_meta.post_id = p.ID
               AND duration_meta.meta_key = %s
            WHERE p.post_type = %s
              AND p.post_status = %s
              AND (tr.object_id IS NOT NULL OR LOWER(TRIM(type_meta.meta_value)) = %s)
        ...
        foreach ((array) $rows as $row) {
            $word_id = isset($row['post_parent']) ? (int) $row['post_parent'] : 0;
            if ($word_id <= 0 || isset($checked_lookup[$word_id])) {
                continue;
            }

            $recording_text = trim((string) ($row['recording_text'] ?? ''));
            if ($recording_text !== '' && function_exists('ll_tools_trim_isolation_transcript')) {
                $recording_text = ll_tools_trim_isolation_transcript($recording_text);
            }
            $recording_ipa = trim((string) ($row['recording_ipa'] ?? ''));

            if (
                $target_field !== 'recording_ipa'
                && $recording_text !== ''
                && ll_tools_wordset_games_count_isolation_words($recording_text) > ll_tools_wordset_games_speaking_isolation_max_word_count()
            ) {
                continue;
            }
            ...
            $duration = isset($row['duration_seconds']) && is_numeric($row['duration_seconds'])
                ...
                : null;
            if ($duration !== null && $duration > ll_tools_wordset_games_speaking_isolation_max_duration_seconds()) {
                continue;
            }

            $checked_lookup[$word_id] = true;
            if ($target_field === 'recording_ipa' && $recording_ipa === '') {
                continue;
            }
            if ($target_field === 'recording_text' && $recording_text === '') {
                continue;
            }

            $eligible_lookup[$word_id] = true;
        }
    }

    return array_values(array_map('intval', array_keys($eligible_lookup)));
}

function ll_tools_wordset_games_build_speaking_deferred_count_pool(
    int $wordset_id,
    int $user_id = 0,
    string $game_slug = 'speaking-practice',
    bool $require_image = false,
    int $launch_word_cap = 0
): array {
    $wordset_id = max(0, $wordset_id);
    $uid = (int) ($user_id ?: get_current_user_id());
    $minimum_word_count = ll_tools_wordset_games_min_word_count();
    $disabled = [
        'minimum_word_count' => $minimum_word_count,
        'pool_source' => 'speaking_deferred_count',
        'category_ids' => [],
        'available_word_count' => 0,
        'words' => [],
        'target_field' => '',
    ...
    $target_field = function_exists('ll_tools_sanitize_wordset_speaking_game_target')
        ? ll_tools_sanitize_wordset_speaking_game_target((string) ($config['target'] ?? 'recording_text'))
        : sanitize_key((string) ($config['target'] ?? 'recording_text'));
    $target_label = (string) ($config['target_label'] ?? __('Written text', 'll-tools-text-domain'));
    $base = array_merge($disabled, [
        'target_field' => $target_field,
        'target_label' => $target_label,
        'provider' => (string) ($config['provider'] ?? ''),
        'provider_label' => (string) ($config['provider_label'] ?? ''),
    ...
    if (function_exists('ll_tools_user_can_access_wordset_speaking_games') && !ll_tools_user_can_access_wordset_speaking_games($wordset_id, $uid)) {
        $base['pool_source'] = 'access_restricted';
        $base['reason_code'] = 'speaking_access_restricted';
        return $base;
    }
    if (empty($config['enabled'])) {
        $base['pool_source'] = 'stt_disabled';
        $base['reason_code'] = 'stt_not_enabled';
        return $base;
    }
    if (empty($config['service_enabled'])) {
        $base['pool_source'] = 'speaking_service_disabled';
        $base['enabled'] = true;
        $base['reason_code'] = 'speaking_service_unavailable';
        return $base;
    }
    ...
    if ($launch_word_cap > 0 && !empty($candidate_ids)) {
        ...
        foreach ($candidate_ids as $word_id) {
            ...
            $raw_category_ids = isset($word_category_ids_map[$word_id]) && is_array($word_category_ids_map[$word_id])
                ? $word_category_ids_map[$word_id]
                : wp_get_object_terms($word_id, 'word-category', ['fields' => 'ids']);
            if (is_wp_error($raw_category_ids)) {
                $raw_category_ids = [];
            }
            $word_category_ids = array_values(array_filter(array_map('intval', (array) $raw_category_ids), static function (int $category_id) use ($visible_category_lookup): bool {
                return $category_id > 0 && !empty($visible_category_lookup[$category_id]);
            }));
            if (empty($word_category_ids)) {
                continue;
        ...
        $launch_candidate_words = ll_tools_wordset_games_limit_launch_words($candidate_words, $launch_word_cap);
        $launch_candidate_word_ids = array_values(array_filter(array_map(static function ($word): int {
            return is_array($word) ? (int) ($word['id'] ?? 0) : 0;
        }, $launch_candidate_words), static function (int $word_id): bool {
            return $word_id > 0;
    ...
    return array_merge($base, [
        'pool_source' => $require_image ? 'speaking_stack_deferred_count' : 'speaking_deferred_count',
        'category_ids' => $visible_category_ids,
        'available_word_count' => $available_count,
        'launch_candidate_words' => $launch_candidate_words,
        'launch_candidate_word_ids' => $launch_candidate_word_ids,
        'enabled' => true,
        'launchable' => $available_count >= $minimum_word_count,
        'reason_code' => $available_count >= $minimum_word_count ? '' : 'not_enough_learned_words',
    ]);
}

function ll_tools_wordset_games_defer_catalog_entry_payload(array $entry, bool $include_payload): array {
    if ($include_payload) {
        return $entry;
    }

    $entry['payload_deferred'] = true;
    if (array_key_exists('words', $entry)) {
        $entry['words'] = [];
    }
    if (array_key_exists('sequences', $entry)) {
        $entry['sequences'] = [];
    }
...
function ll_tools_wordset_games_build_catalog(int $wordset_id, int $user_id = 0, bool $include_payload = true): array {
    ...
    if ($include_payload) {
        ...
        $space_category_ids = ll_tools_wordset_games_pool_category_ids($space_source_pool);
        $bubble_category_ids = ll_tools_wordset_games_visible_category_ids($wordset_id, $user_id, 'bubble-pop');
        $bubble_source_pool = ($bubble_category_ids === $space_category_ids)
            ? $space_source_pool
            : ll_tools_wordset_games_build_practice_source_pool($wordset_id, $user_id, 'bubble-pop');
        ...
        $space_source_pool = ll_tools_wordset_games_build_practice_deferred_count_pool($wordset_id, $user_id, 'space-shooter');
        $space_shooter = ll_tools_wordset_games_finalize_deferred_count_pool(
            $space_source_pool,
            'space-shooter',
            ll_tools_wordset_games_space_shooter_launch_word_cap()
        );
        ...
        $bubble_source_pool = ($bubble_category_ids === $space_category_ids)
            ? $space_source_pool
            : ll_tools_wordset_games_build_practice_deferred_count_pool($wordset_id, $user_id, 'bubble-pop');
        $bubble_pop = ll_tools_wordset_games_finalize_deferred_count_pool(
            $bubble_source_pool,
            'bubble-pop',
            ll_tools_wordset_games_bubble_pop_launch_word_cap()
        );
    ...
    $catalog['space-shooter'] = ll_tools_wordset_games_defer_catalog_entry_payload(array_merge($catalog['space-shooter'], [
    ...
    $catalog['bubble-pop'] = ll_tools_wordset_games_defer_catalog_entry_payload(array_merge($catalog['bubble-pop'], [
    ...
    $unscramble_pool = $include_payload
        ? ll_tools_wordset_games_build_unscramble_pool($wordset_id, $user_id)
        : ll_tools_wordset_games_build_unscramble_deferred_count_pool($wordset_id, $user_id);
    ...
    if ($unscramble_available > 0) {
        $catalog['unscramble'] = ll_tools_wordset_games_defer_catalog_entry_payload([
            'slug' => 'unscramble',
    ...
    $lineup_pool = $include_payload
        ? ll_tools_wordset_games_build_lineup_pool($wordset_id, $user_id)
    ...
    $speaking_pool = $include_payload
        ? ll_tools_wordset_games_build_speaking_launch_pool($wordset_id, $user_id, 'speaking-practice', false)
        : ll_tools_wordset_games_build_speaking_deferred_count_pool($wordset_id, $user_id, 'speaking-practice', false);
    $speaking_minimum = (int) ($speaking_pool['minimum_word_count'] ?? ll_tools_wordset_games_min_word_count());
    $speaking_words = isset($speaking_pool['words']) && is_array($speaking_pool['words']) ? $speaking_pool['words'] : [];
    $speaking_available = $include_payload
        ? (int) ($speaking_pool['available_word_count'] ?? count((array) $speaking_words))
        : (int) ($speaking_pool['available_word_count'] ?? 0);
    $speaking_launch_cap = (int) ($speaking_pool['launch_word_cap'] ?? ll_tools_wordset_games_speaking_practice_launch_word_cap());
    $speaking_launch_words = $include_payload
        ? $speaking_words
        : [];
    $speaking_launch_count = $include_payload
        ? (int) ($speaking_pool['launch_word_count'] ?? count($speaking_launch_words))
        : min($speaking_available, $speaking_launch_cap);
    $speaking_enabled = !empty($speaking_pool['enabled']) && !empty($speaking_pool['service_enabled']);
    if ($speaking_enabled) {
        $catalog['speaking-practice'] = ll_tools_wordset_games_defer_catalog_entry_payload([
            ...
            'launch_word_cap' => $speaking_launch_cap,
            'launch_word_count' => $speaking_launch_count,
    ...
    $speaking_stack_pool = $include_payload
        ? ll_tools_wordset_games_build_speaking_launch_pool($wordset_id, $user_id, 'speaking-stack', true)
        : ll_tools_wordset_games_build_speaking_deferred_count_pool($wordset_id, $user_id, 'speaking-stack', true);
    $speaking_stack_minimum = (int) ($speaking_stack_pool['minimum_word_count'] ?? ll_tools_wordset_games_min_word_count());
    $speaking_stack_words = isset($speaking_stack_pool['words']) && is_array($speaking_stack_pool['words']) ? $speaking_stack_pool['words'] : [];
    $speaking_stack_available = $include_payload
        ? (int) ($speaking_stack_pool['available_word_count'] ?? count((array) $speaking_stack_words))
        : (int) ($speaking_stack_pool['available_word_count'] ?? 0);
    $speaking_stack_launch_cap = (int) ($speaking_stack_pool['launch_word_cap'] ?? ll_tools_wordset_games_speaking_stack_launch_word_cap());
    $speaking_stack_launch_words = $include_payload
        ? $speaking_stack_words
        : [];
    $speaking_stack_launch_count = $include_payload
        ? (int) ($speaking_stack_pool['launch_word_count'] ?? count($speaking_stack_launch_words))
        : min($speaking_stack_available, $speaking_stack_launch_cap);
    ...
    if ($speaking_stack_enabled) {
        $catalog['speaking-stack'] = ll_tools_wordset_games_defer_catalog_entry_payload([
            'slug' => 'speaking-stack',
            ...
            'launch_word_cap' => $speaking_stack_launch_cap,
            'launch_word_count' => $speaking_stack_launch_count,
...
function ll_tools_wordset_games_build_launch_entry(string $slug, int $wordset_id, int $user_id = 0): ?array {
    ...
    if ($slug === 'space-shooter' || $slug === 'bubble-pop') {
        $launch_pool = ll_tools_wordset_games_build_practice_launch_pool($wordset_id, $uid, $slug);
        $minimum = (int) ($launch_pool['minimum_word_count'] ?? ll_tools_wordset_games_min_word_count());
    ...
    if ($slug === 'speaking-practice') {
        $speaking_pool = ll_tools_wordset_games_build_speaking_launch_pool($wordset_id, $uid, 'speaking-practice', false);
        ...
        $available_count = (int) ($speaking_pool['available_word_count'] ?? count($available_words));
        $launch_word_cap = (int) ($speaking_pool['launch_word_cap'] ?? ll_tools_wordset_games_speaking_practice_launch_word_cap());
        $launch_words = $available_words;
        ...
        return [
            ...
            'available_word_count' => $available_count,
            'launch_word_cap' => $launch_word_cap,
            'launch_word_count' => (int) ($speaking_pool['launch_word_count'] ?? count($launch_words)),
    ...
    if ($slug === 'speaking-stack') {
        $speaking_stack_pool = ll_tools_wordset_games_build_speaking_launch_pool($wordset_id, $uid, 'speaking-stack', true);
        ...
        $available_count = (int) ($speaking_stack_pool['available_word_count'] ?? count($available_words));
        $launch_word_cap = (int) ($speaking_stack_pool['launch_word_cap'] ?? ll_tools_wordset_games_speaking_stack_launch_word_cap());
        $launch_words = $available_words;
        ...
        return [
            ...
            'available_word_count' => $available_count,
            'launch_word_cap' => $launch_word_cap,
            'launch_word_count' => (int) ($speaking_stack_pool['launch_word_count'] ?? count($launch_words)),
            'launchable' => $available_count >= $minimum,
            'reason_code' => $available_count >= $minimum
                ? ''
                : (!empty($speaking_stack_pool['reason_code']) ? (string) ($speaking_stack_pool['reason_code']) : 'not_enough_learned_words'),
