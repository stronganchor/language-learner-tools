...
function ll_tools_dictionary_enqueue_assets(): void {
    ...
    if (!$script_localized && wp_script_is('ll-tools-dictionary-shortcode-script', 'enqueued')) {
        wp_localize_script('ll-tools-dictionary-shortcode-script', 'llToolsDictionary', [
            ...
            'detailLoadingLabel' => __('Loading dictionary entry...', 'll-tools-text-domain'),
...
/**
 * Resolve a requested dictionary entry ID.
 */
function ll_tools_dictionary_resolve_entry_id($raw_entry, int $wordset_id = 0): int {
    $raw_entry = sanitize_text_field(wp_unslash((string) $raw_entry));
    if ($raw_entry === '' || !ctype_digit($raw_entry)) {
        return 0;
    }

    $entry_id = (int) $raw_entry;
    if ($entry_id <= 0 || !function_exists('ll_tools_is_dictionary_entry_id')) {
        return 0;
    }

    if (function_exists('ll_tools_dictionary_browser_prepare_source_read')) {
        ll_tools_dictionary_browser_prepare_source_read();
    }
    $is_dictionary_entry = ll_tools_is_dictionary_entry_id($entry_id);
    if (
        function_exists('ll_tools_dictionary_browser_capture_query_error')
        && ll_tools_dictionary_browser_capture_query_error('resolve_entry_post') instanceof WP_Error
    ) {
        return 0;
    }
    ...
    if (
        function_exists('ll_tools_dictionary_browser_capture_query_error')
        && ll_tools_dictionary_browser_capture_query_error('resolve_entry_status') instanceof WP_Error
    ) {
        return 0;
    }
    ...
    if (function_exists('ll_tools_dictionary_current_user_can_view_entry')) {
        $can_view_entry = ll_tools_dictionary_current_user_can_view_entry($entry_id);
        if (
            function_exists('ll_tools_dictionary_browser_get_query_error')
    ...
    if ($wordset_id > 0 && function_exists('ll_tools_dictionary_entry_matches_wordset_context')) {
        $matches_wordset = ll_tools_dictionary_entry_matches_wordset_context($entry_id, $wordset_id);
        if (
            function_exists('ll_tools_dictionary_browser_get_query_error')
            && ll_tools_dictionary_browser_get_query_error() instanceof WP_Error
        ) {
...
function ll_tools_dictionary_shortcode_resolve_requested_entry_id(int $wordset_id = 0): int {
    $raw_entry = isset($_GET['ll_dictionary_entry']) ? $_GET['ll_dictionary_entry'] : '';

    return ll_tools_dictionary_resolve_entry_id($raw_entry, $wordset_id);
}
...
function ll_tools_dictionary_get_public_word_ids_for_entry(int $entry_id, int $limit = -1): array {
    ...
    if (
        function_exists('ll_tools_dictionary_browser_capture_query_error')
        && ll_tools_dictionary_browser_capture_query_error('detail_linked_words_entry') instanceof WP_Error
    ) {
        return [];
    }
    if (!$is_dictionary_entry) {
    ...
    if ($limit < 0 && function_exists('ll_tools_get_dictionary_entry_word_ids')) {
        if (function_exists('ll_tools_dictionary_browser_prepare_source_read')) {
            ll_tools_dictionary_browser_prepare_source_read();
        }
        $raw_linked_ids = ll_tools_get_dictionary_entry_word_ids($entry_id, -1);
        if (
            function_exists('ll_tools_dictionary_browser_capture_query_error')
            && ll_tools_dictionary_browser_capture_query_error('detail_linked_words_all') instanceof WP_Error
        ) {
            return [];
        }

        $linked_ids = [];
        foreach (array_values(array_filter(array_map('intval', (array) $raw_linked_ids))) as $word_id) {
            if (function_exists('ll_tools_dictionary_browser_prepare_source_read')) {
                ll_tools_dictionary_browser_prepare_source_read();
            }
            $word_status = get_post_status($word_id);
            if (
                function_exists('ll_tools_dictionary_browser_capture_query_error')
                && ll_tools_dictionary_browser_capture_query_error('detail_linked_word_status') instanceof WP_Error
            ) {
                return [];
            }
            if ($word_status === 'publish') {
                $linked_ids[] = $word_id;
            }
        }
        if (!empty($linked_ids)) {
    ...
    $word_ids_result = get_posts($query_args);
    if (
        function_exists('ll_tools_dictionary_browser_capture_query_error')
        && ll_tools_dictionary_browser_capture_query_error('detail_linked_words_query', $word_ids_result) instanceof WP_Error
    ) {
        return [];
    }
    $word_ids = array_values(array_filter(array_map('intval', (array) $word_ids_result)));
    if (!empty($word_ids)) {
        return $word_ids;
    }
    ...
    $fallback_word_ids = $wpdb->get_col($wpdb->prepare($sql, $params));
    if (
        function_exists('ll_tools_dictionary_browser_capture_query_error')
        && ll_tools_dictionary_browser_capture_query_error('detail_linked_words_fallback', $fallback_word_ids) instanceof WP_Error
    ) {
        return [];
    }

    return array_values(array_filter(array_map('intval', (array) $fallback_word_ids)));
}

function ll_tools_dictionary_detail_linked_words_preview_limit(): int {
    $limit = (int) apply_filters('ll_tools_dictionary_detail_linked_words_preview_limit', 24);
    if ($limit <= 0) {
        return 0;
    }

    $max_limit = (int) apply_filters('ll_tools_dictionary_detail_linked_words_preview_max_limit', 50);
    $max_limit = max(1, min(100, $max_limit));
...
function ll_tools_dictionary_collect_related_entries(int $entry_id, array $preferred_languages = [], int $limit = 6, $source_filter = []): array {
    ...
    if (
        function_exists('ll_tools_dictionary_browser_capture_query_error')
        && ll_tools_dictionary_browser_capture_query_error('related_entry_post') instanceof WP_Error
    ) {
        return [];
    }
    if (!$is_dictionary_entry) {
    ...
    $cache_args = [
        ...
        'source_completeness_contract' => 2,
    ];
    ...
    if (
        function_exists('ll_tools_dictionary_browser_capture_query_error')
        && ll_tools_dictionary_browser_capture_query_error('related_entry_title') instanceof WP_Error
    ) {
        return [];
    }
    ...
    if (
        function_exists('ll_tools_dictionary_browser_capture_query_error')
        && ll_tools_dictionary_browser_capture_query_error('related_entry_wordset') instanceof WP_Error
    ) {
        return [];
    }
    $candidate_ids = [];
    ...
    foreach ($search_queries as $query_args) {
        $query = function_exists('ll_tools_dictionary_query_entries')
            ...
            : ['items' => []];
        $query_source_error = function_exists('ll_tools_dictionary_browser_get_query_error')
            ? ll_tools_dictionary_browser_get_query_error()
            : null;
        if (!empty($query['retryable']) || $query_source_error instanceof WP_Error) {
            if (!($query_source_error instanceof WP_Error) && function_exists('ll_tools_dictionary_browser_mark_query_error')) {
                ll_tools_dictionary_browser_mark_query_error(
                    'related_entries_query',
                    (string) ($query['error_code'] ?? 'dictionary_source_incomplete')
                );
            }
            return [];
        }
    ...
    foreach (array_keys($candidate_ids) as $candidate_id) {
        ...
        if (
            function_exists('ll_tools_dictionary_browser_capture_query_error')
            && ll_tools_dictionary_browser_capture_query_error('related_candidate_title') instanceof WP_Error
        ) {
            return [];
        }
        ...
        if (
            function_exists('ll_tools_dictionary_browser_capture_query_error')
            && ll_tools_dictionary_browser_capture_query_error('related_candidate_wordset') instanceof WP_Error
        ) {
            return [];
        }
...
function ll_tools_dictionary_render_ai_translation_indicator(): string {
    $label = __('AI translation', 'll-tools-text-domain');
    $description = __('This translation was generated by AI.', 'll-tools-text-domain');
    $icon = '<span class="ll-dictionary__translation-ai-icon" aria-hidden="true">'
        . '<svg viewBox="0 0 24 24" focusable="false" aria-hidden="true">'
        . '<path d="M12 4v3"></path>'
        . '<path d="M8 4h8"></path>'
    ...
    return '<span class="ll-dictionary__translation-ai" aria-label="' . esc_attr($description) . '" title="' . esc_attr($description) . '">'
        . $icon
        . '<span class="ll-dictionary__translation-ai-text">' . esc_html($label) . '</span>'
        . '</span>';
}
...
/**
 * Count example sentences already present in a bounded result-card payload.
 *
 * @param array<int,array<string,mixed>> $senses
 */
function ll_tools_dictionary_count_result_sense_examples(array $senses): int {
    return count(ll_tools_dictionary_collect_sense_examples($senses, 50));
}

/**
 * Render the visible entry-detail affordance for one result card.
 *
 * @param array<int,string> $summary_chips
 */
function ll_tools_dictionary_render_result_detail_action(string $detail_url, int $entry_id = 0, array $summary_chips = []): string {
    if ($detail_url === '') {
        return '';
    }

    $attrs = ' class="ll-dictionary__details-link ll-dictionary__more-link" href="' . esc_url($detail_url) . '" data-ll-dictionary-detail-link';
    if ($entry_id > 0) {
        $attrs .= ' data-entry-id="' . esc_attr((string) $entry_id) . '"';
    }

    $html = '<div class="ll-dictionary__card-actions">';
    $html .= '<a' . $attrs . '>' . esc_html__('View details', 'll-tools-text-domain') . '</a>';
    if (!empty($summary_chips)) {
        $html .= '<span class="ll-dictionary__detail-chips" aria-label="' . esc_attr__('Additional entry information', 'll-tools-text-domain') . '">';
        foreach ($summary_chips as $chip) {
            $chip = trim((string) $chip);
            if ($chip === '') {
                continue;
            }
            $html .= '<span class="ll-dictionary__detail-chip">' . esc_html($chip) . '</span>';
        }
        $html .= '</span>';
    }
    $html .= '</div>';
...
function ll_tools_dictionary_render_result_card(array $item, string $detail_url = ''): string {
    ...
    $translation_is_ai = !empty($item['translation_is_ai']);
    ...
    if ($detail_url !== '') {
        $html .= '<a class="ll-dictionary__title-link" href="' . esc_url($detail_url) . '" data-ll-dictionary-detail-link' . ($entry_id > 0 ? ' data-entry-id="' . esc_attr((string) $entry_id) . '"' : '') . '>' . esc_html($title) . '</a>';
    } else {
        $html .= esc_html($title);
    }
    $html .= '</h3>';
    if (empty($visible_translation_groups) && $translation !== '') {
        $html .= '<div class="ll-dictionary__summary">' . ll_tools_dictionary_render_text_block($translation, 'summary', 220);
        if ($translation_is_ai) {
            $html .= ll_tools_dictionary_render_ai_translation_indicator();
        }
        $html .= '</div>';
    }
    $html .= '</div>';
    ...
    if (!empty($visible_translation_groups)) {
        $html .= '<div class="ll-dictionary__card-translations">';
        if ($translation_is_ai) {
            $html .= '<div class="ll-dictionary__translation-ai-row">' . ll_tools_dictionary_render_ai_translation_indicator() . '</div>';
        }
        $html .= ll_tools_dictionary_render_translation_groups($visible_translation_groups) . '</div>';
    }
    ...
    $example_count = ll_tools_dictionary_count_result_sense_examples($senses);
    if ($example_count > 0) {
        $detail_chips[] = sprintf(
            /* translators: %d: example sentence count */
            _n('%d example', '%d examples', $example_count, 'll-tools-text-domain'),
            $example_count
        );
    }

    $hidden_linked_word_count = max(0, $linked_word_count - count($linked_words));
    if ($hidden_linked_word_count > 0) {
        $detail_chips[] = sprintf(
            /* translators: %d: number of hidden linked words */
            _n('+ %d more linked word', '+ %d more linked words', $hidden_linked_word_count, 'll-tools-text-domain'),
            $hidden_linked_word_count
        );
    }

    $html .= ll_tools_dictionary_render_result_detail_action($detail_url, $entry_id, $detail_chips);
    $html .= '</article>';
...
/**
 * Render the fail-closed state used by synchronous detail requests.
 */
function ll_tools_dictionary_render_source_incomplete_state(): string {
    return '<div class="ll-dictionary__empty"><p>'
        . esc_html__('Dictionary data is temporarily unavailable. Please try again in a moment.', 'll-tools-text-domain')
        . '</p></div>';
}
...
function ll_tools_dictionary_render_detail_view(int $entry_id, string $base_url, array $preferred_languages = [], array $source_ids = []): string {
    ...
    if (
        ...
        return ll_tools_dictionary_render_source_incomplete_state();
    }
    if (empty($entry)) {
    ...
    $translation_is_ai = !empty($entry['translation_is_ai']);
    ...
    if (
        function_exists('ll_tools_dictionary_browser_capture_query_error')
        && ll_tools_dictionary_browser_capture_query_error('detail_review_flag') instanceof WP_Error
    ) {
        return ll_tools_dictionary_render_source_incomplete_state();
    }
    $wordset_names = array_values(array_filter(array_map('strval', (array) ($entry['wordset_names'] ?? []))));
    $linked_word_preview_limit = ll_tools_dictionary_detail_linked_words_preview_limit();
    $linked_word_preview_ids = $linked_word_preview_limit > 0
        ? ll_tools_dictionary_get_public_word_ids_for_entry($entry_id, $linked_word_preview_limit + 1)
        : [];
    if (
        function_exists('ll_tools_dictionary_browser_get_query_error')
        && ll_tools_dictionary_browser_get_query_error() instanceof WP_Error
    ...
    $has_more_linked_words = count($linked_word_preview_ids) > $linked_word_preview_limit;
    $word_ids = $linked_word_preview_limit > 0
        ? array_slice($linked_word_preview_ids, 0, $linked_word_preview_limit)
        : [];
    ...
    $current_source_query = ll_tools_dictionary_shortcode_build_source_query_value($source_ids);
    ...
    $html = '<article class="ll-dictionary__detail" data-ll-dictionary-detail>';
    $html .= '<div class="ll-dictionary__detail-top">';
    $html .= '<a class="ll-dictionary__back" href="' . esc_url($back_url) . '" data-ll-dictionary-back>' . esc_html__('Back to dictionary', 'll-tools-text-domain') . '</a>';
    $html .= '</div>';
    ...
    if (!empty($word_ids)) {
        $linked_words_attrs = $has_more_linked_words
            ? ' data-ll-linked-words-truncated="1" data-ll-linked-words-limit="' . esc_attr((string) $linked_word_preview_limit) . '"'
            : '';
        $html .= '<section class="ll-dictionary__detail-section"' . $linked_words_attrs . '>';
        ...
        $linked_words_html = do_shortcode($shortcode);
        if (
            function_exists('ll_tools_dictionary_browser_capture_query_error')
            && ll_tools_dictionary_browser_capture_query_error('detail_linked_words_render') instanceof WP_Error
        ) {
            return ll_tools_dictionary_render_source_incomplete_state();
        }
        $html .= $linked_words_html;
        $html .= '</section>';
    }
    ...
    if (!empty($related_entries)) {
        ...
        foreach ($related_entries as $related_entry) {
            ...
            $html .= '<a class="ll-dictionary__related-card" href="' . esc_url($related_url) . '" data-ll-dictionary-detail-link data-entry-id="' . esc_attr((string) $related_id) . '">';
    ...
    if (
        function_exists('ll_tools_dictionary_browser_capture_query_error')
        && ll_tools_dictionary_browser_capture_query_error('detail_view_final') instanceof WP_Error
    ) {
        return ll_tools_dictionary_render_source_incomplete_state();
    }
...
function ll_tools_dictionary_render_browse_results(array $query, string $base_url, string $search, array $search_scopes, string $letter, string $pos_slug, array $source_ids = [], string $dialect = ''): string {
    if (!empty($query['retryable'])) {
        return '<div class="ll-dictionary__empty ll-dictionary__empty--retryable" role="status">'
            . esc_html((string) ($query['message'] ?? __('Dictionary data is temporarily unavailable. Please try again in a moment.', 'll-tools-text-domain')))
            . '</div>';
    }
...
function ll_tools_dictionary_resolve_live_base_url(string $raw_base_url = ''): string {
    foreach ([$raw_base_url, (string) wp_get_referer()] as $candidate) {
        $candidate = wp_validate_redirect(trim(esc_url_raw($candidate)), '');
        if ($candidate === '') {
            continue;
        }

        $candidate = (string) remove_query_arg(ll_tools_dictionary_shortcode_query_keys(), $candidate);
        if (function_exists('ll_tools_dictionary_strip_noise_query_args_from_url')) {
            $candidate = ll_tools_dictionary_strip_noise_query_args_from_url($candidate);
        }
        $page_id = url_to_postid($candidate);
        if ($page_id > 0 && get_post_status($page_id) === 'publish') {
            return (string) get_permalink($page_id);
        }
    }

    return function_exists('ll_tools_get_dictionary_page_url')
        ? (string) ll_tools_get_dictionary_page_url()
        : home_url('/');
}
...
function ll_tools_dictionary_ajax_cache_acquire_build_lock(string $key): bool {
    ...
    $token = function_exists('wp_generate_uuid4')
        ? wp_generate_uuid4()
        : hash('sha256', $key . '|' . microtime(true) . '|' . wp_rand());
    $lease_value = $expires_at . '|' . $token;

    if (add_option($option_name, $lease_value, '', false)) {
        if (
            !isset($GLOBALS['ll_tools_dictionary_ajax_cache_build_leases'])
            || !is_array($GLOBALS['ll_tools_dictionary_ajax_cache_build_leases'])
        ) {
            $GLOBALS['ll_tools_dictionary_ajax_cache_build_leases'] = [];
        }
        $GLOBALS['ll_tools_dictionary_ajax_cache_build_leases'][$key] = [
    ...
    $current_expires_at = $separator === false
        ? (int) $current_value
...
function ll_tools_dictionary_ajax_cache_release_build_lock(string $key): void {
    ...
    $leases = isset($GLOBALS['ll_tools_dictionary_ajax_cache_build_leases'])
        && is_array($GLOBALS['ll_tools_dictionary_ajax_cache_build_leases'])
        ? $GLOBALS['ll_tools_dictionary_ajax_cache_build_leases']
        : [];
    $lease = isset($leases[$key]) && is_array($leases[$key])
        ? $leases[$key]
        : [];
    $option_name = (string) ($lease['option_name'] ?? '');
    $lease_value = (string) ($lease['lease_value'] ?? '');
    if ($option_name === '' || $lease_value === '') {
        return;
    }
...
function ll_tools_dictionary_live_search_rate_limit_key(): string {
    $identity = ll_tools_dictionary_live_search_client_identifier();
    $hash = function_exists('wp_hash') ? wp_hash($identity) : md5($identity);

    return 'll_dict_live_search_rl_' . md5($hash);
}

function ll_tools_dictionary_live_search_client_identifier(): string {
...
function ll_tools_dictionary_live_search_rate_limit_status(): array {
    ...
    return ll_tools_public_ajax_reserve_counter(
        'll_dict_live_search_rl_',
        ll_tools_dictionary_live_search_client_identifier(),
        ll_tools_dictionary_live_search_rate_limit_max_requests(),
        ll_tools_dictionary_live_search_rate_limit_window()
    );
}

function ll_tools_dictionary_live_search_rate_limit_allows(): bool {
    $status = ll_tools_dictionary_live_search_rate_limit_status();
    return !empty($status['allowed']);
}

function ll_tools_dictionary_live_search_reset_rate_limit(string $identifier = ''): void {
    if ($identifier === '') {
        $identifier = ll_tools_dictionary_live_search_client_identifier();
    }

    ll_tools_public_ajax_reset_counter('ll_dict_live_search_rl_', $identifier);
    delete_transient(ll_tools_dictionary_live_search_rate_limit_key());
}

function ll_tools_dictionary_live_search_client_inflight_limit(): int {
    return max(0, min(10, (int) apply_filters(
        'll_tools_dictionary_live_search_client_inflight_limit',
        2
    )));
}

function ll_tools_dictionary_live_search_client_inflight_ttl(): int {
    return max(5, min(120, (int) apply_filters(
        'll_tools_dictionary_live_search_client_inflight_ttl',
        30
    )));
}

function ll_tools_dictionary_live_search_acquire_client_inflight(): array {
...
function ll_tools_dictionary_live_search_reset_client_inflight(string $identifier = ''): void {
    if ($identifier === '') {
        $identifier = ll_tools_dictionary_live_search_client_identifier();
    }

    ll_tools_public_ajax_reset_client_leases('ll_dict_live_search_inflight_', $identifier);
}

function ll_tools_dictionary_live_search_build_wait_ms(): int {
    return max(0, min(5000, (int) apply_filters(
        'll_tools_dictionary_live_search_build_wait_ms',
        750
    )));
}

function ll_tools_dictionary_ajax_cache_wait_for_payload(string $namespace, array $args, int $wait_ms) {
    return ll_tools_public_ajax_wait_for_value(
        static function () use ($namespace, $args) {
            return ll_tools_dictionary_ajax_cache_get($namespace, $args);
        },
        $wait_ms
    );
}

function ll_tools_dictionary_send_cache_warming_ajax(int $retry_after = 1): void {
    ...
    wp_send_json_error([
        'code' => 'cache_warming',
        'message' => __('Search results are still being prepared. Please try again in a moment.', 'll-tools-text-domain'),
        'retry_after' => $retry_after,
    ], 202);
}

function ll_tools_dictionary_send_source_incomplete_ajax(?WP_Error $error = null): void {
    $retry_after = 2;
    if (!headers_sent()) {
        header('Retry-After: ' . $retry_after);
    }
    ...
    wp_send_json_error([
        'code' => $error instanceof WP_Error ? (string) $error->get_error_code() : 'dictionary_source_incomplete',
        'message' => $error instanceof WP_Error
            ? (string) $error->get_error_message()
            : __('Dictionary data is temporarily unavailable. Please try again in a moment.', 'll-tools-text-domain'),
        'retryable' => true,
        'retry_after' => $retry_after,
    ], 503);
}
...
/**
 * Abort one public dictionary request when a completeness-sensitive read failed.
 */
function ll_tools_dictionary_abort_on_source_incomplete_ajax(): void {
    $error = function_exists('ll_tools_dictionary_browser_get_query_error')
        ? ll_tools_dictionary_browser_get_query_error()
        : null;
    if ($error instanceof WP_Error) {
        ll_tools_dictionary_send_ajax_cache_header('SOURCE_INCOMPLETE');
        ll_tools_dictionary_send_source_incomplete_ajax($error);
    }
}

function ll_tools_dictionary_send_rate_limited_ajax(array $status = []): void {
...
function ll_tools_dictionary_build_toolbar_panel_context(int $wordset_id, array $source_ids = [], string $dialect = ''): array {
    if (function_exists('ll_tools_dictionary_browser_clear_query_error')) {
    ...
    $source_error = function_exists('ll_tools_dictionary_browser_get_query_error')
        ? ll_tools_dictionary_browser_get_query_error()
        : null;
    ...
    return [
        ...
        'retryable' => $source_error instanceof WP_Error,
        'error_code' => $source_error instanceof WP_Error ? (string) $source_error->get_error_code() : '',
        'message' => $source_error instanceof WP_Error ? (string) $source_error->get_error_message() : '',
    ];
}
...
function ll_tools_dictionary_render_toolbar_panel(
    ...
    if (!empty($context['retryable'])) {
        return '<div class="ll-dictionary__toolbar-panel ll-dictionary__toolbar-panel--retryable" role="status" data-ll-dictionary-toolbar-panel>'
            . esc_html((string) ($context['message'] ?? __('Dictionary data is temporarily unavailable. Please try again in a moment.', 'll-tools-text-domain')))
            . '</div>';
    }
...
function ll_tools_dictionary_handle_toolbar_bootstrap(): void {
    ...
    ll_tools_dictionary_abort_on_source_incomplete_ajax();
    ...
    $rate_limit_status = ll_tools_dictionary_live_search_rate_limit_status();
    if (empty($rate_limit_status['allowed'])) {
        ll_tools_dictionary_send_ajax_cache_header('RATE_LIMITED');
        ll_tools_dictionary_send_rate_limited_ajax($rate_limit_status);
    }
    ...
    if (is_array($cached)) {
        ...
        $lock_acquired = false;
        ll_tools_dictionary_send_ajax_cache_header('WAIT_HIT');
        wp_send_json_success($cached);
    }
    ...
    if ($source_error instanceof WP_Error) {
        ll_tools_dictionary_send_ajax_cache_header('SOURCE_INCOMPLETE');
        ll_tools_dictionary_send_source_incomplete_ajax($source_error);
    }
...
/**
 * Handle enhanced in-place entry detail requests for the dictionary shortcode.
 */
function ll_tools_dictionary_handle_entry_detail(): void {
    ...
    $raw_entry_id = $_POST['entry_id'] ?? ($_POST['ll_dictionary_entry'] ?? '');
    $entry_id = ll_tools_dictionary_resolve_entry_id($raw_entry_id, $wordset_id);
    ll_tools_dictionary_abort_on_source_incomplete_ajax();
    if ($entry_id <= 0) {
        wp_send_json_error([
            'message' => __('That dictionary entry could not be found.', 'll-tools-text-domain'),
        ], 404);
    }
    ...
    ll_tools_dictionary_abort_on_source_incomplete_ajax();
    $source_ids = ll_tools_dictionary_shortcode_resolve_source_ids_from_request($_POST);
    $source_query = ll_tools_dictionary_shortcode_build_source_query_value($source_ids);
    $dialect = isset($_POST['ll_dictionary_dialect']) ? trim(sanitize_text_field(wp_unslash((string) $_POST['ll_dictionary_dialect']))) : '';
    $base_url = ll_tools_dictionary_resolve_live_base_url(isset($_POST['base_url']) ? (string) wp_unslash((string) $_POST['base_url']) : '');
    $preferred_languages = ll_tools_dictionary_shortcode_resolve_display_languages($search_scopes, $wordset_id, $gloss_lang);
    $title_language = function_exists('ll_tools_dictionary_get_effective_title_language_code')
    ...
    $detail_url = ll_tools_dictionary_build_detail_url(
        $base_url,
        $entry_id,
        $search,
        $search_scopes,
    ...
    $cache_args = [
        ...
        'title_language' => $title_language,
        'linked_word_preview_limit' => ll_tools_dictionary_detail_linked_words_preview_limit(),
        'detail_view_schema' => 3,
    ];
    $cached = ll_tools_dictionary_ajax_cache_get('entry_detail', $cache_args);
    if (is_array($cached)) {
        ll_tools_dictionary_send_ajax_cache_header('HIT');
        wp_send_json_success($cached);
    }
    ...
    $lock_key = ll_tools_dictionary_ajax_cache_build_lock_key('entry_detail', $cache_args);
    $lock_acquired = ll_tools_dictionary_ajax_cache_acquire_build_lock($lock_key);
    if (!$lock_acquired) {
        $cached = ll_tools_dictionary_ajax_cache_get('entry_detail', $cache_args);
        if (is_array($cached)) {
    ...
    $_GET = [
        'll_dictionary_q' => $search,
        'll_dictionary_scope' => ll_tools_dictionary_shortcode_build_scope_query_value($search_scopes),
        'll_dictionary_letter' => $letter,
        'll_dictionary_pos' => $pos_slug,
        'll_dictionary_source' => $source_query,
        'll_dictionary_dialect' => $dialect,
        'll_dictionary_page' => (string) $page,
    ];
    $payload = [];
    ...
    try {
        try {
            ...
            $html = ll_tools_dictionary_render_detail_view($entry_id, $base_url, $preferred_languages, $source_ids);
        } finally {
            $_GET = $previous_get;
        }

        $source_error = function_exists('ll_tools_dictionary_browser_capture_query_error')
            ? ll_tools_dictionary_browser_capture_query_error('entry_detail_render')
            : null;
        if (!($source_error instanceof WP_Error)) {
            $payload = [
                'html' => $html,
            ...
            ll_tools_dictionary_ajax_cache_set('entry_detail', $cache_args, $payload);
        }
    } finally {
        if ($lock_acquired) {
            ll_tools_dictionary_ajax_cache_release_build_lock($lock_key);
...
add_action('wp_ajax_ll_tools_dictionary_entry_detail', 'll_tools_dictionary_handle_entry_detail');
add_action('wp_ajax_nopriv_ll_tools_dictionary_entry_detail', 'll_tools_dictionary_handle_entry_detail');
...
function ll_tools_dictionary_handle_live_search(): void {
    ...
    if ($has_active_browse_query) {
        ...
        $lock_acquired = ll_tools_dictionary_ajax_cache_acquire_build_lock($lock_key);
        if (!$lock_acquired) {
            $cached = ll_tools_dictionary_ajax_cache_wait_for_payload(
                'live_search',
                $cache_args,
                ll_tools_dictionary_live_search_build_wait_ms()
            );
            ...
            ll_tools_dictionary_send_cache_warming_ajax(1);
        }
        $cached = ll_tools_dictionary_ajax_cache_get('live_search', $cache_args);
        if (is_array($cached)) {
            ll_tools_dictionary_ajax_cache_release_build_lock($lock_key);
        ...
        if (!is_user_logged_in()) {
            $client_lease = ll_tools_dictionary_live_search_acquire_client_inflight();
            if (empty($client_lease['acquired'])) {
                ll_tools_dictionary_ajax_cache_release_build_lock($lock_key);
                $lock_acquired = false;
                ll_tools_dictionary_send_cache_warming_ajax(
                    (int) ($client_lease['retry_after'] ?? 1)
                );
            }
        }
    }
    ...
    try {
        ...
        if (!empty($query['retryable'])) {
            ...
            if (!($source_error instanceof WP_Error)) {
                $source_error = new WP_Error(
                    (string) ($query['error_code'] ?? 'dictionary_source_incomplete'),
                    (string) ($query['message'] ?? __('Dictionary data is temporarily unavailable. Please try again in a moment.', 'll-tools-text-domain'))
                );
            }
            $payload = [];
        } else {
        ...
        if (!empty($client_lease['option_name'])) {
...
function ll_tools_dictionary_shortcode($atts = [], $content = null, $tag = ''): string {
    ...
    $has_active_browse_query = ($search !== '' || $letter !== '' || $pos_slug !== '' || !empty($source_ids) || $dialect !== '' || $source_error instanceof WP_Error);
    ...
    if ($source_error instanceof WP_Error) {
        ...
        $query['error_code'] = (string) $source_error->get_error_code();
        $query['message'] = (string) $source_error->get_error_message();
        $query['retry_after'] = 2;
    } elseif ($has_active_browse_query) {
        $query = ll_tools_dictionary_run_browse_query(
            $wordset_id,
            $search,
            $search_scopes,
