(function (root) {
    ...
    function queueForRepetition(targetWord, options) {
        ...
        const offset = (applyStarBias && isStarred(targetWord.id))
            ? ((Util && typeof Util.randomInt === 'function') ? Util.randomInt(2, 3) : (Math.floor(Math.random() * 2) + 2))
    ...
    function selectTargetWord() {
        if (FlashcardOptions && typeof FlashcardOptions.calculateNumberOfOptions === 'function') {
            FlashcardOptions.calculateNumberOfOptions(State.wrongIndexes, State.isFirstRound, State.currentCategoryName);
        } else if (Array.isArray(State.wrongIndexes)) {
    ...
    function handleNoTarget(ctx) {
        const genderSupported = (Selection && typeof Selection.isGenderSupportedForCategories === 'function')
            ? Selection.isGenderSupportedForCategories(State.categoryNames)
            : false;
        if (State.isFirstRound && !genderSupported) {
    ...
    function configureTargetAudio(target) {
        ...
        const preferredOrder = ['isolation', 'question', 'introduction', 'in sentence'];
