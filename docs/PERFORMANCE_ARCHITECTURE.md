...
| Public wordset buttons | `includes/shortcodes/wordset-buttons-shortcode.php`, `includes/taxonomies/word-category-taxonomy.php` | A cold anonymous render must never synchronously scan every lesson/category pair or every word/prompt card after a content-epoch change. Discover lesson IDs in bounded keyset pages, resume one exact pair through shared prompt-card/raw-word query and row budgets, and publish counts only after the complete generation is fenced to its current epochs and lock. Preserve only short-lived, structurally scoped anonymous last-known-good HTML while rebuilding; never share signed-in/private output. Source failures back off, ordinary budget exhaustion advances one deduplicated anonymous cron continuation, signed-in requests remain bounded without creating cron, and evicted/expired state options and paired locks must be reclaimed. Default non-opt-in quiz callers retain their existing exact behavior. |
