...
| Progress Select all and bounded Learning launches | `includes/user-progress.php`, `includes/user-study.php`, `includes/wordset-isolation.php`, `includes/pages/wordset-pages.php`, `js/wordset-pages.js` | Treat exact-ID acquisition, launch planning, current-chunk hydration, and flashcard commit as one single-flight operation. The click opens the popup/loader synchronously. Reuse a complete filtered-page ID snapshot only when count and filter/visibility/category signature match; invalidate it on every analytics refresh and progress/star mutation. Send the canonical selection as one scalar `candidate_word_ids` field, intersect it with current renderable wordset/category membership without reapplying the progress criterion, disable conflicting controls while active, actively abort whichever ID/plan/hydration request is current after dismissal or a scope change, and token-fence every callback/retry. Keep the inline loading state until commit and preserve one unchanged Retry specification after a fetch, plan, or hydration failure. ID-only paths read starred IDs directly instead of repairing the entire saved category state. Large exact selections must use the bounded planner and first-chunk hydration in all five modes; only Practice may stream later chunks into one pre-results logical session, while Learn, Listen, Gender, and Self Check advance through explicit results-stage continuation. The user-invariant presentation aggregate is keyed by an explicit schema plus wordset and structural/content epochs so different users and routine releases share it safely. Learning chunks must remain presentation- and aspect-compatible, contain 8-15 words across no more than eight categories, and track exact target IDs separately from compatible filler IDs. Allocate shared filler words across every compatibility group before finalizing chunk ownership so a flexible group cannot starve a constrained group. Never expand a bounded Learning launch through complete-category hydration or broader cached rows; fail closed when no compatible minimum chunk exists, and advance later Learning chunks through the results-screen Next action rather than Practice's continuous pre-results continuation. |
