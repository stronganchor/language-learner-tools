...
| Wordset landing pages, search, lazy cards | `includes/pages/wordset-pages.php`, `js/wordset-pages.js` | Keep the first paint bounded. Use category summaries, shell cards, and AJAX hydration instead of loading every word/media item. Avoid per-word postmeta loops in request setup. Preview deduplication may use attachment IDs, URLs, stored path identity, and already-stored visual-hash metadata; it must not read/hash attachment files or compute missing visual hashes during interactive rendering. |
...
| Wordset editor/settings | `includes/pages/wordset-editor.php`, `includes/pages/wordset-pages.php` | Keep editor rows paged. Respect `ll_tools_wordset_editor_can_use_paged_query()` and avoid falling back to all-row builds unless the operation is intentionally small or filtered. Effective-image summary, filter, and sort paths must reuse the cached keyset-batched word-ID membership index; do not put the correlated effective-image predicate back into a whole-wordset count or order query. The settings hub should use stored settings and aggregate term counts only; it must not hydrate full editor rows, media/template catalogs, offline export options, or the generic per-category preview catalog before a tool is opened. Recorder overviews may materialize and cache the compact category identity list needed for ordering and pagination, but must not build the full learner-card/media catalog; summary rendering then operates on one category page, image-to-word resolution is limited to the current candidate batch, and hidden-entry wordset membership is resolved in bulk. Hidden queue details must remain paged rather than rendering the full retained list. |
