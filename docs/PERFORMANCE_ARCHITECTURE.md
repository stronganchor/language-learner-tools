...
| --- | --- | --- |
| Flashcard and quiz payloads | `includes/shortcodes/flashcard-widget.php`, `includes/shortcodes/quiz-pages-shortcodes.php`, `includes/taxonomies/word-category-taxonomy.php`, `includes/lib/public-ajax-resource-guards.php` | Treat `ll_get_words_by_category()` as an expensive hydration path. Use `ll_get_words_by_category_count()` or bounded candidate IDs when only counts, eligibility, or a launch subset are needed. Anonymous AJAX cache misses reserve atomic weighted per-client budgets, at most two exact-owner client leases, and one exact cache-key build lease; cache hits bypass admission, duplicate builders receive retryable warming responses, and candidate input is capped before traversal. No-candidate full-category reads use the existing durable `wp_ll_flashcard_payload_rows` materializer and signed page cursors; never restore a synchronous whole-category build or mistake the admission guard for a bound on proportional work. Durable quiz-catalog workers finish one valid partial generation when no usable latest snapshot exists, but a plugin-versioned builder token resets incompatible pre-deploy chunks and an empty stale snapshot never justifies resetting the only advancing build. Workers must not persist per-category derived count/gender/aspect/default-wordset/eligibility transients. AJAX and signed no-JavaScript continuations each advance bounded work without running unrelated cron callbacks. |
| Public wordset buttons | `includes/shortcodes/wordset-buttons-shortcode.php`, `includes/taxonomies/word-category-taxonomy.php` | A cold render must never synchronously scan every lesson/category pair or every word/prompt card after a content-epoch change. Discover lesson IDs in bounded keyset pages, resume one exact pair through shared prompt-card/raw-word query and row budgets, and publish counts only after the complete generation is fenced to its current epochs, builder schema, user identity, and lock. Preserve only complete, structurally scoped anonymous last-known-good or exact-cache HTML while rebuilding; signed-in scopes may consume that public subset but must never write private output to it. An incomplete signed-in shell may additionally render private terms from the current viewer's freshly resolved exact visibility scope with a hidden presentation-only count sentinel, keeping their links usable without publishing provisional membership or counts. Anonymous HTML publication must reuse captured pre-render exact/LKG keys and refuse any epoch drift so old public scope can never poison a new generation. A nonce-protected authenticated loader may advance one bounded batch per serial request. Anonymous cold responses must be non-cacheable and use the separate rate-limited `ll_tools_wordset_buttons_status` action with an expiring HMAC token bound to normalized attributes and current public epochs; status polling accepts no caller scope, never acquires a builder lock or scans eligibility, and only reads complete state while scheduling the existing deduplicated cron continuation. Both paths replace the shell only after exact completion and expose Retry after their wait bound. Source failures back off, continuation events carry the initiating user ID, and the worker restores that user context only while advancing the state. A proven complete-empty scope remains empty. Evicted/expired state options and paired locks must be reclaimed, and default non-opt-in quiz callers retain their existing exact behavior. Direct role-change invalidation is a separate maintenance item. |
| Wordset landing pages, search, lazy cards | `includes/pages/wordset-pages.php`, `includes/lib/wordset-category-search-index.php`, `includes/user-progress.php`, `js/wordset-pages.js` | Keep the first paint and activity startup bounded. Use category summaries, shell cards, and AJAX hydration instead of loading every word/media item. Logged-in filtered or broad selections, explicit signed-in categories, and full-scope signed-in fallbacks resolve through one nonce-protected ID-only launch plan that, on success, preserves every exact match in balanced, category-aware transport chunks; a sparse layout that cannot form minimum-size rounds within the eight-category hard cap fails closed instead of returning a partial plan. Direct words marked as specific-wrong-answer-only are option-support rows, not plan targets; do not filter the final canonical union because prompt-card answers using those same word IDs remain legitimate targets. Keep category-owned queues contiguous, order owned queues largest-first, then order runnable chunks by fewest represented categories, fullest word count, and stable original order so startup needs the fewest category requests; retain the filter label across hydration boundaries. Candidate-specific requests hydrate only the current chunk. Practice, Listen, and Self Check append verified chunks into one logical pre-results session without reinitializing cumulative progress or refetching successful rows. Gender owns homogeneous starting-level groups exactly once, checkpoints Level 1 at results, and automatically advances Levels 2/3; Learning retains explicit results-stage continuation. Never flatten the plan into an all-category hydration drain. A 429 or candidate failure closes the loader and leaves the unchanged chunk retryable instead of advancing or retrying in the foreground indefinitely. Word/translation search reads only a complete published generation from the durable category-search table. Builders advance through bounded ID-keyset, relationship, row, and byte batches; exact-owner leases, dependency-signature compare-and-swap publication, generation rotation on expired takeover, and bounded old-generation cleanup prevent stale workers from publishing. Public requests expose loading/error/Retry states while preparation advances and never fall back to rebuilding the complete search index inline. Filter categories already proven ineligible before computing content summaries; bulk-prime the exact term, lesson, and wrong-answer-owner candidates; and do not re-query flat taxonomy membership already established by the category-scoped candidate query. Avoid per-word postmeta loops in request setup. Preview deduplication may use attachment IDs, URLs, stored path identity, and already-stored visual-hash metadata; it must not read/hash attachment files or compute missing visual hashes during interactive rendering. |
| Inactive lesson previews and published lesson map | `includes/pages/wordset-pages.php`, `js/wordset-pages.js` | Staff inactive-category previews convert word images in resumable 10-item ID-keyset batches (25 hard maximum per continuation) behind a renewable exact-owner lease. Advance the persisted cursor only after complete success or idempotent disappearance; retry failures on the same item. Keep one serial browser continuation with visible progress and preserve no-JavaScript resume. The complete lesson map has one cold-build owner and returns a seven-day last-known-good map immediately under contention; a truly cold contender fails closed. The winning cold rebuild still enumerates all published lessons, so durable materialization remains a measurement-led follow-up rather than a hidden claim that the builder itself is bounded. |
| Progress Select all and bounded multi-mode launches | `includes/user-progress.php`, `includes/user-study.php`, `includes/wordset-isolation.php`, `includes/pages/wordset-pages.php`, `js/wordset-pages.js` | Treat exact-ID acquisition, launch planning, current-chunk hydration, and flashcard commit as one single-flight operation. The click opens the popup/loader synchronously. Reuse a complete filtered-page ID snapshot only when count and filter/visibility/category signature match; invalidate it on every analytics refresh and progress/star mutation. Send the canonical selection as one scalar `candidate_word_ids` field, intersect it with current renderable wordset/category membership without reapplying the progress criterion, disable conflicting controls while active, actively abort whichever ID/plan/hydration request is current after dismissal or a scope change, and token-fence every callback/retry. Keep the inline loading state until commit and preserve one unchanged Retry specification after a fetch, plan, or hydration failure. ID-only paths read starred IDs directly instead of repairing the entire saved category state. All five modes use the bounded planner and first-chunk hydration. Practice, Listen, and Self Check continue through verified chunks before final results; Self Check must not score the same-image word twice across a boundary. Gender groups the immutable logical scope by effective starting level, caps Level 1 groups at 10 and Levels 2/3 at 15, checkpoints after Level 1, and automatically advances Levels 2/3 while allowing newer browser progress to re-bucket a stale hint without dropping or duplicating the word. The user-invariant presentation aggregate is keyed by an explicit schema plus wordset and structural/content epochs so different users and routine releases share it safely. Learning chunks remain presentation- and aspect-compatible, contain 8-15 words across no more than eight categories, prefer at most 12 words in large scopes, and use 13-15 only when a valid eight-word split is impossible; they track exact target IDs separately from compatible filler IDs. Allocate shared filler words across every compatibility group before finalizing chunk ownership so a flexible group cannot starve a constrained group. Never expand a bounded Learning launch through complete-category hydration or broader cached rows; fail closed when no compatible minimum chunk exists, and advance later Learning chunks through the results-screen Next action rather than automatic continuation. |
...
| Dictionary public filters | `includes/shortcodes/dictionary-shortcode.php`, `includes/lib/dictionary-static-cache.php` | Before normalization, recursive static-cache traversal, registry hydration, cache-key work, admission, or SQL construction, hard-clamp raw bytes, value bytes, cardinality, and shapes for all nine public query keys: `q`, scope, page, canonical/legacy letter, POS, source, dialect, and entry. Only scope/POS/source accept bounded multi-values; scalar arrays are invalid. AJAX rejects invalid input while GET/static-cache paths drop only the invalid argument to its safe default. Keep deduplication lookup-based rather than repeatedly scanning a growing result array. |
| Public language switching | `includes/shortcodes/language-switcher-shortcode.php`, `css/language-switcher.css`, `js/language-switcher.js` | Render locale choices as nonce-protected POST forms so cacheable public HTML does not advertise one dynamic URL per locale to crawlers. Keep legacy signed GET requests compatible and redirect both valid POST and legacy GET actions to the clean current URL. |
| AI crawler exports | `includes/lib/ai-crawler-support.php` | Keep generated exports bounded and route-cache backed. Evaluate public eligibility independently of the current user and password cookie, including dictionary initials and linked term labels. Never include a post with a stored password. Cache publication must retain the captured generation, and old privacy schemas cannot be reused. HEAD requests stay header-only and never build cold export bodies. |
| Manual category ordering | `includes/taxonomies/wordset-taxonomy.php` | A complete saved order needs no lesson-date query. Only missing candidate categories enter the grouped first-lesson-date query, in chunks of 200 IDs, without loading lesson posts or their meta into PHP. Preserve earliest local-date ordering, deterministic ID ties, first category-meta semantics, and the selected lesson's valid GMT timestamp. Failed reads must remain uncached. |
| WebP optimizer queues | `includes/admin/image-webp-optimizer-admin.php` | Require the current viewer's post, media, owner-wordset, and category access before hydrating queue metadata. Cache IDs/counts by viewer permissions, assignments, and visibility generations; discard a build when that identity changes before publication. Conversion still requires edit permission. |
| Import and metadata job checkpoints | `includes/lib/mutation-job-state.php`, `includes/admin/export-import.php`, `includes/api/word-metadata-plan-rest.php` | Acquire a connection-owned lock on the canonical storage identity before loading durable job state. Fence checkpoint writes to the owning connection and expected stored value. Persist an in-progress marker before side effects; uncertain completion requires explicit recovery and must never offer automatic Resume. Processing, discard, and cleanup share the lock. |
| Offline export and builder preparation | `includes/admin/offline-app-export.php`, `offline-app-builder/scripts/prepare-bundle.mjs` | Hold the per-job file lock across the fresh manifest read, append work, and atomic manifest replacement. A leftover pending-step marker terminates that export instead of replaying uncertain appends. Cleanup uses the same lock, whose stable file lives outside the deleted job directory. Builder preparation validates staging before replacing output, rejects nested directory links/junctions, and rolls back bundle/config/state publication errors. |
...
| Masked media proxy | `includes/lib/media-proxy.php` | A missing local file may fall back only to a validated public URL. Stream into a same-bucket temporary file with `max + 1` transport enforcement, validate final size and raster MIME, and publish under an exact-owner key lease. Fresh disk hits avoid origin traffic; contenders wait briefly for the owner, then serve validated stale data only within the 14-day absolute ceiling or redirect to the safe origin. Origin failures back off for five minutes. Keep each attachment/size bucket to four images and 32 MB by default, remove all deterministic attachment shards on deletion, and prune the 65,536 attachment/size shards through a daily bounded cursor plus one-minute continuations; native shard seeks remain capped at 1,024 entries. Stream through an already-open handle so Windows replacement cannot produce an empty 200, and keep client freshness no longer than the server TTL. |
| Expired transient maintenance | `includes/lib/expired-transient-maintenance.php` | Run only from the hourly cron hook. With an external object cache, skip general cache-transient rows but still reclaim the direct atomic guard rows LL Tools writes to `wp_options`; otherwise use the full exact audited LL-owned cache/rate-limit prefix allowlist. Candidate selection remains capped at 200 timeout rows older than a five-minute grace period, a one-second default loop budget, and a two-second hard cap. Each deletion rechecks the selected timeout ID, name, value, and expiry in the same SQL statement that removes its exact value row; a direct lease also needs its embedded expiry past the grace cutoff, so a takeover between value and timeout refresh survives. Timeout-only rows can be reclaimed, and a saturated/time-limited pass schedules one coalesced continuation. Never broaden this into global transient cleanup, persistent `ll_tools_*` option/job cleanup, or a hot-path fallback. Telemetry may expose aggregate counts, namespace buckets, and byte totals, never keys or values. |
...
| Automation raw-password compatibility | `includes/api/automation-rest.php`, `includes/lib/expired-transient-maintenance.php` | Bound raw header/username/password bytes before Base64 decoding or `wp_authenticate()`, then reserve an atomic coarse canonical direct-peer allowance and a peer-plus-normalized-login allowance. This prevents oversized inputs and rotating usernames from bypassing admission. Successful raw-password attempts refund both reservations, failures remain generic, and exhausted callers receive 429 plus `Retry-After`. Cookie and application-password successes bypass this compatibility guard; expired counter rows remain within the audited hourly cleanup namespace and budget. |
