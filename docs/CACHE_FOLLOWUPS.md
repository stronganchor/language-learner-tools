...
- Locale-switch choices are nonce-protected POST forms, so cacheable public HTML no longer advertises one dynamic locale URL per language to crawlers. Legacy signed GET URLs remain compatible, while unsigned or expired GET/HEAD/POST requests redirect to the clean URL without saving a locale.
...
- Anonymous dictionary live-search and flashcard category AJAX misses now reserve atomic weighted per-client budgets and exact-owner concurrency leases. A single exact query/category cache-key owner builds while duplicate callers wait briefly for the cache or receive a retryable warming response; cache hits bypass admission. Candidate-ID payloads are bounded before traversal. The remaining admitted no-candidate flashcard cold build is deliberately tracked as a durable payload-materialization project rather than hidden behind a result cap.
- Wordset category word/translation search now reads a complete published generation from a durable materialized table. Bounded keyset/relationship/write batches, generation-scoped rows, expiring exact-owner leases, dependency-signature compare-and-swap publication, and bounded old-generation cleanup keep stale workers from exposing partial or superseded indexes. The browser retains loading and explicit Retry states while the materializer advances.
- Dictionary query-result transients store ordered entry IDs plus pagination/filter metadata instead of duplicating each entry's full display payload. Reads hydrate through the canonical per-entry cache, batch-prime object/database-transient values for the bounded result page, and continue accepting legacy full query payloads until they expire naturally.
...
- Re-audit public language-switcher markup after cache/template changes to ensure nonce-bearing GET links are not reintroduced and cached forms still receive refreshed nonce placeholders.
