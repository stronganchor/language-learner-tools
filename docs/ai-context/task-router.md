...
| Public wordset-button shortcode, anonymous homepage lesson counts, quiz-eligibility cache misses, resumable aggregate workers | `wordset-vocab-manager` | `ll_wordset_buttons`, `wordset-buttons-shortcode`, `ll_tools_get_wordset_button_lesson_counts_bounded`, `ll_can_category_generate_quiz`, `quiz content epoch`, `last-known-good`, `raw query budget`, `prompt-card cursor` |
