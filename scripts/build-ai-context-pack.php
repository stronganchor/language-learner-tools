    ...
    return [
        ...
        'wordset-vocab-manager' => [
            ...
            'invariants' => [
                ...
                'Logged-in broad or progress-filtered selections use one capped ID-only launch plan and candidate-specific transport hydration. Progress-table Select all sends its canonical IDs as one bounded scalar candidate_word_ids field; the server intersects that exact snapshot with current renderable wordset/category membership without reapplying the progress criterion.',
                'Progress-table Select all launch orchestration is single-flight from exact-ID acquisition through plan, current-chunk hydration, and flashcard commit. Disable conflicting controls while active, abort or token-fence stale scope, retain the loading state until commit, and expose one unchanged inline Retry specification after failure.',
